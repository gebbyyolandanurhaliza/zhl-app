...
function pesan($pesan, $tipe, $pdf_link = '', $extra_button = '', $xls_link = '', $edit_link = '', $edit_caption = '', $direct_link = '', $direct_caption = '')
{
  switch ($tipe) {
    case 'success':
  ...
  if ($pdf_link) {
    $note .= "<a class='btn red' target='_blank' href='$pdf_link'><i class='fa fa-file-pdf-o'></i> View PDF</a>";
  }
  ...
  if ($edit_link) {
    $edit_caption_show = ($edit_caption) ? $edit_caption : 'Edit Last Transaction';
    $note .= "<a class='btn yellow-gold' href='$edit_link'><i class='fa fa-pencil'></i> $edit_caption_show</a>";
  }

  if ($direct_link) {
    $direct_caption_show = ($direct_caption) ? $direct_caption : 'Go To Next Transaction';
    $note .= "<a class='btn blue-madison' href='$direct_link'><i class='fa fa-arrow-right'></i> $direct_caption_show</a>";
  }
...
function encode_str($value, $gembok = '')
{
  ...
  $skey = (trim_all($gembok) == '' ? 'd0n`t-tRyth15_@h' : $gembok);
...
function convert_number_to_words_2($number)
{
  $hyphen      = ' ';
  $conjunction = ' ';
  $separator   = ' ';
  $negative    = 'negative ';
  $decimal     = ' Singapore Dollars and ';
  $dictionary  = array(
    0                   => 'Zero',
    1                   => 'One',
    2                   => 'Two',
  ...
  switch (true) {
    ...
    case $number < 1000:
      ...
      if ($remainder) {
        $string .= $conjunction . convert_number_to_words($remainder, $dictionary);
      }
      break;
    default:
      $baseUnit = pow(1000, floor(log($number, 1000)));
      ...
      if ($remainder) {
        $string .= $remainder < 100 ? $conjunction : $separator;
        $string .= convert_number_to_words($remainder, $dictionary);
      }
      break;
  }
  ...
  if (null !== $fraction && is_numeric($fraction)) {
    ...
    $string .= $stringdecimal . ' Cents Only';
  }
...
{
  if (date('Y', strtotime($date)) == '1970' || date('Y', strtotime($date)) == '0000')
    return '';

  return date($format, strtotime($date));
}

function setDateFormat2($date, $format)
{
  // if (date('Y', strtotime($date)) == '1970' || date('Y', strtotime($date)) == '0000')
  // 	return '';

  // $dateString = '08/04/2023';
  $dateCreate = date_create_from_format('d/m/Y', $date);
  return date_format($dateCreate, $format); // Output: 2023-04-08

  // return date($format, strtotime($date));
}

function getInOutward($tipe)
{
  if ($tipe == 1) {
    return "Outward List";
  } else if ($tipe == 2) {
...
function getContainerType($tipe)
{
  if ($tipe == 1)  return 20;
...
// {
// 	echo "<h5 style='font-weight: Bold; font-size: 20px;'>".$param['title']."</h5>";

// 	echo '<nav aria-label="breadcrumb">';
// 		echo '<ol class="breadcrumb">';
// 			foreach($param['det'] as $val):
