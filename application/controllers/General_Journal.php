...
class General_Journal extends CI_Controller {
    ...
    function __construct() {
        ...
        $this->load->model(array('M_General_Journal','M_vcdn', 'm_po_journal'));
    ...
    public function index() {
        ...
        $data['List_payable'] = $this->M_General_Journal->get_list_hutang();
        $data['List_general'] = $this->M_General_Journal->get_list();
       $this->template->display('accounting/General_Journal/General_Journal_list', $data);
    }

     function ambil_currency() {
        $kurs= $this->input->get('kurs');
        $tgl = date('Y/m/d',strtotime($this->convert($this->input->get('tgl'))));
        $data['currency'] = $this->M_General_Journal->tampil_po_rate($kurs, $tgl);
    ...
    function add_new() {
        ...
        $date = date('Y-m-d');
        $data['SupplierID'] = $this->M_General_Journal->get_supplier();
        $data['dept_code'] = $this->M_General_Journal->get_departmentcode();
        $data['dept_code_json'] = json_encode($data['dept_code']);

        $data['Currency'] = $this->M_General_Journal->get_currency($date);
        $data['List_coa'] = $this->M_vcdn->get_coa($company);

        $data['_reff'] = $this->get_refnumber($date);
        $data['cur']=  $this->m_po_journal->tampil_cur();
        $this->template->display('accounting/General_Journal/General_form', $data);
    }

    function edit() {
        // $data['SupplierID'] = $this->M_General_Journal->get_supplier();
        // $data['Currency'] = $this->M_General_Journal->get_currency();
        // $data['CurrencyID'] = $this->M_General_Journal->get_currency_detail();
        // //cari nota debet, berdasarkan total seluruh transaksi "Bayar"
        // $id = $this->input->get("id");

        // //variable invoice number header
        // $data['HeaderID'] = $id;
        // $data['nota'] = $this->M_General_Journal->nota($id);
        // $data['get_data_header'] = $this->M_General_Journal->get_data_header($id);
        // //variable invoice number detail
        // $data['get_data_detail'] = $this->M_General_Journal->get_data_detail($id);
        // //variable invoice number footer
        // $data['get_data_footer'] = $this->M_General_Journal->get_data_footer($id);
        $id = $this->input->get('id');
        $company = strtoupper($this->session->userdata('company_id'));
    ...
    function list_currency() {
        $data['Currency'] = $this->M_General_Journal->get_currency_detail();
        $this->load->view('accounting/list_currency', $data);
    }

    function get_currency_date() {
        $sekarang = date($this->uri->segment(3));
        $data['Currency'] = $this->M_General_Journal->get_currency_date($sekarang);
    ...
    function list_payable() {
        ...
        //variable invoice number header
        $this->M_General_Journal->get_data_header($id);
    }
    ...
    function get_refnumber($tgli){
        ...
        // echo $date;
        $ini = $this->M_General_Journal->get_refnum($tahun, $bulan);

        if(empty($ini)){
            $ref = 'GV'.$bulan.$tahun.'0001';
            // echo $ref;
        }
        else{
             $no1 = $ini->no_reff;
    ...
    function get_refnumber1(){
        ...
        $this->load->view('accounting/General_Journal/General_ref', $data);
    }
    ...
    function save(){
        ...
        if($submit == 'Save'){
            for($j = 0; $j < count($txtdebet); $j++){
                ...
                $this->M_General_Journal->simpan_header($data);
            }
            ...
            for($i = 0; $i < count($txtdebet); $i++){
                ...
                $this->M_General_Journal->simpan_jurnal($data_jurnal);
            }
            redirect("General_Journal/edit?id=$no_ref");
        }
        else{
            $this->M_General_Journal->delete_header($no_ref);
            $this->M_General_Journal->delete_jurnal($no_ref);
        ...
        //     redirect("General_Journal/add_new");
        // }
        // else
        // {
    ...
    function delete() {
        ...
        $this->M_General_Journal->delete_item($id);
     ...
     function hapus(){
        ...
        $cek_gl = $this->M_General_Journal->cek_gl();
        ...
        if($id != $cek_gl->no_reff){
            ...
            $this->M_General_Journal->update_header($id,$data_hdr);
            ...
            $this->M_General_Journal->update_jurnal($id,$data_jur);
        } else {
            $this->M_General_Journal->delete_header($id);
            $this->M_General_Journal->delete_jurnal($id);
        }

        if ($this->db->trans_status() === FALSE)
        {
            $this->db->trans_rollback();
            redirect("General_Journal?stat=notok");
        }
        else
        {
            $this->db->trans_commit();
            redirect("General_Journal?stat=ok");
        }
    ...
    function print_gj() {
        ...
        $jurnal = $this->M_General_Journal->get_jurnal($id);
        $totals = 0;
        if(!empty($jurnal))
        {
            foreach ($jurnal as $v) {
    ...
    function selectInvoiceGJ(){
        $data   = array(
            '_selectHeader' => $this->M_General_Journal->selectInvoiceforFindGJ()->result()
        );
        $this->load->view('accounting/General_Journal/FindIGJ/selectIGJ',$data);
    }
}
