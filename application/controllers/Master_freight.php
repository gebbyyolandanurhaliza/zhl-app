...
class Master_freight extends CI_Controller
{
    ...
    function index()
    {
        $data['dest']=$this->M_master_freight->get_port_destination();
        $data['cont']=$this->M_master_freight->get_container_type();
        $data['ship']=$this->M_master_freight->tradingterm_get_fob();
        $data['con'] =$this->M_master_freight->get_consignee();
        $data['freight']=$this->M_master_freight->tampil_mst_freight();
    ...
    function add()
    {
        ...
        $cbo_consignee      = $this->M_master_freight->get_consignee();
        $cbo_shipping_line  = $this->M_master_freight->get_shipping_line();
    ...
    function save()
    {
        ...
        if($insertid != ''){
        $this->session->set_flashdata('message', pesan('Freight Charge Succesfully Saved.', pesan_sukses()));        
        redirect(site_url('master-freight/edit/'.$insertid));
        }else{
        $this->session->set_flashdata('message', pesan('Freight Charge Saving Error.', pesan_error()));        
        redirect(site_url('master-freight/add/'));            
        }
    }
    ...
    function update()
    {
        ...
        redirect(site_url('master-freight/edit/'.$id));
    }
    ...
    function container_stock_filter(){
        ...
        $data['freight']=$this->M_master_freight->filter_freight($dest,$cont,$ship,$con);
        $this->load->view('shipping/master_freight/freight_charges/freight_filter',$data);
    }
    function container_stock_filterpss(){
        $dest = $this->input->get('dest');
        $cont = $this->input->get('cont');
        $ship = $this->input->get('ship');
        $con  = $this->input->get('con');
        ...
        $data['freight']=$this->M_master_freight->filter_freight($dest,$cont,$ship,$con);
        $this->load->view('shipping/master_freight/freight_charges/freight_filterpss',$data);
    }

    function get_new_save(){
        $data['action']               = site_url('master_freight/save_new');
        ...
        $data['cbo_container_type'] = $this->M_master_freight->get_container_type();
        $data['cbo_port']           = $this->M_master_freight->get_port();
        $data['cbo_country']        = $this->M_master_freight->get_country();
        $data['cbo_consignee']      = $this->M_master_freight->get_consignee();
        $data['cbo_shipping_line']  = $this->M_master_freight->get_shipping_line();
        $data['cbo_fob']            = $this->M_master_freight->tradingterm_get_fob();

        $this->load->view('shipping/master_freight/freight_charges/freight_form_new_save',$data);        
    }
    ...
    function save_new()
    {
        ...
        redirect(site_url('master-freight/edit/'.$id_old));            
        }
    }

    function toexcel_master_freight(){
        $this->load->library("excel/PHPExcel");
        $data['freight']=$this->M_master_freight->tampil_mst_freight();
        $this->load->view('shipping/master_freight/freight_charges/toexcel_master-freight',$data);
    }
    ...
    function pss()
    {
        ...
        $data['con'] =$this->M_master_freight->get_consignee();
        $data['freight']=$this->M_master_freight->tampil_mst_freight();
        $this->template->display('shipping/master_freight/freight_charges/freight_listpss', $data);
    }
}
