...
class APtrans_zht extends CI_Controller{
    public function __construct() {
        ...
        $this->load->model(array('M_CashBank','M_Fin_AP_zht','M_Fin_CB','M_General_Journal','M_vcdn'));
    }
    ...
    function index(){
        $company   = 2;
        $data['_periode']           = $this->M_Fin_AP_zht->getPeriod_map();
        ...
        $this->template->display('finance/transaction/ap_trans_zht/index',$data);
    }
    ...
    function newGenerateReffNumber2(){
        ...
        $num    = $this->M_Fin_AP_zht->newCheckReffNumber2('OUT', $tanggal, $currency);
        $get    = str_pad($num, 4, 0, STR_PAD_LEFT);
        $set    = 'ZHTP'.'/'.date('Y',strtotime($this->input->get('txtDateForGen'))).'/'.date('m',strtotime($this->input->get('txtDateForGen'))).'/'.$get;
        echo $set;
    ...
    function generateReffNumAP(){
        $num    = $this->M_Fin_AP_zht->generateReffAP();
        $get    = str_pad($num, 4, 0, STR_PAD_LEFT);
        $set    = date('ym').$get;
        echo    $set;
    }
    ...
    function selectSupplierForAP(){
        ...
        $data   = array(
            '_selectSupplier' => $this->M_Fin_AP_zht->selectSupplierforAPtrans($tgl)->result()
        );
        $this->load->view('finance/transaction/ap_trans_zht/selectSupplier',$data);
    }
    function selectInvoiceForAP(){
        ...
        $data   = array(
            'idSupp'    => $idSupp,
            '_arrDraf'  => $this->M_Fin_AP_zht->selectInvoiceInDrafArray(),
            '_selectInvoice' => $this->M_Fin_AP_zht->selectInvoiceWhereForAPtrans($idSupp, $idDate)->result()
        );
        $this->load->view('finance/transaction/ap_trans_zht/selectInvoice',$data);
    }
    function selectModalAP(){
        $data   = array(
            '_selectAP' => $this->M_Fin_AP_zht->selectAPfromAccounting()->result()
        );
        $this->load->view('finance/transaction/ap_trans_zht/selectAP',$data);
    }
    function selectCOA(){
        $company   = 2;
        $data   = array(
            '_getMasterCOA' => $this->M_Fin_AP_zht->selectCOAforAPtrans($company)->result()
        );
        $this->load->view('finance/transaction/ap_trans_zht/selectMCOA',$data);
    }
    function selectCurrencyAP(){
        $idCurr = $this->input->post('txtCurrAjax');
        $selectKurs = $this->M_Fin_AP_zht->getKursByID($idCurr)->row();
        $data   = array(
    ...
    function getCurrOnDateInvoice(){
        ...
        $selectKurs = $this->M_Fin_AP_zht->getKursByID($noCurrency, $tglInvoice)->row();
        
        $expld      = explode('|', $noInvoice);
        $count      = count($expld);
        for($x = 0; $x <$count; $x++){
            $get[] = $this->M_Fin_AP_zht->getCurrOnDateInvoiceByCurrencyID($expld[$x], $noCurrency);
        }
    ...
    function getCurrBank(){
        ...
        $selectKurs = $this->M_Fin_AP_zht->getKursByID($idCurr,$txDate)->row();
        $data       = array(
            'rate_usd'  => number_format($selectKurs->rate_usd,6),
            'rate_sgd'  => number_format($selectKurs->rate_kurs,6)
            );
    ...
    function insertAPpayment(){
        ...
        $suppCOA    = $this->M_Fin_AP_zht->getSupplierCOA($suppID);
        ...
        $headerID   = $this->M_Fin_AP_zht->insertHeaderAPpayment($dataHdr);
        // echo "<pre>";
        // print_r($headerID);
        // echo "</pre>";
        // die;
        ...
        for($x = 0; $x < count($txtNoCOA); $x++):
            ...
            $this->M_Fin_AP_zht->insertDetailAPjurnal($detail);
        endfor;
        ...
        for($i = 0; $i < count($txtNoCOA); $i++):
            ...
            $this->M_Fin_AP_zht->insertCBhistory($history);
        endfor;
        ...
        $period = $this->M_CashBank->getPeriod();
        $getKursSGD = $this->M_Fin_AP_zht->getKursByID($this->input->post('txtCurrBayar'),$this->input->post('txtTransDate'))->row();
        ...
        for($y = 0; $y < count($txtNoCOA); $y++):
            ...
            if ($jml <> 0 OR $jml2 <> 0) {
                $this->M_Fin_AP_zht->insertToJurnalAcc($dataJurnal);
            }
        ...
        //========================= END =======================================
        redirect('APtrans_zht/reviewAPpayment/'.encode_str($headerID));
    }
    ...
    function InsertExcRtJurnal($headerID, $noReffAP, $nocoa){
        ...
        for($i = 0; $i < count($noinv); $i++){
            ...
            $total += $totalAsal[$i];
            $this->M_Fin_AP_zht->insertdtlexcRt($detailexc);
        }
    }
    ...
    function insertAPpaymentToDraf(){
        ...
        for($x =0 ; $x < $dtlRow; $x++):
            ...
            $this->M_Fin_AP_zht->insertDetailAPpayment($dataDtl);
        endfor;
    ...
    function updateHutangPerInvoiceNew(){
        ...
        for($x = 0; $x < $rowsPPI; $x++):
            $getBayar       = $this->M_Fin_AP_zht->getBayarFromUtang($idIncPerInvoice[$x],$jenis[$x]);
            $getSaldoHutang = $this->M_Fin_AP_zht->getHutangFromUtang($idIncPerInvoice[$x],$jenis[$x]);
            $data   = array(
                'bayar'         => $getBayar + $paymentPerInvoice[$x],
                'saldo_hutang'  => $getSaldoHutang - $paymentPerInvoice[$x]
            );
            $this->M_Fin_AP_zht->updateUtangByNoInvoiceInOneAPnumber($idIncPerInvoice[$x],$jenis[$x],$data); // Update table Hutang (ACC)
        endfor;
    }
    function updateHutangBulananPerInvoiceNew(){
        ...
        $periode    = $this->M_Fin_AP_zht->getPeriod_map();
        $thnPeriod  = date('Y', strtotime($periode));
        $blnPeriod  = date('m', strtotime($periode));
        
        for($x = 0; $x < $rowsPPI; $x++):
            $this->M_Fin_AP_zht->updateUtangBulananByNoInvoiceInOneAPnumberQuery($idIncPerInvoice[$x],$jenis[$x],$blnPeriod,$thnPeriod,$paymentPerInvoice[$x]); // Update table Hutang (ACC)
        endfor;
    }
    ...
    function updateDepositAPinHeader(){
        ...
        for($x = 0; $x < $rowsPPI; $x++):
            $this->M_Fin_AP_zht->updateDepositAPinHeaderByInv($idIncPerInvoice[$x], abs(str_replace(',', '',$paymentPerInvoice[$x]))); // Update Deposit
        endfor;
    }
    ...
    function insertDetailAPaccounting($noReffAP){
        ...
        for($x = 0; $x < $rowsPPI; $x++){
            ...
            $this->M_Fin_AP_zht->insertDetailAPaccountingFromPayment($data);
        }
    }
    // ################################################################################
    // ################################################################################
    ...
    function selectAPpayment(){
        $data   = array(
            '_selectHeader' => $this->M_Fin_AP_zht->selectAPforFindAP()->result()
        );
        $this->load->view('finance/transaction/ap_trans_zht/findAP/selectAP',$data);
    }
    function selectSuppliertForAPbySuppCode(){
        $idSupp = $this->input->post('txtCodeSupp');
        $get    = $this->M_Fin_AP_zht->selectSupplierForAPbyCode($idSupp);
        $data   = array(
    ...
    function reviewAPpayment($headerID){
        ...
        $get    = $this->M_Fin_AP_zht->selectHeaderAPforReview($headID);
        ...
        $data   = array(
            ...
            '_selectHeaderAP'   => $this->M_Fin_AP_zht->selectHeaderAPforReview($headID),
            '_selectInvoiceAP'  => $this->M_Fin_AP_zht->selectInvoiceAPforReview($noReff),
            '_selectDtlJrnal'   => $this->M_Fin_AP_zht->selectDetailJurnalAPforReview($headID)->result()
        );
        // echo "<pre>";
        // print_r($data);
        // echo"</pre>";
        // die;
        $this->template->display('finance/transaction/ap_trans_zht/findAP/index-review',$data);
    }
    // ============== ALL About Draft ==================
    function reviewAPforDraft($headerID){
        ...
        $this->template->display('finance/transaction/ap_trans_zht/saveDraft/index-draft',$data);
    }
    ...
    function payAPfromDraft(){
        ...
        $this->M_Fin_AP_zht->updateHeaderAPpayment($dataHdr,$headerID);

        // ############ Delete dulu detail Draft ======================================================
        // Delete AP Detail In Accounting ######
        $this->M_Fin_AP_zht->deleteDetailAPaccountingFromPayment($noReff);
        // ############ Delete dulu detail Draft ======================================================
    ...
    function deleteAPpayment(){
        ...
        $get        = $this->M_Fin_AP_zht->selectHeaderAPforReview($primary);
        $noReff     = $get->no_facture;
        $thnPeriod  = $get->thn_periode;
        $blnPeriod  = $get->bln_periode;
        $cur        = $get->currency_id;
        ...
        $cek_reff = $this->M_Fin_AP_zht->cek_reff('AP',$thnPeriod,$cur, $blnPeriod);
        ...
        // #### ========== ####
        $this->M_Fin_AP_zht->deleteAPfronCBhistory($noReff,$primary);
        $this->M_Fin_AP_zht->deleteDetailAPjurnal($noReff,$primary);
        ...
        if($noReff != $cek_reff->GEN){
            ...
            $this->M_Fin_AP_zht->updateHeaderAPpayment($primary,$data_hdr);
            ...
            // Delete Journal ######
            $this->M_Fin_AP_zht->updateAPfromJurnalAcc($noReff,$data_jur);
        } else {
            $this->M_Fin_AP_zht->deleteHeaderAPpayment($primary);
            $this->M_Fin_AP_zht->deleteAPfromJurnalAcc($noReff);
        }
        
        redirect(site_url('APtrans_zht'));
    }
    function returnBackHutangPerInvoiceByPass(){
        ...
        for($x = 0; $x < $rowsPPI; $x++):
            $this->M_Fin_AP_zht->resetPaymentHutang($idIncPerInvoice[$x],$jenis[$x]);
            $this->M_Fin_AP_zht->resetPaymentHutangSaldo($idIncPerInvoice[$x],$jenis[$x]);
        endfor;        
    }
    ...
    function returnBackHutangBulananPerInvoice($thnPeriod,$blnPeriod){
        ...
        for($x = 0; $x < $rowsPPI; $x++):
            $this->M_Fin_AP_zht->updateUtangBulananByNoInvoiceInOneAPnumberQueryReturn($idIncPerInvoice[$x],$jenis[$x],$blnPeriod,$thnPeriod,$paymentPerInvoice[$x]); // Update table Hutang (ACC)
        endfor;
    }

    // =========================== Print Report ===========================
    function paymentAdvice($noAP){
        $no_AP = decode_str($noAP);
        $get    = $this->M_Fin_AP_zht->selectHeaderAPforPrint($no_AP);
        ...
        $data   = array(
            ...
            '_selectInvoice'    => $this->M_Fin_AP_zht->selectInvoiceForPaymentAdvice($no_AP)
        );
