...
class Purchasing_lr extends CI_Controller
{
    function __construct()
    {
        parent::__construct();
        $this->load->model(array('m_purchasing', 'm_purchasing_lr', 'm_purchasing_so'));
    ...
    function index()
    {
        $this->template->display('purchasing/loading_report/loading_report_add');
    }

    public function sales_order_cust()
    {
        $data['cust'] =  $this->m_purchasing->tampil_cust_pur(trim($this->input->get('cust')));
        $this->load->view('purchasing/loading_report/loading_report_filter_cust', $data);
    }

    public function loading_report_po_pl()
    {
        $data['po'] =  $this->m_purchasing_lr->tampil_pl_pur_po($this->input->get('cust'), $this->input->get('po'));
        $this->load->view('purchasing/loading_report/loading_report_filter_pl_po', $data);
    }
    ...
    public function save_lr($trans)
    {
        ...
        $query =  $this->m_purchasing_lr->simpan_lr_sp($datahdr);

        if ($query['flag'] == 1) {
            $this->session->set_flashdata("message", "<div class=\"alert alert-success\" id=\"alert\">Transaction Success</div>");
            redirect('purchasing_lr/loading_report_show?lr=' . $query['lrno']);
        } else {
            $this->session->set_flashdata("message", "<div class=\"alert alert-danger\" id=\"alert\">Save Transaction Broken.</div>");
            redirect('purchasing_lr');
        }
    }

    public function loading_report_inv()
    {
        $data['lr'] =  $this->m_purchasing_lr->tampil_lr_all($this->input->get('inv'));
        $this->load->view('purchasing/loading_report/loading_report_all', $data);
    }

    public function loading_report_show()
    {
        $data['lr'] =  $this->m_purchasing_lr->tampil_lr_where($this->input->get('lr'));
        $this->template->display('purchasing/loading_report/loading_report_edit', $data);
    }


    public function print_report_lr()
    {
        $lr = $this->m_purchasing_lr->tampil_lr_where($this->input->get('lr'));
        $data['lr'] = $lr;
        $this->load->view('purchasing/loading_report/loading_report_print', $data);
    }

    public function remove_loading_report()
    {
        $data['lr'] =  $this->m_purchasing_lr->tampil_lr_where($this->input->get('delete'));
        $this->load->view('purchasing/loading_report/loading_report_filter_modal_delete', $data);
    }

    public function loading_report_delete()
    {
        $this->m_purchasing_lr->delete_loading_report($this->input->get('lr'), 0);
        redirect('loading_report/loading_report_add');
    }

    public function loading_report_delete_item_dp()
    {
        $lr = $this->input->get('lrno');
        $this->m_purchasing_lr->loading_report_delete_item_dp($this->input->get('itemid'), 0);
        redirect('loading_report/loading_report_show?lr=' . $lr);
    }
    ...
    {
        $so =  $this->m_purchasing_lr->tampil_pl_where($this->input->get('pl'));
        $data['so'] = $so;
        foreach ($so as $r) {
            $country_id = $r->country_id;
        }
    ...
    public function simpan_pl($trans)
    {
        ...
        $datahdr = array('pl_no' => $pl_no, 'packing' => $packing, 'totalpack' => $totalpack, 'shipdate_pl' => $shipdate_pl, 'via' => $via, 'createdby' => strtoupper($this->session->userdata('userid')), 'createddate' => date('Y-m-d H:i:s'));

        $query =  $this->m_purchasing_lr->simpan_pl_sp($datahdr);
    ...
    function edit()
    {
        $data['packdo']   = $this->m_purchasing_lr->tampil_packdo_where($this->input->get('noreff'));
        $data['_factory'] = $this->m_purchasing_lr->get_factory();
    ...
    function remove_packdo()
    {
        ...
        $query   = $this->m_purchasing_lr->delete_detail_do($id);
    ...
    {
        $packdo = $this->m_purchasing_lr->tampil_pl_where($this->input->get('pl'));
        $data['packdo'] = $packdo;
    ...
    {
        $data['_datagr'] = $this->m_purchasing_lr->get_gr($this->input->get('cust'), $this->input->get('po'));
    ...
    function tampil_gr_do()
    {
        ...
        $data['_datagr'] = $this->m_purchasing_lr->get_gr_do($factory);
        $this->load->view('purchasing/packing_do/data_gr_ajax_do', $data);
    }

    public function filter_do()
    {
        $data['packdo'] =  $this->m_purchasing_lr->tampil_pack_list($this->input->get('po'));
        $data['_factory'] = $this->m_purchasing_lr->get_factory();
    ...
    {
        $data['so'] =  $this->m_purchasing_lr->tampil_pl_all($this->input->get('inv'));
    ...
    function mon_packdo_filter()
    {
        ...
        $data['_list'] = $this->m_purchasing_lr->tampil_packdo_filter($p_dari, $p_sampai, $mainpo, $item, $npbb, $status);
    ...
    public function pack_do_excel()
    {
        ...
        $data['record'] = $this->m_purchasing_lr->tampil_packdo_filter($p_dari, $p_sampai, $mainpo, $item, $npbb, $status);
        $this->load->view('purchasing/packing_do/toexcel_warehouse', $data);
    }

    public function packing_list_delete()
    {
        $this->m_purchasing_lr->delete_packing_list($this->input->get('pl'));
    ...
    {
        $data['pl'] =  $this->m_purchasing_lr->tampil_pl_where($this->input->get('delete'));
    ...
    {
        $this->template->display('purchasing/packing_do/add_delivery_order');
    }

    public function delivery_oder_show()
    {
        $data['gr'] =  $this->m_purchasing_lr->tampil_do_where($this->input->get('do'));
        $this->template->display('purchasing/packing_do/edit_delivery_order', $data);
    }

    public function delivery_oder_gr()
    {
        $data['po'] =  $this->m_purchasing_lr->tampil_gr_where_other($this->input->get('po'));
        $this->load->view('purchasing/packing_do/delivery_oder_filter_gr', $data);
    }

    public function delivery_oder_do()
    {
        $data['do'] =  $this->m_purchasing_lr->tampil_do(trim($this->input->get('do')));
        $this->load->view('purchasing/packing_do/delivery_oder_do_all', $data);
    }
    ...
    public function delivery_oder_save($trans)
    {
        ...
        $query = $this->m_purchasing_lr->simpan_do_sp($datahdr);
    ...
    {
        $data['do'] =  $this->m_purchasing_lr->tampil_do_where($this->input->get('delete'));
    ...
    {
        $docno = $this->input->post('docno');
        $query = $this->m_purchasing_lr->delete_delivery_order($docno);
    ...
    {
        $data['book'] =  $this->m_purchasing_lr->tampil_bookref_where($this->input->get('bookref_no'));
        $this->template->display('purchasing/packing_do/book_order_show', $data);
    }

    public function book_order_all()
    {
        $data['book'] =  $this->m_purchasing_lr->tampil_bookref_all($this->input->get('inv'));
        $this->load->view('purchasing/packing_do/book_order_all', $data);
    }

    public function add_container_po()
    {
        $data['po'] =  $this->m_purchasing_lr->tampil_container_po($this->input->get('po'));
        $this->load->view('purchasing/packing_do/add_container_filter_po', $data);
    }

    public function add_container_get()
    {
        $data['cont'] =  $this->m_purchasing_lr->tampil_cont_shp($this->input->get('cust'), $this->input->get('ship'), $this->input->get('cont'), $this->input->get('contno'));
        $this->load->view('purchasing/packing_do/get_add_container', $data);
    }

    public function add_container_po_dtl()
    {
        $data['po'] =  $this->m_purchasing_lr->tampil_container_po($this->input->get('cust'), $this->input->get('po'));
    ...
    public function save_book_ref($trans)
    {
        ...
        $datahdr = array('trans' => $trans, 'bookref_no' => $bookref_no, 'barge' => $barge, 'voyage' => $voyage, 'etd' => $etd, 'date' => $date, 'cust' => $cust, 'ammend' => $ammend, 'createdby' => strtoupper($this->session->userdata('userid')));

        $query =  $this->m_purchasing_lr->simpan_book_ref_sp($datahdr);
    ...
    {
        $data['book'] =  $this->m_purchasing_lr->tampil_bookref_where($this->input->get('bookref_no'));
        $data['cont'] =  $this->m_purchasing_lr->get_bookref_shp2($this->input->get('cust'), $this->input->get('ship'));
    ...
    public function save_book_ref_ship()
    {
        ...
        $datahdr = array('bookref_no' => $bookref_no, 'createdby' => strtoupper($this->session->userdata('userid')));

        $query =  $this->m_purchasing_lr->simpan_book_ref_shp_sp($datahdr);
    ...
    {
        $data['po'] =  $this->m_purchasing_lr->tampil_po_hdr($this->input->get('remark'));
    ...
    public function book_order_shipping_excel()
    {
        ...
        $data['book'] =  $this->m_purchasing_lr->get_shp_excel($this->input->get('bookref_no'));
        $data['whs'] =  $this->m_purchasing_lr->get_shp_whs($this->input->get('bookref_no'));
