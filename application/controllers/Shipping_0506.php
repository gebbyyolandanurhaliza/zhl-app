...
//    ------------------------------------------------------------------ABOUT Container Outward--------------------------------------------------------
    ...
    public function container_inward_edit(){
        ...
        // $data['container'] = $this->m_shipping->getcontainerinward($contid);
        // $data['container'] = $this->m_shipping->tampil_cont_where($contid);
        // $this->load->view('shipping/container_inward_edit',$data);   
...
//-------------------------------Modul Container Stock-------------------------------//
    public function container_stock(){
        $data['container_number']=$this->m_shipping->tampil_factory_stock_container();
...
public function container_stock_save (){
        ...
        if ($stock_id == ''){
            $data=array('container_number'=>$container_number,'container_id'=>$container_id, 'loading_port'=>$loading_port,'free_time'=>$free_time,'arrival_date'=>$p_tanggal, 'Remark'=>$Remark, 'factory'=>$factory,'supplier'=>$supplier,'import_bl_no'=>$import_bl_no, 'eta'=>$q_tanggal, 'free_time_expiry'=>$r_tanggal);
            $this->m_shipping->simpan_container_stock($data);
            $message='Save Data Success';
        } else{
            $data=array('container_number'=>$container_number,'container_id'=>$container_id, 'loading_port'=>$loading_port,'free_time'=>$free_time,'arrival_date'=>$p_tanggal, 'Remark'=>$Remark, 'factory'=>$factory,'supplier'=>$supplier,'import_bl_no'=>$import_bl_no, 'eta'=>$q_tanggal, 'free_time_expiry'=>$r_tanggal);
            $this->m_shipping->update_container_stock($data,$stock_id);
            $message='Update Data Success';
        }
