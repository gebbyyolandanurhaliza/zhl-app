...
class Supplier_card extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model(array('M_Supplier_card', 'M_Fin_AP', 'M_Fin_Dp'));
    ...
    public function index() {
        ...
        $data['SupplierID'] = $this->M_Supplier_card->get_supplier();
        $data['CurrencyID'] = $this->M_Supplier_card->get_currency();

        $this->template->display('accounting/supplier_card', $data);
    }
    ...
    function search() {
        ...
        $data['SupplierID'] = $this->M_Supplier_card->get_supplier();
        $data['CurrencyID'] = $this->M_Supplier_card->get_currency();
        $data['Get_aging']  = $this->M_Supplier_card->call_data($sup, $cur,$from,$to, $coa);
    ...
    function reviewAPpayment(){
        ...
        $get    = $this->M_Fin_AP->selectHeaderAPforReviewByNoReff($headID);
        ...
        $data   = array(
            ...
            '_selectHeaderAP'   => $this->M_Fin_AP->selectHeaderAPforReview($noReff),
            '_selectInvoiceAP'  => $this->M_Fin_AP->selectInvoiceAPforReview($headID),
            '_selectDetailAP'   => $this->M_Fin_AP->selectDetailAPforReview($noReff)->result(),
            '_selectDtlJrnal'   => $this->M_Fin_AP->selectDetailJurnalAPforReview($noReff)->result()
        );
    ...
    function print_report() {
        ...
        $data['from'] = $from;
        $data['to'] = $to;
        $su = $this->M_Supplier_card->get_supp($sup);
        $data['supplier'] = $su->suppliercompany;
        $data['get_data']  = $this->M_Supplier_card->call_data($sup, $cur,$from,$to, $coa);

        $this->load->view('accounting/rpt/rpt_supplier_card', $data);
    }
