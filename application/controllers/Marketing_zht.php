...
class Marketing_zht extends CI_Controller
{
	...
	function __construct()
	{
		...
		$this->load->model(array('M_zht_gen_master', 'M_zht_mar_master', 'M_mar_product', 'M_mar_sales_quotation'));
	...
	function master($nama_master)
	{
		switch ($nama_master) {
			...
			case 'customer':
				...
				$master = $this->M_zht_mar_master->cust_get_all();
				...
				$this->template->display('marketing_zht/master/customer/list', $data);

				break;

			case 'group_customer':
				$master = $this->M_zht_gen_master->get_all('group_customer');
				...
				$this->template->display('marketing_zht/master/customer_group/list', $data);
				break;
	...
	public function delete_master($nama_master, $id)
	{
		switch ($nama_master) {
			...
			case 'customer':
				$row = $this->M_zht_mar_master->cust_get_by_id($id);

				if ($row) {
					$this->M_zht_mar_master->cust_delete($id);
					$this->M_zht_mar_master->cust_payterm_delete($id);
					$this->session->set_flashdata('message', pesan('Delete Record Success', pesan_sukses()));
					redirect(site_url('marketing_zht/master/customer'));
				} else {
			...
			case 'group_customer':
				$row = $this->M_zht_gen_master->get_all('group_customer');

				if ($row) {
					$this->M_zht_gen_master->delete('group_customer', $id);
					$this->session->set_flashdata('message', pesan('Delete Record Success', pesan_sukses()));
					redirect(site_url('marketing_zht/master/group_customer'));
				} else {
					$this->session->set_flashdata('message', pesan('Record Not Found', pesan_peringatan()));
					redirect(site_url('marketing_zht/master/group_customer'));
				}
	...
	public function find_master($nama_master)
	{
		switch ($nama_master) {
			case 'customer':
				...
				//} else {
				$find_record = $this->M_zht_mar_master->cust_by_search();
				$cbo_country = $this->M_zht_gen_master->get_all('country');

				if ($find_record) {

					$data = array(
						'button'				=> '<i class="fa fa-search"></i> Search ...',
						'action'				=> site_url('marketing_zht/find_master/customer'),
					...
					$data['message'] = $this->session->flashdata('message');
					//	$this->load->view('marketing_zht/master/customer/find', $data);
					$this->template->display('marketing_zht/master/customer/find', $data);
				} else {
	...
	{
		$cbo_country	= $this->M_zht_gen_master->get_all('country');
		$cbo_group		= $this->M_zht_gen_master->get_all('group_customer');
		$list_document	= $this->M_zht_mar_master->document_get_all();

		//update 2016-07
		$cbo_payterm	= $this->M_zht_mar_master->paymentterm_get_all();
		//update 2018-01-24
		$cbo_bank		= $this->M_zht_mar_master->bank_get_all();

		$data = array(
			'button'				=> '<i class="fa fa-save"></i> Save',
			'action'				=> site_url('marketing_zht/save_master_customer'),
			'header_title'			=> 'Master ZHT Customer - Create New',
			'cbo_country'			=> $cbo_country,
			'cbo_group'				=> $cbo_group,
		...
		$this->template->display('marketing_zht/master/customer/create_zht', $data);
	}
	...
	function save_master_customer()
	{
		...
		if ($this->M_zht_mar_master->cust_exists($param) == false) {
			$customer_id	= $this->M_zht_mar_master->cust_insert($data);
			$customer_code	= $this->M_zht_mar_master->cust_update_code($customer_id, $company_name);
			if ($customer_id != 0) {
				$this->M_zht_mar_master->cust_payterm_insert($customer_id);
				$this->M_zht_mar_master->cust_bank_insert($customer_id);
				$this->M_zht_mar_master->cust_document_insert($customer_id);
				$this->M_zht_mar_master->cust_product_insert($customer_id);
			}
	...
	function do_edit_customer($id)
	{
		...
		$cbo_country	= $this->M_zht_gen_master->get_all('country');
		$cbo_group		= $this->M_zht_gen_master->get_all('group_customer');
		$detail_payterm = $this->M_zht_mar_master->payterm_get_detail($id);
		$detail_bank	= $this->M_zht_mar_master->bank_get_detail($id);
		$list_document	= $this->M_zht_mar_master->document_get_all();
		$selected_document = $this->M_zht_mar_master->cust_document_get_all($id);
		...
		if ($row) {
			$data = array(
				'button'				=> 'Update',
				'action'				=> site_url('marketing_zht/update_customer'),
	...
	function update_customer()
	{
		...
		$this->M_zht_mar_master->cust_payterm_delete($customer_id);
		$this->M_zht_mar_master->cust_bank_delete($customer_id);
		$this->M_zht_mar_master->cust_document_delete($customer_id);
		$this->M_zht_mar_master->cust_product_delete($customer_id);

		$this->M_zht_mar_master->cust_update($customer_id, $data);
	...
	function get_product_purchase()
	{
		if ('IS_AJAX') {
			...
			$this->load->view('marketing_zht/master/customer/product_purchase', $data);
		}
	}
	...
	private function do_create_customer_group ()
	{
		$data = array(
			'button'				=> '<i class="fa fa-save"></i> Save',
			'action'				=> site_url('marketing_zht/save_master_customer_group'),
		...
		$this->template->display('marketing_zht/master/customer_group/create', $data);
	}
	...
	function save_master_customer_group()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_zht_gen_master->insert('group_customer', $data);
	...
	{
		$row			= $this->M_zht_gen_master->get_by_id('group_customer', $id);

		if ($row) {
			$data = array(
				'button'				=> 'Update',
				'action'				=> site_url('marketing_zht/update_customer_group'),
				'header_title'			=> 'Master Customer - Edit',
				'customer_group_id'		=> set_value('customer_group_id', $row->customer_group_id),
	...
	function update_customer_group()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_zht_gen_master->update('group_customer', $customer_group_id, $data);
	...
	{
		$cbo_country	= $this->M_zht_gen_master->get_all('country');
		$cbo_customer	= $this->M_zht_mar_master->cust_get_all_active();

		$data = array(
			'button'				=> '<i class="fa fa-save"></i> Save',
			'action'				=> site_url('marketing_zht/save_master_agent'),
		...
		$this->template->display('marketing_zht/master/agent/create', $data);
	}
	...
	function save_master_agent()
	{
		...
		$this->M_zht_mar_master->agent_insert($data);
		$this->session->set_flashdata('message', pesan('Create Record Success', pesan_sukses()));
		redirect(site_url('marketing_zht/master/agent'));
		//        }
	}
	...
	function do_find_agent()
	{
		...
		$data['message'] = $this->session->flashdata('message');
		$this->template->display('marketing_zht/master/agent/list', $data);
	}

	function do_edit_agent($id)
	{
		$row			= $this->M_zht_mar_master->agent_get_by_id($id);
		$cbo_country	= $this->M_zht_gen_master->get_all('country');
		$cbo_customer	= $this->M_zht_mar_master->cust_get_all();

		if ($row) {
			$data = array(
				'button'				=> '<i class="fa fa-save"></i> Update',
				'action'				=> site_url('marketing_zht/update_agent'),
	...
	function update_agent()
	{
		...
		$this->M_zht_mar_master->agent_update($agent_id, $data);
	...
	private function do_create_factory()
	{
		$data = array(
			'button'		=> '<i class="fa fa-save"></i> Save',
			'action'		=> site_url('marketing_zht/save_master_factory'),
		...
		$this->template->display('marketing_zht/master/factory/create', $data);
	}
	...
	function save_master_factory()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_zht_gen_master->insert('factory', $data);
			$this->session->set_flashdata('message', pesan('Create Record Success', pesan_sukses()));
			redirect(site_url('marketing_zht/master/factory'));
		}
	}

	function do_edit_factory($id)
	{
		$row = $this->M_zht_gen_master->get_by_id('factory', $id);

		if ($row) {
			$data = array(
				'button'			=> 'Update',
				'action'			=> site_url('marketing_zht/update_factory'),
	...
	function update_factory()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_zht_gen_master->update('factory', $this->input->post('factory_id', TRUE), $data);
			$this->session->set_flashdata('message', pesan('Update Record Success', pesan_sukses()));
			redirect(site_url('marketing_zht/master/factory'));
		}
	}
	...
	{
		$cbo_factory			= $this->M_zht_gen_master->get_all('factory');
		$cbo_brand				= $this->M_zht_mar_master->brand_get_all();
		$cbo_uom_volume			= $this->M_zht_mar_master->uom_volume_get_all();
		$cbo_uom_quantity		= $this->M_zht_mar_master->uom_quantity_get_all();
		$cbo_packing_size		= $this->M_zht_mar_master->packing_size_get_all();
		$cbo_product_category = $this->M_mar_product->product_category_get_all();

		$data = array(
			'button'			=> '<i class="fa fa-save"></i> Save',
			'action'			=> site_url('marketing_zht/save_master_product'),
		...
		$this->template->display('marketing_zht/master/product/create', $data);
	}
	...
	function save_master_product()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			redirect(site_url('marketing_zht/master/product'));
		}
	}
	...
	function do_edit_product($id)
	{
		...
		if (isset($row)) {
			...
			$data = array(
				'button'			=> 'Update',
				'action'			=> site_url('marketing_zht/update_product'),
	...
	function update_packing_list()
	{
		...
		redirect(site_url('marketing_zht/master/product_packing_list'));
	}
	...
	{
		$cbo_main_category	= $this->M_zht_mar_master->maincate_get_all();
		...
		$data = array(
			'button'				=> '<i class="fa fa-save"></i> Save',
			'action'				=> site_url('marketing_zht/save_master_product_category'),
		...
		$this->template->display('marketing_zht/master/product_category/create', $data);
	}
	...
	function save_master_product_category()
	{
		...
		$this->session->set_flashdata('message', pesan('Create Record Success', pesan_sukses()));
		redirect(site_url('marketing_zht/master/product_category'));
		//        }
	}
	...
	function do_edit_product_category($id)
	{
		...
		if ($row) {
			$data = array(
				'button'				=> 'Update',
				'action'				=> site_url('marketing_zht/update_product_category'),
	...
	private function do_create_brand()
	{
		$data = array(
			'button'		=> '<i class="fa fa-save"></i> Save',
			'action'		=> site_url('marketing_zht/save_master_brand'),
		...
		$this->template->display('marketing_zht/master/brand/create', $data);
	}
	...
	function save_master_brand()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_zht_mar_master->brand_insert($data);
			$this->session->set_flashdata('message', pesan('Create Record Success', pesan_sukses()));
			redirect(site_url('marketing_zht/master/brand'));
		}
	}

	function do_edit_brand($id)
	{
		$row = $this->M_zht_mar_master->brand_get_by_id($id);

		if ($row) {
			$data = array(
				'button'		=> 'Update',
				'action'		=> site_url('marketing_zht/update_brand'),
	...
	function update_brand()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_zht_mar_master->brand_update($this->input->post('brand_id', TRUE), $data);
			$this->session->set_flashdata('message', pesan('Update Record Success', pesan_sukses()));
			redirect(site_url('marketing_zht/master/brand'));
		}
	}
	...
	private function do_create_document()
	{
		$data = array(
			'button'			=> '<i class="fa fa-save"></i> Save',
			'action'			=> site_url('marketing_zht/save_master_document'),
		...
		$this->template->display('marketing_zht/master/document/create', $data);
	}
	...
	function save_master_document()
	{
		...
		$id = $this->M_zht_mar_master->document_exists($param);

		if ($id > 0) {
			$this->M_zht_mar_master->document_delete($id);
		}
		...
		$this->M_zht_mar_master->document_insert($data);

		$this->session->set_flashdata('message', pesan('Create Record Success', pesan_sukses()));
		redirect(site_url('marketing_zht/master/document'));
		//        }
	}
	...
	function save_master_document_modal($special_doc = 0)
	{
		...
		$doc_id = $this->M_zht_mar_master->document_insert($data);
		...
		$this->load->view('marketing_zht/modal/document_row', $data_reload);
	...
	{
		$row = $this->M_zht_mar_master->document_get_by_id($id);

		if ($row) {
			$checked = ($row->special == 1) ? TRUE : FALSE;
			$data = array(
				'button'			=> 'Update',
				'action'			=> site_url('marketing_zht/update_document'),
	...
	function update_document()
	{
		...
		$this->M_zht_mar_master->document_update($this->input->post('document_id', TRUE), $data);
		$this->session->set_flashdata('message', pesan('Update Record Success', pesan_sukses()));
		redirect(site_url('marketing_zht/master/document'));
		//        }
	}
	...
	private function do_create_payment_term()
	{
		$data = array(
			'button'			=> '<i class="fa fa-save"></i> Save',
			'action'			=> site_url('marketing_zht/save_master_payment_term'),
		...
		$this->template->display('marketing_zht/master/payment_term/create', $data);
	}

	function do_edit_payment_term($id)
	{
		$row = $this->M_zht_mar_master->paymentterm_get_by_id($id);
		$position_id = $this->session->userdata('position_id');

		if ($row) {
			$data = array(
				'button'			=> 'Update',
				'action'			=> site_url('marketing_zht/update_payment_term'),
			...
			$this->session->set_flashdata('message', pesan('Record Not Found', pesan_error()));
			redirect(site_url('marketing_zht/master/payment_term'));
		}
	}
	...
	function save_master_payment_term()
	{
		...
		$id = $this->M_zht_mar_master->paymentterm_exists($param);
		...
		$this->M_zht_mar_master->paymentterm_insert($data);
	...
	function update_payment_term()
	{
		...
		$this->M_zht_mar_master->paymentterm_update($this->input->post('payment_term_id', TRUE), $data);
		$this->session->set_flashdata('message', pesan('Update Record Success', pesan_sukses()));
		redirect(site_url('marketing_zht/master/payment_term'));
	}
	...
	private function do_create_shelf_life()
	{
		$data = array(
			'button'			=> '<i class="fa fa-save"></i> Save',
			'action'			=> site_url('marketing_zht/save_master_shelf_life'),
			'header_title'		=> 'Master Shelf Life - Create New',
			'shelf_life_id'		=> set_value('shelf_life_id'),
			'shelf_life'		=> set_value('shelf_life'),
		);

		$this->template->display('marketing_zht/master/shelf_life/create', $data);
	}
	...
	function save_master_shelf_life()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_zht_mar_master->shelf_insert($data);
			$this->session->set_flashdata('message', pesan('Create Record Success', pesan_sukses()));
			redirect(site_url('marketing_zht/master/shelf_life'));
		}
	}
	...
	function save_shelf_life_modal()
	{
		...
		$shelf_id = $this->M_zht_mar_master->shelf_insert($data);

		$cbo_shelf			= $this->M_zht_mar_master->shelf_get_all();
		$data1 = array(
			'cbo_shelf'				=> $cbo_shelf,
			'product_shelf_life_id' => set_value('product_shelf_life_id', $shelf_id, true),
		);
		$this->load->view('marketing_zht/reload/reload_shelf_life', $data1);
	}

	function do_edit_shelf_life($id)
	{
		$row = $this->M_zht_mar_master->shelf_get_by_id($id);

		if ($row) {
			$data = array(
				'button'			=> 'Update',
				'action'			=> site_url('marketing_zht/update_shelf_life'),
	...
	function update_shelf_life()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_zht_mar_master->shelf_update($this->input->post('shelf_life_id', TRUE), $data);
			$this->session->set_flashdata('message', pesan('Update Record Success', pesan_sukses()));
			redirect(site_url('marketing_zht/master/shelf_life'));
		}
	}
	...
	private function do_create_packing_size()
	{
		$data = array(
			'button'			=> '<i class="fa fa-save"></i> Save',
			'action'			=> site_url('marketing_zht/save_master_packing_size'),
		...
		$this->template->display('marketing_zht/master/packing_size/create', $data);
	}
	...
	function save_master_packing_size()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_zht_mar_master->packing_size_insert($data);
			$this->session->set_flashdata('message', pesan('Create Record Success', pesan_sukses()));
			redirect(site_url('marketing_zht/master/packing-size'));
		}
	}

	function do_edit_packing_size($id)
	{
		$row = $this->M_zht_mar_master->packing_size_get_by_id($id);

		if ($row) {
			$data = array(
				'button'			=> 'Update',
				'action'			=> site_url('marketing_zht/update_packing_size'),
	...
	function update_packing_size()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_zht_mar_master->packing_size_update($this->input->post('packing_size_id', TRUE), $data);
			$this->session->set_flashdata('message', pesan('Update Record Success', pesan_sukses()));
			redirect(site_url('marketing_zht/master/packing-size'));
		}
	}
	...
	private function do_create_packing_type()
	{
		$data = array(
			'button'			=> '<i class="fa fa-save"></i> Save',
			'action'			=> site_url('marketing_zht/save_master_packing_type'),
		...
		$this->template->display('marketing_zht/master/packing_type/create', $data);
	}
	...
	function save_master_packing_type()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_zht_mar_master->packing_type_insert($data);
			$this->session->set_flashdata('message', pesan('Create Record Success', pesan_sukses()));
			redirect(site_url('marketing_zht/master/packing-type'));
		}
	}

	function do_edit_packing_type($id)
	{
		$row = $this->M_zht_mar_master->packing_type_get_by_id($id);

		if ($row) {
			$data = array(
				'button'			=> 'Update',
				'action'			=> site_url('marketing_zht/update_packing_type'),
	...
	function update_packing_type()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_zht_mar_master->packing_type_update($this->input->post('packing_type_id', TRUE), $data);
			$this->session->set_flashdata('message', pesan('Update Record Success', pesan_sukses()));
			redirect(site_url('marketing_zht/master/packing-type'));
		}
	}
...
//==================================================================================================
	...
	private function do_create_bank()
	{
		...
		$data = array(
			'button'			=> '<i class="fa fa-save"></i> Save',
			'action'			=> site_url('marketing_zht/save_master_bank'),
		...
		$this->template->display('marketing_zht/master/bank/create', $data);
	}
	...
	function save_master_bank()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_zht_mar_master->bank_insert($data);
			$this->session->set_flashdata('message', pesan('Create Record Success', pesan_sukses()));
			redirect(site_url('marketing_zht/master/bank'));
		}
	}

	function do_edit_bank($id)
	{
		$row = $this->M_zht_mar_master->bank_get_by_id($id);
		$cbo_country = $this->M_gen_master->get_all('country');
		$cbo_currency	= $this->M_gen_master->get_all('currency');
		...
		if ($row) {
			$data = array(
				'button'			=> 'Update',
				'action'			=> site_url('marketing_zht/update_bank'),
	...
	function update_bank()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_zht_mar_master->bank_update($this->input->post('bank_id', TRUE), $data);
			$this->session->set_flashdata('message', pesan('Update Record Success', pesan_sukses()));
			redirect(site_url('marketing_zht/master/bank'));
		}
	}
	...
	function save_master_trading_term()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_zht_mar_master->tradingterm_insert($data);
			$this->session->set_flashdata('message', pesan('Create Record Success', pesan_sukses()));
			redirect(site_url('marketing_zht/master/trading_term'));
		}
	}
	...
	function save_trading_term_modal()
	{
		...
		$trading_term_id = $this->M_zht_mar_master->tradingterm_insert($data);

		$cbo_tradingterm	= $this->M_zht_mar_master->tradingterm_get_all();
		$data1 = array(
			'cbo_tradingterm'	=> $cbo_tradingterm,
			'trading_term_id'	=> $trading_term_id,
		);
		$this->load->view('marketing_zht/reload/reload_trading_term', $data1);
	}

	function do_edit_trading_term($id)
	{
		$row = $this->M_zht_mar_master->tradingterm_get_by_id(decode_str($id));
		$master = $this->M_zht_mar_master->tradingterm_get_all();

		if ($row) {
			$data = array(
				'master_data'		  => $master,
				'button'			  => 'Update',
				'action'			  => site_url('marketing_zht/update_trading_term'),
			...
			$data['message'] = $this->session->flashdata('message');
			$this->template->display('marketing_zht/master/trading_term/index', $data);
		} else {
			$this->session->set_flashdata('message', pesan('Record Not Found', pesan_error()));
			redirect(site_url('marketing_zht/master/trading_term'));
		}
	...
	function update_trading_term()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->M_zht_mar_master->tradingterm_update(decode_str($id), $data);
			$this->session->set_flashdata('message', pesan('Update Record Success', pesan_sukses()));
			redirect(site_url('marketing_zht/master/trading_term'));
		}
	}
...
//==================================================================================================
	...
	function save_payterm_modal()
	{
		...
		$this->M_zht_mar_master->cust_payterm_insert_modal($customer_id, $payment_term);

		if ($customer_id) {
			$cbo_payterm	= $this->M_zht_mar_master->payterm_get_detail($customer_id);
		} else {
			$cbo_payterm	= $this->M_zht_mar_master->payterm_get_all();
		}
		...
		$this->load->view('marketing_zht/reload/reload_payment_term', $data);
	}
	...
	function add_bank_row()
	{
		...
		$this->load->view('marketing_zht/master/customer/bank_row_output', $data);
	}
	...
	function add_payterm_row()
	{
		...
		$this->load->view('marketing_zht/master/customer/payterm_row_output', $data);
	}
