...
class ARtrans_zht extends CI_Controller{
    public function __construct() {
        ...
        $this->load->model(array('M_CashBank','M_Fin_AR_zht','M_Fin_CB','M_General_Journal','M_vcdn'));
    }
    
    function index(){
        // $company   = strtoupper($this->session->userdata('company_id'));
        $data['_periode']           = $this->M_Fin_AR_zht->getPeriod_mar();
        ...
        $data['List_coa']           = $this->M_vcdn->get_coa_zht();
        ...
        $this->template->display('finance/transaction/ar_trans_zht/index',$data);
    }
    ...
    function newGenerateReffNumber2(){
        ...
        $num    = $this->M_Fin_AR_zht->newCheckReffNumber2('OUT', $tanggal, $currency);
        $get    = str_pad($num, 4, 0, STR_PAD_LEFT);
        $set    = 'ZHTR'.'/'.date('Y',strtotime($this->input->get('txtDateForGen'))).'/'.date('m',strtotime($this->input->get('txtDateForGen'))).'/'.$get;
        echo $set;
    ...
    function selectCustomerForAR(){
        ...
        $data   = array(
            '_selectSupplier' => $this->M_Fin_AR_zht->selectCustomerforARtrans($tgl)->result()
        );
        $this->load->view('finance/transaction/ar_trans_zht/selectSupplier',$data);
    }
    ...
    function selectInvoiceForAR(){
        ...
        $data   = array(
            '_selectInvoice' => $this->M_Fin_AR_zht->selectInvoiceWhereForARtrans($idSupp, $idDate)->result()
        );
        
        $this->load->view('finance/transaction/ar_trans_zht/selectInvoice',$data);
    }

    function selectModalAR(){
        $data   = array(
            '_selectAP' => $this->M_Fin_AR_zht->selectARfromAccounting()->result()
        );
        $this->load->view('finance/transaction/ar_trans_zht/selectAR',$data);
    }
    function selectCOA(){
        $data   = array(
            '_getMasterCOA' => $this->M_Fin_AR_zht->selectCOAforARtrans_zht()->result()
        );
        $this->load->view('finance/transaction/ar_trans_zht/selectMCOA',$data);
    }
    function selectCurrencyAR(){
        $idCurr = $this->input->post('txtCurrAjax');
        $selectKurs = $this->M_Fin_AR_zht->getKursByIDonAR($idCurr)->row();
        $data   = array(
    ...
    function getCurrOnDateInvoice(){
        ...
        $selectKurs = $this->M_Fin_AR_zht->getKursByIDonAR($noCurrency,$tglInvoice)->row();
        
        $expld      = explode('|', $noInvoice);
        $count      = count($expld);
        for($x = 0; $x <$count; $x++){
            $get[] = $this->M_Fin_AR_zht->getCurrOnDateInvoiceByCurrencyIDonAR($expld[$x], $noCurrency);
        }
    ...
    function getCurrBank(){
        ...
        $selectKurs = $this->M_Fin_AR_zht->getKursByID($idCurr,$txDate)->row();
    ...
    function insertARpayment(){
        ...
        $suppCOA    = $this->M_Fin_AR_zht->getCustomerCOA($suppID);
        ...
        $headerID   = $this->M_Fin_AR_zht->insertHeaderAPpayment($dataHdr);
        ...
        for($x = 0; $x < count($txtNoCOA); $x++):
            ...
            $this->M_Fin_AR_zht->insertDetailARjurnal($detail);
        endfor;
        ...
        for($i = 0; $i < count($txtNoCOA); $i++):
            ...
            $this->M_Fin_AR_zht->insertCBhistory($history);
        endfor;
        ...
        $getKursSGD = $this->M_Fin_AR_zht->getKursByIDonAR($this->input->post('txtCurrBayar'),$this->input->post('txtTransDate'))->row();
        $period = $this->M_CashBank->getPeriod();
        ...
        for($y = 0; $y < count($txtNoCOA); $y++):
            ...
            if ($jml <> 0 OR $jml2 <> 0) {
                $this->M_Fin_AR_zht->insertToJurnalAcc($dataJurnal);
            }
        endfor;
        ...
        redirect('ARtrans_zht/reviewARpayment/'.encode_str($headerID));
    }
    ...
    function updatePiutangPerInvoiceNew(){
        ...
        for($x = 0; $x < $rowsPPI; $x++):
            $getBayar       = $this->M_Fin_AR_zht->getBayarFromPiutang($idIncPerInvoice[$x],$jenis[$x]);
            $data   = array(
                'bayar'         => $getBayar + $paymentPerInvoice[$x]
            );
            $this->M_Fin_AR_zht->updatePiutangByNoInvoiceInOneARnumber($idIncPerInvoice[$x],$jenis[$x], $data); // Update table Piutang (ACC)
        endfor;
    }
    function updatePiutangBulananPerInvoiceNew(){
        ...
        for($x = 0; $x < $rowsPPI; $x++):
            $this->M_Fin_AR_zht->updatepiutangBulananByNoInvoiceInOneARnumberQuery($idIncPerInvoice[$x],$jenis[$x],$blnPeriod,$thnPeriod,$paymentPerInvoice[$x]); // Update table Hutang (ACC)
        endfor;
    }
    ...
    function insertDetailAPaccounting($noReffAR){
        ...
        for($x = 0; $x < $rowsPPI; $x++){
            ...
            $this->M_Fin_AR_zht->insertDetailARaccountingFromPayment($data);
        }
    }
    ...
    function updatePurShipInvoice(){
        ...
        for($x = 0; $x < $rows; $x++){
            $cek    = $this->M_Fin_AR_zht->checkInvoiceInPurShip($idIncPerInvoice[$x]);
            if ($cek['status'] == TRUE){
                $this->M_Fin_AR_zht->updatePurchase($cek['noInvoice']);
                $this->M_Fin_AR_zht->updateShipping($cek['noInvoice']);
            }
        }
    }
    ...
    function selectARpayment(){
        $data   = array(
            '_selectHeader' => $this->M_Fin_AR_zht->selectARforFindAR()->result()
        );
        $this->load->view('finance/transaction/ar_trans_zht/findAR/selectAR',$data);
    }
    function selectCustomerForARbyCustCode(){
        $idSupp = $this->input->post('txtCodeSupp');
        $get    = $this->M_Fin_AR_zht->selectCustomerForARbyCode($idSupp);
        $data   = array(
    ...
    function reviewARpayment($headerID){
        ...
        $get    = $this->M_Fin_AR_zht->selectHeaderARforReview($headID);
        ...
        $data   = array(
            ...
            '_selectHeaderAP'   => $this->M_Fin_AR_zht->selectHeaderARforReview($headID),
            '_selectInvoiceAP'  => $this->M_Fin_AR_zht->selectInvoiceARforReview($noReff),
            '_selectDetailAP'   => $this->M_Fin_AR_zht->selectDetailARforReview($headID)->result(),
            '_selectDtlJrnal'   => $this->M_Fin_AR_zht->selectDetailJurnalARforReview($headID)->result()
        );
        $this->template->display('finance/transaction/ar_trans_zht/findAR/index-review',$data);
    }

     function print_report($noAR) {
    
        $get    = $this->M_Fin_AR_zht->selectHeaderARforReview($noAR);
        $data   = array(
                    'HeaderAR'   => $this->M_Fin_AR_zht->selectHeaderARforReview($noAR),
                    'DetailAP'   => $this->M_Fin_AR_zht->selectInvoiceARforReview($noAR),
                    'DtlJrnal'   => $this->M_Fin_AR_zht->selectDetailJurnalARforReview($noAR)         
    ...
    function deleteARpayment(){
        ...
        $get        = $this->M_Fin_AR_zht->selectHeaderARforReview($primary);
        $noReff     = $get->no_facture;
        $thnPeriod  = $get->thn_periode;
        $blnPeriod  = $get->bln_periode;
        $cur        = $get->currency_id;
        ...
        $cek_reff = $this->M_Fin_AR_zht->cek_reff('AR',$thnPeriod,$cur, $blnPeriod);

        $this->M_Fin_AR_zht->deleteDetailARaccountingFromPayment($noReff);
        $this->returnBackPiutangPerInvoiceByPass();
        $this->returnBackPiutangBulananPerInvoice($thnPeriod, $blnPeriod);
        
        // #### ========== ####
        $this->M_Fin_AR_zht->deleteARfromCBhistory($noReff,$primary);
        $this->M_Fin_AR_zht->deleteDetailARpayment($noReff,$primary);
        $this->M_Fin_AR_zht->deleteDetailARjurnal($noReff,$primary);
        ...
        if($noReff != $cek_reff->GEN){
            ...
            $this->M_Fin_AR_zht->updateHeaderARpayment($primary,$data_hdr);
            ...
            // Delete Journal ######
            $this->M_Fin_AR_zht->updateARfromJurnalAcc($noReff,$data_jur);
        } else {
            $this->M_Fin_AR_zht->deleteHeaderARpayment($primary);
            $this->M_Fin_AR_zht->deleteARfromJurnalAcc($noReff);
        }

        
        redirect('ARtrans_zht');
    }
    function returnBackPiutangPerInvoiceByPass(){
        ...
        for($x = 0; $x < $rowsPPI; $x++):
            $this->M_Fin_AR_zht->resetPaymentPiutang($idIncPerInvoice[$x],$jenis[$x]);
        endfor;        
    }
    function returnBackPiutangBulananPerInvoice($thnPeriod, $blnPeriod){
        ...
        for($x = 0; $x < $rowsPPI; $x++):
            $this->M_Fin_AR_zht->updatepiutangBulananByNoInvoiceInOneARnumberQueryReturn($idIncPerInvoice[$x],$jenis[$x],$blnPeriod,$thnPeriod,$paymentPerInvoice[$x]); // Update table Hutang (ACC)
        endfor;
    }
}