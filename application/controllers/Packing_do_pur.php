...
class Packing_do_pur extends CI_Controller
{

    function __construct()
    {
        parent::__construct();
        $this->load->model(array('m_purchasing', 'M_packing_do', 'm_purchasing_so'));
    ...
    {
        $data['_factory'] = $this->M_packing_do->get_factory();
        $this->template->display('purchasing/packing_do/add_loading_report_pur', $data);
    }
    ...
    {
        $data['po'] =  $this->M_packing_do->tampil_gr_pur_po($this->input->get('cust'), $this->input->get('po'));
    ...
    public function save_lr($trans)
    {
        ...
        $datahdr = array(
            'trans' => $trans, 'lrno' => $lrno, 'custid' => $cust,
            'remark' => $remark, 'via' => $via, 'docdate' => $docdate, 'shipdate' => $shipdate, 'total_pack' => $total_pack, 'createdby' => strtoupper($this->session->userdata('userid'))
        );

        $query =  $this->M_packing_do->simpan_lr_pur_sp($datahdr);

        if ($query['flag'] == 1) {
            $this->session->set_flashdata("message", "<div class=\"alert alert-success\" id=\"alert\">Transaction Success</div>");
            redirect('Packing_do_pur/loading_report_show?lr=' . $query['lrno']);
        } else {
            $this->session->set_flashdata("message", "<div class=\"alert alert-danger\" id=\"alert\">Save Transaction Broken.</div>");
            redirect('Packing_do_pur/add_loading_report');
        }
    }
    ...
    {
        $data['po'] =  $this->M_packing_do->tampil_gr_where_other($this->input->get('po'));
        $this->load->view('purchasing/packing_do/delivery_oder_filter_gr', $data);
    }

    public function delivery_oder_do()
    {
        $data['do'] =  $this->M_packing_do->tampil_do(trim($this->input->get('do')));
