...
class Container extends CI_Controller
{
    ...
    public function shipping_liner_save()
    {
        ...
        if ($shippingid == '') {
            $data = array('shipping_name' => $shippingname, 'shipping_tipe' => $shippingtipe, 'createdby' => strtoupper($this->session->userdata('userid')), 'createddate' => date('Y-m-d H:i:s'));
            $this->m_shipping->simpan_shipping_liner($data);
            $message = 'Save Data Success';
        } else {
            $data = array('shipping_name' => $shippingname, 'shipping_tipe' => $shippingtipe, 'lastupdatedby' => strtoupper($this->session->userdata('userid')), 'lastupdateddate' => date('Y-m-d H:i:s'));
    ...
    public function port_save()
    {
        ...
        if ($portid == '') {
            $data = array('port_code' => $portcode, 'port_name' => $portname, 'country_ids' => $country, 'created_by' => strtoupper($this->session->userdata('userid')), 'created_date' => date('Y-m-d H:i:s'));
            $this->m_shipping->simpan_port($data);
            $message = 'Save Data Success';
        } else {
            $data = array('port_code' => $portcode, 'port_name' => $portname, 'country_ids' => $country, 'updated_by' => strtoupper($this->session->userdata('userid')), 'updated_date' => date('Y-m-d H:i:s'));
    ...
    public function container_save($trans)
    {
        ...
        $datahdr = array(
            'trans' => $trans, 'contid' => $contid, 'tipe' => $tipe,
    ...
    public function container_loading_save($trans)
    {
        ...
        $dataHdr = array(
            ...
            'to' => $to, 'attn' => $attn, 'from' => $from, 'remarks' => $remarks, 'createdby' =>  strtoupper($this->session->userdata('userid'))
        );
    ...
    public function container_loading_excel()
    {
        ...
        $counter++;
        $objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A' . $counter, 'PLS CONFIRM ALL DETAILS ARE CORRECT BEFORE 1ST CARRIER ARRIVAL');
        $counter++;
        $objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A' . $counter, 'CONTAINER MUST BE STOWE "UNDER DECK AWAY BOILER"');
        $counter++;
        $objPHPExcel->setActiveSheetIndex(0)
            ->setCellValue('A' . $counter, 'CONTAINER ARE DECLARED UNDER TRANSSHIPMENT');
        $counter++;
        $objPHPExcel->setActiveSheetIndex(0)
