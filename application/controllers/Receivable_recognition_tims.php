...
class Receivable_recognition_tims extends CI_Controller
{
    ...
    function __construct()
    {
        ...
        $this->load->model(array('M_Receivable_recognition_tims', 'M_vcdn','M_login', 'M_Sales_inv', 'M_login','M_General_Journal'));
    ...
    {
        $data['SupplierID']     = $this->M_Receivable_recognition_tims->get_customer_tims_list();
        $data['title']          = "List of Receivable Recognition Tims";
        $data['List_payable']   = $this->M_Receivable_recognition_tims->get_list_piutang();
        $this->template->display('accounting/Receivable_recognition_tims/Receivable_recognition_list', $data);
    }

    function buat_nofak()
    {
        $tahun = $this->input->get("tahun");
        $data['nofak'] = $this->M_Receivable_recognition_tims->cari_nofaktur($tahun);
    ...
    function search()
    {
        ...
        $data['tgl'] = $dari;
        if ($dari != '') {
            $data['List_payable'] = $this->M_Receivable_recognition_tims->advance_list_piutang($p_dari, $p_sampai, $invoice, $supplier);
            $data['search_active'] = !empty($data['List_payable']);
        } else {
            $data['List_payable'] = $this->M_Receivable_recognition_tims->advance_list_piutang1($invoice, $supplier);
        }
    ...
    function add_new()
    {
        ...
        $data['SupplierID'] = $this->M_Receivable_recognition_tims->get_customer_tims();
        $data['Currency']   = $this->M_Receivable_recognition_tims->get_currency();
        ...
        $this->template->display('accounting/Receivable_recognition_tims/Receivable_recognition_form', $data);
    }
    ...
    function get_supp_piu(){
        ...
        $cekPiutang = $this->M_Receivable_recognition_tims->call_cek_sp_rec_piutang($supp, $currency, $periode); 
        if ($cekPiutang) {
    ...
    function data_dp()
    {
        ...
        $data['pilih_dp'] = $this->M_Receivable_recognition_tims->pilih_dp($id, $currency);
    ...
    function edit()
    {
        ...
        $data['Currency']       = $this->M_Receivable_recognition_tims->get_currency();
        $data['message']        = $this->session->flashdata('message');
        $data['CurrencyID']     = $this->M_Receivable_recognition_tims->get_currency_detail();
        $data['List_coa']       = $this->M_vcdn->get_coa($company);
        $id = $this->input->get("id");

        //variable invoice number header
        $data['HeaderID']        = $id;
        $data['get_data_header'] = $this->M_Receivable_recognition_tims->get_data_header($id);
        $data['nota']            = $this->M_Receivable_recognition_tims->nota($id);
        //variable invoice number detail
        $data['get_data_detail'] = $this->M_Receivable_recognition_tims->get_data_detail($id);
        //variable invoice number footer
        $data['get_data_footer'] = $this->M_Receivable_recognition_tims->get_data_footer($id);
        $data['attach'] = $this->M_Receivable_recognition_tims->list_attach($data['nota'][0]->nofaktur);


        $data['get_data_jurnal1'] = $this->M_Receivable_recognition_tims->get_data_awal($id, 'AR/GL');
        $data['get_data_jurnal2'] = $this->M_Receivable_recognition_tims->get_data_jurnal(2, $id, 'RRGT');
        $data['get_data_jurnal3'] = $this->M_Receivable_recognition_tims->get_data_jurnal(3, $id, 'RRGT');
        $data['get_data_jurnal4'] = $this->M_Receivable_recognition_tims->get_data_jurnal(4, $id, 'RRGT');
        $data['get_data_jurnal5'] = $this->M_Receivable_recognition_tims->get_data_jurnal(5, $id, 'RRGT');
        $data['get_data_jurnal6'] = $this->M_Receivable_recognition_tims->get_data_jurnal(6, $id, 'RRGT');

        $data['dept_code'] = $this->M_General_Journal->get_departmentcode();
        $data['dept_code_json'] = json_encode($data['dept_code']);
        $data['data_bawah']       = $this->M_Receivable_recognition_tims->data_paling_bawah($id);
        $close_date         = $this->M_login->ambil_tgl()->row();
        $data['closing']    = date_format(date_create($close_date->tanggal), "d/m/Y");
    ...
    {
        // $data['SupplierID'] = $this->M_Receivable_recognition_tims->get_customer();
        // $data['Currency'] = $this->M_Receivable_recognition_tims->get_currency();
        // $data['CurrencyID'] = $this->M_Receivable_recognition_tims->get_currency_detail();
        ...
        // echo "</pre>";
        // die;
        $a = $this->M_Receivable_recognition_tims->get_data_detail($id);
        $i = "INVOICE";
        $totalgst = 0;
        if (!empty($a)) {
            foreach ($a as $r) {
        ...
        $this->load->view('accounting/rpt/rpt_receivable_recognition_tims', $data);
    }
    ...
    function merged_pdf()
    {
        ...
        $list = $this->M_Receivable_recognition_tims
            ->advance_list_piutang($p_dari, $p_sampai, $invoice, $supplier);
        ...
        $data['listInvoice'] = $list;
        $this->load->view('accounting/rpt/rpt_receivable_recognition_tims_merged', $data);
    }

    function list_currency()
    {
        $data['Currency'] = $this->M_Receivable_recognition_tims->get_currency_detail();
    ...
    function list_payable()
    {
        ...
        //variable invoice number header
        $this->M_Receivable_recognition_tims->get_data_header($id);
    }
    ...
    function save_receivable_rec()
    {
        ...
        $trgt_app       = str_replace('/', '-', $this->input->post('trgt_app'));
        $target_app     = date('Y-m-d', strtotime($trgt_app));
        $shipper        = $this->input->post('shipper');
        $bargenum       = $this->input->post('bargenum');
        $head_jur       = $this->input->post('JenisJurnal_head');
        ...
        if ($nofaktur == '') {
            ...
            $sql_faktur = $this->M_Receivable_recognition_tims->get_nofaktur($p_tahun, $p_bulan);
            $nofaktur = $sql_faktur;
        }

        $this->M_Receivable_recognition_tims->delete_piutang($nofaktur);
            ...
            for ($i = 0; $i < $count; $i++) {
                ...
                if ($_FILES['file_atch']['type'] == 'application/pdf') {
                    ...
                    if ($this->upload->do_upload('file_atch')) {
                        ...
                        $this->M_Receivable_recognition_tims->save_file($data);
                    }
                }
            }
        }
        ...
        for ($i = 0; $i < count($this->input->post('Detail_item_id')); $i++) {
            ...
            $this->M_Receivable_recognition_tims->simpan_pr($det_item);
            ...
            if ($txtGST[$i] <> "") {
                $this->M_Receivable_recognition_tims->simpan_gst_payable($gst_item);
            }
        }
        ...
        for ($a = 1; $a < count($this->input->post('DetailID')); $a++) {
            ...
            if ($jr_total[$a] <> 0) {
                $this->M_Receivable_recognition_tims->simpan_jurnal($det_jur);
            }
        }
        ...
        for ($i = 0; $i < count($this->input->post('Detail_item_id')); $i++) {
            ...
            if ($head_jur=='CCN') {
                $sum_debet     += number_format(str_replace(",", "", $credit2), 2, ".", "");
                $sum_debet_sgd += number_format(str_replace(",", "", $credit_sgd2), 2, ".", "");
                ...
                $type_chk = "D";
                $type_debt = number_format(str_replace(",", "", $credit2), 2, ".", "");
                $type_cred = 0;
                $type_debt_sgd = number_format(str_replace(",", "", $credit_sgd2), 2, ".", "");
                $type_cred_sgd = 0;
            }else{
                ...
                $type_chk = "C";
                $type_debt = 0;
                $type_cred = number_format(str_replace(",", "", $credit2), 2, ".", "");
                $type_debt_sgd = 0;
                $type_cred_sgd = number_format(str_replace(",", "", $credit_sgd2), 2, ".", "");
            ...
            $this->M_Receivable_recognition_tims->simpan_jurnal($detail_jur);
        }
        // $this->M_Receivable_recognition_tims->hapus_COA_kosong();
        ...
        if ($stsDP == "DP") {
            ...
            $headerID = $this->M_Receivable_recognition_tims->insertCashBankHeaderx($data);
            ...
            for ($x = 0; $x < count($txtNoCOA); $x++) :
                ...
                $this->M_Receivable_recognition_tims->insertCashBankDetailx($detail);
            endfor;
            ...
            for ($i = 0; $i < count($txtNoCOA); $i++) :
                ...
                $this->M_Receivable_recognition_tims->insertCBhistoryx($history);
            endfor;
            $txtCustID = $supplier;
            ...
            for ($z = 0; $z < $cnLoop; $z++) {
                ...
                $this->M_Receivable_recognition_tims->insertDetailPOcbTransactionx($data);
            }
        }
        ...
        // echo "</pre>";
        // die;
        $this->M_Receivable_recognition_tims->call_sp_rec_piutang_new($data);
        ...
        redirect("Receivable_recognition_tims/edit?id=$nofaktur");
    ...
    function delete()
    {
        ...
        $this->M_Receivable_recognition_tims->delete_item($id);
        $this->M_Receivable_recognition_tims->delete_jurnal($idj);
        redirect("Receivable_recognition_tims/edit?id=$nofaktur");
    }
    ...
    {
        $id = $this->input->get("id");
        $data['select_hutang'] = $this->M_Receivable_recognition_tims->nota($id);
    ...
    function hapus()
    {
        $id = $this->input->get("id");
        $nofaktur = $this->input->get("nofaktur");
        $this->M_Receivable_recognition_tims->delete_jurnal($id);
        redirect("Receivable_recognition_tims/edit?id=$nofaktur");
    }
    ...
    {
        $id = $this->input->get("id");
        $this->M_Receivable_recognition_tims->delete_piutang($id);
        redirect("Receivable_recognition_tims");
    }

    function selectInvoiceReceivable()
    {
        $data   = array(
            '_selectHeader' => $this->M_Receivable_recognition_tims->selectInvoiceforFindRR()->result()
        );
        $this->load->view('accounting/Receivable_recognition_tims/FindRR/selectIRR', $data);
    }

    function delete_file()
    {
        $id = $this->input->post('id');
        $this->M_Receivable_recognition_tims->delete_file($id);
        echo json_encode(['status' => true]);
    }

    //------------------------------------------------About Mon Purchase Order-------------------------------------------------------
    function mon_receivable_recognition_zht()
    {
        $data['SupplierID']     = $this->M_Receivable_recognition_tims->get_sup();
        $data['ItemId']     = $this->M_Receivable_recognition_tims->item_rrg();
        $data['title']          = "Monitoring Receivable Recognition ZHT";
        $this->template->display('accounting/Receivable_recognition_tims/Receivable_recognition_mon', $data);
    }

    function filter_receivable_recognition_zht()
    {
        ...
        $data['po'] =  $this->M_Receivable_recognition_tims->receivable_recognition_zht_filter($from, $to, $customer, $item, $invtype);
        
        $this->load->view('accounting/Receivable_recognition_tims/Receivable_recognition_filter', $data);
    }

    function tampil_item_cust()
    {
        $customer = $this->input->get("cust");
        $data = $this->M_Receivable_recognition_tims->item_rrg_customer($customer);
