...
class Customers_zht extends CI_Controller {

    function __construct() {
        parent::__construct();
        $this->load->model(array('M_Customer_card_zht','M_Fin_AR_zht','M_Fin_Dp'));
    ...
    public function index() {
        ...
        $data['SupplierID'] = $this->M_Customer_card_zht->get_customer();
        $data['CurrencyID'] = $this->M_Customer_card_zht->get_currency();
        
        $this->template->display('accounting/Customer_card_zht', $data);
    }
    function search() {
        ...
        $data['SupplierID'] = $this->M_Customer_card_zht->get_customer();
        $data['CurrencyID'] = $this->M_Customer_card_zht->get_currency();
        $data['Get_aging']  = $this->M_Customer_card_zht->call_data2($sup,$cur, $from,$to, $coa);
    ...
    function reviewARpayment(){
        ...
        $get    = $this->M_Fin_AR_zht->selectHeaderARforReviewByNoReff($headID);
        ...
        $data   = array(
            ...
            '_selectHeaderAP'   => $this->M_Fin_AR_zht->selectHeaderARforReview($noReff),
            '_selectInvoiceAP'  => $this->M_Fin_AR_zht->selectInvoiceARforReview($headID),
            '_selectDetailAP'   => $this->M_Fin_AR_zht->selectDetailARforReview($noReff)->result(),
            '_selectDtlJrnal'   => $this->M_Fin_AR_zht->selectDetailJurnalARforReview($noReff)->result()
    ...
    //     $data['from'] = $from;
    //     $data['to'] = $to;
    //     $data['Get_aging']  = $this->M_Customer_card_zht->call_data($sup,$cur, $from,$to, $coa);
    ...
    function toPrintCustomerCard()
    {
        ...
        $s1 = $this->M_Customer_card_zht->get_cust($sup);
        // $s2 = $s1->customer_name;

        $data['from'] = $from;
        $data['to'] = $to;
        $data['supplier']  = $s1->customer_name;
        // $data['get_data']  = $this->M_Customer_card_zht->call_data($sup,$cur, $from,$to, $coa);
        $data['get_data'] =  $this->M_Customer_card_zht->call_data2($sup, $cur,$from, $to,$coa);

        // echo $s2." ".$from." ".$to."<br>";
        // foreach ($a as $r) {
        //     echo $r->tmp_kodecus."<br>";
        // }
        $this->load->view('accounting/rpt/rpt_customer_card_zht', $data);
