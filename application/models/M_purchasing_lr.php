...
class M_purchasing_lr extends CI_Model
{

    function tampil_pl_pur_po($cust, $po)
    {
        $query1 = "custid='" . $cust . "'";
        ...
        $result = $this->db->get('zhl_purchasing_vw_trn_pl_by_lr');
    ...
    function tampil_lr_all($data)
    {
        ...
        $result = $this->db->get('zhl_purchasing_vw_trn_lr_hdr');
        return $result->result();
    }
    ...
    function simpan_lr_sp_hdr($datahdr, $jenis, $pl_no)
    {
        ...
        $this->db->query("call zhl_sp_purchasing_tbl_trn_lr_hdr('" . $datahdr['trans'] . "',@lrno,'" . $datahdr['custid'] . "',"
    ...
    function simpan_lr_sp_dtl($lrno, $SeqNo, $itemid, $itemname, $qty, $uom, $gweight, $nweight, $docno, $mainpo, $npbbno, $cust, $jenis, $transaksi, $id_lr, $ppbid, $jml)
    {
        for ($i = 0; $i < $jml; $i++) {
            $this->db->query("SET @flag = 0");
            $this->db->query("call zhl_sp_purchasing_tbl_trn_lr_dtl('" . $lrno . "','" . $SeqNo[$i] . "','" . $itemid[$i] . "','" . str_replace("'", '`', $itemname[$i]) . "','" . $qty[$i] . "','" . $uom[$i] . "','" . $gweight[$i] . "','" . $nweight[$i] . "',"
    ...
    function tampil_lr_where($data)
    {
        ...
        $result = $this->db->get('zhl_purchasing_vw_trn_lr');
        return $result->result();
    }
    ...
    {
        $this->db->query("update zhl_purchasing_tbl_trn_lr_hdr set status=1 where lrno='$lr_no'");
        $this->db->where('lr_no', $lr_no);
        $this->db->delete('zhl_purchasing_tbl_trn_lr_dtl');
    }
    ...
    {
        $result =  $this->db->get('zhl_pur_tbl_mst_customer');
        return $result->result();
    }
 ...
 */
    ...
    {
        $this->db->query("SET @flag = 0");
        $this->db->query("call zhl_sp_pur_tbl_trn_pl_hdr(
            '" . $datahdr['pl_no'] . "',
            '" . $datahdr['packing'] . "',
            '" . $datahdr['totalpack'] . "',
            '" . $datahdr['shipdate_pl'] . "',
            '" . $datahdr['via'] . "',
            '" . $datahdr['createdby'] . "',@flag)");
        $row =  $this->db->query("Select @flag as flag")->row();
        return $row;
    }
    ...
    function simpan_do_sp($datahdr)
    {
        ...
        $query2 = $this->simpan_do_sp_dtl($docno1, $MainPO, $itemid, $itemname, $qty, $npbb, $custid, $sono, $docno_gr, $ppbid, $datahdr['createdby'], $jml);
        $docno2 = $query2->docno;
        $flag2 = $query2->flag;
        ...
        if ($this->db->trans_status() === FALSE) {
            ...
            $this->db->trans_commit();
            $data = array('flag' => $flag2, 'docno' => $docno2);
            return $data;
        }
    }
    ...
    function delete_do_sp($docno1, $MainPO, $itemid, $npbb, $custid, $docno_gr, $jml)
    {
        for ($i = 0; $i < $jml; $i++) {
            $this->db->query("SET @flag = 0");
            $this->db->query("call sp_pur_tbl_trn_do_del('" . $docno1 . "','" . $MainPO[$i] . "','" . $itemid[$i] . "','" . $npbb[$i] . "','" . $custid[$i] . "','" . $docno_gr[$i] . "',@flag)");
        }
        $row =  $this->db->query("Select @flag as flag")->row();
        return $row;
    }
