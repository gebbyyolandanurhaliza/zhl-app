...
class M_Balance_sheet extends CI_Model {
    ...
    // tambah baru
    function getdata_zht($sampai){
        $sql = $this->db->query("call zhl_balancesheet_2022_zht('$sampai')");
        $res = $sql->result();
        $sql->next_result();
        $sql->free_result();
        return $res;
    ...
    function getdata($sampai){
        $sql = $this->db->query("call zhl_balancesheet_2022('$sampai')");
        $res = $sql->result();
        $sql->next_result();
        $sql->free_result();
        return $res;
    ...
    function getdata_2025($sampai){
        $sql = $this->db->query("call zhl_balancesheet_2025('$sampai')");
        $res = $sql->result();
        $sql->next_result();
        $sql->free_result();
        return $res;
    ...
    function coa_number_zht() {
        $this->db->select('Kombinasi_COA, AccountName');
        $this->db->from('zhl_vw_new_coa_dept_code');
        $sql = $this->db->get();
        if ($sql->num_rows() > 0) {
            $result[''] = 'Select';
            foreach ($sql->result_array() as $row) {
                $result[$row['Kombinasi_COA']] = ucwords(strtolower($row['Kombinasi_COA']) . " | " . strtoupper($row['AccountName']));
            }
            return $result;
        } else {
            echo "Not data avaible";
