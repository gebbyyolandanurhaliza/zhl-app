...
class M_General_Journal extends CI_Model {
    ...
    function simpan_header($data){
        $this->db->insert('zhl_acc_tbl_trn_general_detail', $data);
    }
    ...
    function delete_header($header){
        ...
        $this->db->delete('zhl_acc_tbl_trn_general_detail');
    }
    ...
    function update_header($header,$data){
        $this->db->where('no_reff', $header);
        $this->db->update('zhl_acc_tbl_trn_general_detail',$data);
    }

    function get_header($id){
        $this->db->where('no_reff', $id);
        $sql = $this->db->get('zhl_acc_tbl_trn_general_detail');
        if($sql->num_rows() > 0){
    ...
    function get_list(){
        $this->db->select('*');
        $sql = $this->db->get('zhl_vw_acc_trb_general');
        if($sql->num_rows() > 0){
            foreach ($sql->result() as $data) {
                $hasil[] = $data;
            }
    ...
    function get_refnum($thn, $bln){
        ...
        $sql = $this->db->query("SELECT no_reff FROM zhl_acc_tbl_trn_general_detail
            WHERE YEAR(tanggal) = '$thn' AND MONTH(tanggal) = '$bln' AND `no_reff` LIKE '%GV%' 
    ...
    function cek_gl($inv){
        $sql = $this->db->query("SELECT no_reff as no_reff FROM zhl_acc_tbl_trn_general_detail
