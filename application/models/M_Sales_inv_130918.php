...
class M_Sales_inv extends CI_Model
{
    ...
    function get_isilclcont($tgl, $bargedest){
        $sql = " AND bargejurnal IS NULL "; 
        if($bargedest == 'idn' || $bargedest == 'idn2'){ 
            $sql = $sql." AND (stuffing IN ('LE')) ";
            $stf = '';
            if($bargedest == 'idn'){
                $sql = $sql." AND etd = 'PSG00' ";
            }else{
                $sql = $sql." AND etd = 'RSUP00' ";
            }
        }else if($bargedest == 'sin' || $bargedest == 'sin2'){ 
        ...
        SELECT container_id as contid, container_number as container_id, container_type as container_name, etd, eta, etadate, etddate, stuffing FROM ship_vw_container_local  where container_number != '' AND shipmentdate = '".$tgl."' ".$sql." ) A
        GROUP BY A.container_name, A.stuffing ";
        // echo $sqlit;
    ...
    function get_isilclcontdtl($tgl, $bargedest){
        $sql = " AND bargejurnal IS NULL "; 
        if($bargedest == 'idn' || $bargedest == 'idn2'){ 
            $sql = $sql." AND (stuffing IN ('LE')) ";
            $stf = '';
    ...
    function get_harga($idcon, $jenisinv){
        ...
        if($jenisinv == 'EE' || $jenisinv == 'EL' || $jenisinv == 'IT' || $jenisinv == 'IL' || $jenisinv == 'LL' || $jenisinv == 'LE' || $jenisinv == 'RE'){
    ...
    function get_detail_freigth($awal, $akhir){
        $sql = $this->db->query("SELECT A.invno, A.custcompany,  A.destination_id, A.destination,A.container_name, COUNT(container) as jumlah_container,
                    ponumberinv(A.invno) as po_num, A.Harga, A.contid FROM (SELECT DISTINCT  A.invno, A.custcompany, A.docdate, D.container_name, C.destination_id, 
                    F.destination, F.container, IFNULL(H.cust_rates,0) AS Harga, C.container_id AS contid  FROM ship_tbl_trn_inv_hdr A LEFT JOIN ship_tbl_trn_inv_dtl B 
                    ...
                    zhl_shp_tblmst_freightcharges H ON (C.destination_id = H.port_id AND C.container_id = H.container_id) where G.tipe = 2 
    ...
    function get_detail_freigthcont($awal, $akhir){
        $sql = $this->db->query("SELECT DISTINCT  A.invno, F.container,F.contid, C.container_id, D.container_name, F.seal , '' as stuffing  
                                ...
                                where G.tipe = 2 AND A.docdate BETWEEN '$awal' AND '$akhir' AND F.jurnal_freight_sales IS NULL AND C.container_id not in (6,9)");
        return $sql->result();
    }
