...
class M_zht_mar_master extends CI_Model
{
    ...
    private $containerzhl_table    = 'zhl_mar_tblmst_container_zhl';
    private $containerzhl_id        = 'container_zhl_id';
    ...
    private $paymentterm_table    = 'zht_mar_tblmst_payment_term';
    ...
    private $comp_table            = 'zhl_gen_tbl_mst_company_acc';
    ...
    private $sql_cust = 'SELECT a.customer_id,	a.customer_code, a.customer_name,	a.customer_company_name,
			...
			FROM
				zht_mar_tblmst_customer a	LEFT JOIN
				zhl_gen_tbl_mst_country b ON b.country_id = a.country_id LEFT JOIN
				zht_mar_tblmst_customer_group c ON c.customer_group_id = a.customer_group_id
			WHERE 1=1 ';
    ...
    //gebby
    function cust_get_all_zht()
    {
        $this->db->order_by('customer_company_name', $this->order);
        return $this->db->get($this->cust_zht_view)->result();
    }

    function cust_get_name($cust_id)
    {
        $this->db->select('customer_company_name')
            ->from('zht_mar_tblmst_customer')
    ...
    {
        return $this->db->get('zhl_mar_tblmst_payment_term')->result();
    ...
    function cust_payterm_get_all_filtered($filter)
    {
        ...
        return $this->db->get('zhl_mar_tblmst_customer_payterm')->result();
    }
    ...
    function document_get_by_customer($cust_id, $special = 0)
    {
        ...
        $this->db->order_by('document_name', $this->order);
        return $this->db->get('zhl_mar_vw_mst_customer_document')->result();
    }
    ...
    function container_get_all()
    {
        ...
        $this->db->order_by('container_id', $this->order);
    ...
    function container_insert_zhl($data)
    {
        $exec = $this->db->insert($this->containerzhl_table, $data);
    ...
    // delete data container zhl
    function container_delete_zhl($param)
    {
        $id = $param['container_zhl_id'];
        $this->db->where($this->containerzhl_id, $id);
        $exec = $this->db->update($this->containerzhl_table, $param);

        return $exec;
    }

    //==================================================================================================
    // Container ZHL Section
    //==================================================================================================
    // get all
    function container_zhl_get_all()
    {
        $this->db->select("a.*, b.container_name");
        $this->db->join($this->container_table . " b", "a.container_id = b.container_id");
        $this->db->where('a.not_active', 0);
        $this->db->order_by('container_zhl_id', $this->order);
        return $this->db->get($this->containerzhl_table . " a")->result();
    }
    ...
    function get_all_comp()
    {
        ...
        return $this->db->get($this->comp_table)->result();
    }

    function get_comp_access($user_group_id)
    {
        $this->db->where('user_group_id', $user_group_id);
        return $this->db->get('zhl_gen_tbl_utl_company_acc_access')->result();
    }
    ...
    function save_company_access($user_group_id, $factory_id)
    {
        ...
        if (!$frow){
            ...
            $this->db->insert('zhl_gen_tbl_utl_company_acc_access', $data);
        }
    }
    ...
    function get_company_access_by_id($user_group_id, $factory_id)
    {
        ...
        return $this->db->get('zhl_gen_tbl_utl_company_acc_access')->row();
    }
}
