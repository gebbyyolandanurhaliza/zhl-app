...
class M_master_freight_new_new extends CI_Model {
    ...
    private $tbl_freight_hdr ='zhl_shp_tblmst_freightcharges_hdr';
    private $tbl_freight_dtl ='zhl_shp_tblmst_freightcharges_dtl';
   ...
   function get_customer_name()
    {
         ...
         // $query = $this->db->query('SELECT * FROM zhl_shp_tblmst_freightcharges_hdr as a left join zhl_shp_tblmst_freightcharges_dtl as x on a.freight_charges_id = x.freight_charges_id LEFT JOIN mar_tblmst_customer as b on a.consignee = b.customer_id  where a.deleted = 0 group by consignee order by customer_name ASC');
         $query = $this->db->query('select * from zhl_ship_vw_trn_freight_charges_for_master_new where kadaluarsa > -60 and deleted = 0  group by consignee order by customer_name ASC');
        return $query->result(); 
    }

     function get_data()
    {
        $query = $this->db->query('select * from zhl_ship_vw_trn_freight_charges_for_master_new where kadaluarsa > -60 and deleted = 0  group by freight_charges_id order by port_name ASC');
        return $query->result();
        // $query = $this->db->query('select * from zhl_shp_tblmst_freightcharges_dtl ');
        // return $query->result();
    }
    function get_freight_hdr($freight_charges_id)
    {
        $this->db->where('freight_charges_id', $freight_charges_id);
        return $this->db->get($this->tbl_freight_hdr)->row();
        // $this->db->where($this->freight_charges_id, $freight_charges_id);
        // return $this->db->get($this->tbl_freight_hdr)->row();
    }
     function get_freight_dtl($freight_charges_id)
    {
        $this->db->where('freight_charges_id', $freight_charges_id);
        $this->db->where('deleted', 0);
        return $this->db->get($this->tbl_freight_dtl)->result();
        // $sql = $this->db->query("call sp_mst_freight($freight_charges_id)");
        // $res = $sql->result();
        // $sql->next_result();
        // $sql->free_result();
        // return $res;
     ...
     function insert() {
        ...
        if ($this->db->trans_status() === FALSE) {
            log_message('error', 'saving master freight');
            return 0;
        } else {
            return $header_id;
        }
     ...
     function update()
    {
        //$freight_charges_id = decode_str($this->input->post('freight_charges_id'));
        $freight_charges_id = ($this->input->post('freight_charges_id'));
    //   echo json_encode($freight_charges_id);
    //     die;
        $this->db->trans_off();
        
        $this->db->trans_start();       
        $this->update_header($freight_charges_id);
        $this->update_detail($freight_charges_id);
        ...
        if ($this->db->trans_status() === FALSE)
        {
            log_message('error', 'error updating master freight');
            return 0;
        } else {
            return $freight_charges_id;
        }
    }
    ...
    private function insert_header(){
        ...
        $info_header = array(
            ...
            'shipping_term_id'  => $this->input->post('fob_id'),
            ...
            'comfirm'           => $this->input->post('comfirm')
        );

        $this->db->insert($this->tbl_freight_hdr, $info_header);
        $freight_header = $this->db->insert_id();

        return $freight_header;
    }

    function update_header($freight_charges_id)
    {
        ...
        $this->db->update($this->tbl_freight_hdr, $info_header, array('freight_charges_id' => $freight_charges_id));
        
    }

    private function insert_detail($freight_charges_id){
        $detail_count   = count($this->input->post('freight_charges_dtl_id'));
        $freight_charges_dtl_id  = $this->input->post('freight_charges_dtl_id');
        $shipping_id    = $this->input->post('shipping_id');
        $shipping_line  = $this->input->post('shipping_line');
        $vendor_rates   = $this->input->post('vendor_rates');
        $vendor_rates2  = $this->input->post('vendor_rates2');
        $vendor_rates3  = $this->input->post('vendor_rates3');
        $vendor_misc    = $this->input->post('vendor_misc');
        $cust_rates     = $this->input->post('cust_rates');
        ...
        for ($i = 0; $i < $detail_count; $i++) {
            ...
            'shipping_line'      => $shipping_line[$i],
            'vendor_rates'       => remove_thousand_separator($vendor_rates[$i]),
            'vendor_rates2'      => remove_thousand_separator($vendor_rates2[$i]),
            'vendor_rates3'      => remove_thousand_separator($vendor_rates3[$i]),
            'vendor_misc'        => $vendor_misc[$i],
            'cust_rates'         => remove_thousand_separator($cust_rates[$i]),
            'cust_misc'          => $cust_misc[$i]
        );

            $this->db->insert($this->tbl_freight_dtl, $info_detail);
        }

    }

      private function update_detail($freight_charges_id)
    {
        $detail_count = count($this->input->post('freight_charges_dtl_id'));
        $freight_charges_dtl_id  = $this->input->post('freight_charges_dtl_id');
        $shipping_id    = $this->input->post('shipping_id');
        ...
        for ($i = 0; $i < $detail_count; $i++){
            ...
            if ($freight_charges_dtl_id[$i] == 0){
                $this->db->insert($this->tbl_freight_dtl, $info_detail);
            } else {
                $this->db->update($this->tbl_freight_dtl, $info_detail, array('freight_charges_dtl_id' => $freight_charges_dtl_id[$i]));
            }
        }       
    }
    function remove_detail()
    {
        $freight_charges_dtl_id = $this->input->post('id');
        $this->db->where('freight_charges_dtl_id', $freight_charges_dtl_id);
        $this->db->update($this->tbl_freight_dtl, array('deleted' => 1));
    }
      // get data by id
    private $sql_cust ='select * from zhl_ship_vw_trn_freight_charges_for_master_new WHERE 1=1 ';

    function freight_get_by_id($id)
    {
        $sql = $this->sql_cust." and freight_charges_id = ".$id.";";
        return $this->db->query($sql)->row();
    }
    function freight_delete($id)
    {
        $this->db->where('freight_charges_id', $id);
        $this->db->update($this->tbl_freight_hdr, array('deleted'=> 1));
    }

    function freight_dtl_delete($freight_charges_id)
    {
        $this->db->where('freight_charges_id', $freight_charges_id);
        $this->db->delete($this->tbl_freight_dtl, array('deleted'=> 1));
    }


    function tampil_mst_freight(){
       // $query = $this->db->query('select * from zhl_ship_vw_trn_freight_charges_for_master_new where kadaluarsa > -60 order by port_name ASC ');
        $query = $this->db->query('select * from zhl_ship_vw_trn_freight_charges_for_master_new where kadaluarsa > -60 and deleted = 0  GROUP BY freight_charges_id order by consignee ASC ');
        return $query->result();
    }
    ...
    //         'rates_cust'        => remove_thousand_separator($this->input->post('cust_rates')),
    //         'vendor_misc'       => $this->input->post('vendor_misc'),
    //         'cust_misc'         => $this->input->post('cust_misc'),
    //         'validity_from'     => dmy_to_ymd($this->input->post('validity_from')),
    //         'validity_till'     => dmy_to_ymd($this->input->post('validity_till')),
    ...
    function tradingterm_get_fob()
    {
        ...
        return $this->db->get('mar_tblmst_trading_term_new_version_1')->result();
    }
    ...
    function filter_freight($dest,$cont,$ship,$con){
        ...
        $query = ('port_id = "'.$dest.'" and container_id = "'.$cont.'" and shipping_term_id = "'.$ship.'" and consignee = "'.$con.'" and kadaluarsa > -60');
        $this->db->where($query);

        } else if($dest !== ''){
        
        $query = ('port_id = "'.$dest.'" and kadaluarsa > -60');
        $this->db->where($query);

        } else if ($cont !== ''){
        
        $query = ('container_id = "'.$cont.'" and kadaluarsa > -60');
        $this->db->where($query);

        } else if ($ship !== ''){
        
        $query = ('shipping_term_id = "'.$ship.'" and kadaluarsa > -60');
        $this->db->where($query);

        } else if ($con !== ''){
        
        $query = ('consignee = "'.$con.'" and kadaluarsa > -60');
        $this->db->where($query);
        ...
        $this->db->where('deleted', 0);
        $this->db->group_by('freight_charges_id', 'ASC');
        $this->db->order_by('port_name', 'ASC');
        return $this->db->get('zhl_ship_vw_trn_freight_charges_for_master_new')->result();        
    }
}
