...
class M_Fin_DpNew extends CI_Model{
    ...
    function selectCustomerforDP(){
        return $this->db->query("SELECT customer_code, customer_company_name, customer_group_name, coa, zhl_acc_master_coa.AccountName AS name_coa FROM zhl_fin_vw_mst_customer JOIN zhl_acc_master_coa ON zhl_fin_vw_mst_customer.coa = zhl_acc_master_coa.NoCOA");
    }
    ...
    function updateJurnal($noInv,$data){
        $this->db->where(array('NoJurnal' => $noInv, 'jenis_trans' => 'PDP'));
    ...
    function getRateByCurrBank($tgl, $coa){
        $query  = $this->db->query("SELECT *, "
                . "(SELECT rate_usd FROM zhl_acc_tbl_trn_kurs WHERE periode < '".$tgl."' AND currency_id = zhl_fin_tblmst_awal_saldo.Currency ORDER BY periode DESC LIMIT 1) AS rate_usd, "
                . "(SELECT rate_kurs FROM zhl_acc_tbl_trn_kurs WHERE periode < '".$tgl."' AND currency_id = zhl_fin_tblmst_awal_saldo.Currency ORDER BY periode DESC LIMIT 1) AS rate_sgd "
                . "FROM  zhl_fin_tblmst_awal_saldo WHERE no_coa = '".$coa."'");
        return $query->row();
    }
}