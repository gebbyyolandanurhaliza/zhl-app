...
class M_Fin_AP extends CI_Model{
    ...
    function selectInvoiceWhereForAPtrans($idSupp, $idCurr, $date){
        $getPeriod  = $this->getPeriod_map();
        $blnPeriod  = date('m',  strtotime($getPeriod));
        $thnPeriod  = date('Y',  strtotime($getPeriod));
        /*return $this->db->query("SELECT * FROM acc_tbl_trn_hutang WHERE kode_sup = '".$idSupp."' AND "
                . "currency_id = '".$idCurr."' AND tanggal <= '".$date."' AND hutang-bayar != 0 "
                . "AND acc_tbl_trn_hutang.nofaktur IN (SELECT acc_tbl_trn_hutang_bulanan.nofaktur "
                . "FROM acc_tbl_trn_hutang_bulanan WHERE hutang-bayar > 0 AND acc_tbl_trn_hutang_bulanan.periode_bulan = '".$blnPeriod."' "
                . "AND acc_tbl_trn_hutang_bulanan.periode_tahun = '".$thnPeriod."')");*/
        /*return $this->db->query("SELECT * FROM acc_tbl_trn_hutang WHERE kode_sup = '".$idSupp."' AND "
                . "currency_id = '".$idCurr."' AND tanggal <= '".$date."' AND hutang-bayar != 0 ");*/

        /*return $this->db->query("SELECT *, 1 AS in_draft FROM acc_tbl_trn_hutang  WHERE "
                . "nofaktur IN (SELECT fin_vw_trn_is_invoice_in_draft.NoInvoice FROM fin_vw_trn_is_invoice_in_draft) AND "
                . "kode_sup = '".$idSupp."' AND currency_id = '".$idCurr."' AND tanggal <= '".$date."' AND hutang-bayar != 0 "
                . "UNION "
                . "SELECT *, 0 AS in_draft FROM acc_tbl_trn_hutang WHERE "
                . "nofaktur NOT IN (SELECT fin_vw_trn_is_invoice_in_draft.NoInvoice FROM fin_vw_trn_is_invoice_in_draft) AND "
                . "kode_sup = '".$idSupp."' AND currency_id = '".$idCurr."' AND tanggal <= '".$date."' AND hutang-bayar != 0 ");*/
        ...
        return $this->db->query("SELECT *, '0' AS in_draft, "
                ...
                . " FROM zhl_acc_tbl_trn_hutang WHERE "
                . "kode_sup = '".$idSupp."' AND currency_id = '".$idCurr."' AND tanggal <= '".$date."' AND hutang-bayar != 0 ");
        /*return $this->db->query("SELECT *, '0' AS in_draft, "
                ...
                . "nofaktur NOT IN (SELECT fin_vw_trn_is_invoice_in_draft.NoInvoice FROM fin_vw_trn_is_invoice_in_draft) AND "
                . "kode_sup = '".$idSupp."' AND currency_id = '".$idCurr."' AND tanggal <= '".$date."' AND hutang-bayar != 0 ");*/
        /*return $this->db->query("SELECT *, '1' AS in_draft, "
                . "(SELECT acc_tbl_trn_hutang_bulanan.rate_akhir FROM acc_tbl_trn_hutang_bulanan "
                . "WHERE acc_tbl_trn_hutang_bulanan.nofaktur = acc_tbl_trn_hutang.nofaktur "
                . "ORDER BY acc_tbl_trn_hutang_bulanan.created_date DESC LIMIT 1) AS rate_akhir_bulanan "
                . "FROM acc_tbl_trn_hutang WHERE "
    ...
    function resetPaymentHutang($noInvoice){
        $this->db->query("UPDATE zhl_acc_tbl_trn_hutang SET bayar = (SELECT SUM(Total) FROM zhl_acc_tbl_trn_appaymentdtl "
                . "WHERE zhl_acc_tbl_trn_appaymentdtl.NoInvoice = '".$noInvoice."') WHERE zhl_acc_tbl_trn_hutang.nofaktur = '".$noInvoice."'");
    }
    function resetPaymentHutangSaldo($noInvoice){
        $this->db->query("UPDATE zhl_acc_tbl_trn_hutang SET saldo_hutang = hutang-bayar WHERE "
                . "zhl_acc_tbl_trn_hutang.nofaktur = '".$noInvoice."'");
    }
    ...
    function updateHeaderAPpayment($data, $headerID){
        $this->db->where('header_id', $headerID);
    ...
    function updateDetailPOinPurchaseByInvoice($noInvoice){
        $this->db->query("UPDATE zhl_pur_tbl_trn_inv_dtl SET proses = '1' WHERE mainpo IN (SELECT no_po FROM zhl_acc_tbl_trn_payable_recognition "
