...
class M_packing_do extends CI_Model
{
    ...
    {
        $this->db->query("update zhl_pur_tbl_trn_lr_hdr set status=1 where lrno='$lr_no'");

        $this->db->where('lr_no', $lr_no);
        $this->db->delete('zhl_pur_tbl_trn_lr_dtl');
    }
    ...
    function get_mon_lr_filter($year)
    {
        $this->db->where('year(docdate)', $year);
        $result = $this->db->get('zhl_pur_vw_trn_lr');
        return $result->result();
    }
}
