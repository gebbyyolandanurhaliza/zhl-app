...
class M_Sales_inv2 extends CI_Model
{
    ...
    function getAjaxTanggaltet($id, $bargedest){
        ...
        else
        {
            ...
            // else if($id == 'fre') { $sql = " AND jurnal_freight_sales IS NULL ";} 
            // else if($id == 'trn') { $sql = " AND jurnal_transport_sales IS NULL ";}else{$sql="";}
            $a = "SELECT DISTINCT shipmentdate FROM ship_vw_trn_cont Where tipe = 2 ".$sql." AND shipmentdate > '2020-01-01'
                    UNION
                  SELECT DISTINCT shipmentdate FROM ship_vw_container_local2 where shipmentdate > '2020-01-01' AND bargejurnal IS NULL AND stuffing = 'LLTP' ".$etda;
            // echo $a;
            $sql_prov = $this->db2->query("SELECT DISTINCT shipmentdate FROM ship_vw_trn_cont Where tipe = 2 ".$sql." AND shipmentdate > '2018-01-01'
                    UNION
                  SELECT DISTINCT shipmentdate FROM ship_vw_container_local2 where shipmentdate > '2020-01-01' AND bargejurnal IS NULL AND stuffing IN ('LLTP') ".$etda);
        }
        if ($sql_prov->num_rows() > 0) {
    ...
    function get_isidetailTET($tgl, $bargedest){
        $sql = " AND (jurnal_barge_sales IS NULL OR jurnal_barge_sales = '') "; 
        if($bargedest == 'idn' || $bargedest == 'idn2'){ 
            $sql = $sql." AND (stuffing IN ('ELTP')) ";
            $stf = '';
      ...
      function get_isidetailTET2($tgl, $bargedest){
        ...
        if($jenisinv == 'trucking20ft' || $jenisinv == 'trucking40ft'){
            ...
            if(!empty($resi)){
                ...
                { $sql = "cust_export_laden";}
                 ...
                 $query = "SELECT ".$sql." FROM zhl_shp_tblmst_bargecharges WHERE container_id = '28' order by validity desc LIMIT 1 ";
                if($jenisinv == 'ELTP'){
                ...
                { $sql = "cust_local_laden";}
                 $query = "SELECT ".$sql." FROM zhl_shp_tblmst_bargecharges WHERE container_id = '29' order by validity desc LIMIT 1 ";
                if($jenisinv == 'LLTP'){
                $res = $this->db->query($query)->row();
                if(!empty($res)){
                    return $res->$sql;
            ...
            $query = "SELECT ".$sql." FROM zhl_shp_tblmst_bargecharges WHERE container_id = '" .$idcon."' order by validity_till desc LIMIT 1 ";
