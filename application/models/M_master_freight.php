...
class M_master_freight extends CI_Model {
    ...
    function insert()
    {
        ...
        $query = $this->db->query('select * from zhl_shp_tblmst_freightcharges where container_id = "'.$cont.'" and port_id = "'.$port.'" and shipping_term_id = "'.$ship.'" and consignee = "'.$con.'" and validity_from between "'.$from.'" and "'.$till.'" and validity_till between "'.$from.'" and "'.$till.'" order by freight_charges_id');
    ...
    {
        $query = ('trading_term_id in (2,3,15,16,17)');
        // $this->db->where('trading_term_id', 15);
        $this->db->where($query);
        $this->db->order_by('trading_term_name', 'ASC');
        return $this->db->get('mar_tblmst_trading_term')->result();
    }

    function tampil_mst_freight(){

        $this->db->order_by('port_name', 'ASC');
        $result=$this->db->get('zhl_ship_vw_trn_freight_charges_hdr_dtl');
        // $result=$this->db->get('zhl_ship_vw_trn_freight_charges');
    ...
    function filter_freight($dest,$cont,$ship,$con){
        ...
        return $this->db->get('zhl_ship_vw_trn_freight_charges_hdr_dtl')->result();       
       // return $this->db->get('zhl_ship_vw_trn_freight_charges')->result();        
    }
}
