...
class M_Receivable_statement extends CI_Model {
    ...
    function call_data_aging($sup, $curreny, $tgl, $sampai) {
        $sql = $this->db->query("call zhl_sp_acc_rpt_receivable_aging('$sup', '$curreny', '$tgl', '$sampai')");
        $res = $sql->result();
        $sql->next_result();
        $sql->free_result();
        return $res;
    ...
    function get_mutation($sup, $curreny, $thn, $bulan) {
        ...
        $this->db->where('currency', $curreny);
    ...
    function get_total($sup, $curreny, $thn, $bulan) {
        $this->db->select('SUM(piutang*rate_awal) as piutang, SUM(bayar) as bayar, currency');
        $this->db->where('kode_sup', $sup);
        $this->db->where('currency', $curreny);
        $this->db->where('YEAR(tanggal)', $thn);
        $this->db->where('MONTH(tanggal)', $bulan);
    ...
    function get_data_supplier($sup){
        ...
        $this->db->where('customer_code', $sup);
         $sql_product = $this->db->get('zhl_mar_tblmst_customer');
