...
class M_CashBank extends CI_Model
{
  ...
  private $tblTran    = array(
    'CashBankHeader'    => 'zhl_fin_tbltrn_cashbank_journal_header',
    'CashBankDetail'    => 'zhl_fin_tbltrn_cashbank_journal_detail',

    'PaymentAPheader'   => 'zhl_fin_tbltrn_payment_hdr',
    'PaymentAPdetail'   => 'zhl_fin_tbltrn_payment_dtl'
  );
  ...
  function updateCashBankHistory($detailIDHistory, $data)
  {
    ...
    $this->db->update('zhl_fin_tbltrn_cashbank_journal_history', $data);
  }
  ...
  function getDetailHistory($headerID)
  {
    ...
    $get = $this->db->get('zhl_fin_tbltrn_cashbank_journal_history');
    return $get->result();
  }
  ...
  function getDetailJurnal($NoJurnal)
  {
    ...
    $get = $this->db->get('zhl_acc_tbl_trn_jurnal');
    return $get->result();
  }
  ...
  public function getDetailIdHstry($NoFacture) {
    ...
    $query = $this->db->get('zhl_fin_tbltrn_cashbank_journal_history');
  ...
  {
    $get = $this->db->query("SELECT NoInvoice FROM zhl_acc_tbl_trn_appaymentdtl WHERE NomorAP = '" . $apNumber . "'");
    return $get->result_array();
  }
  function getTotalFromAPAcc($noInvoice, $idVoucher)
  {
    $get = $this->db->query("SELECT Total FROM zhl_acc_tbl_trn_appaymentdtl WHERE NoInvoice = '" . $noInvoice . "' AND NomorAP = '" . $idVoucher . "'");
    $row = $get->row();
    return $row->Total;
  }
  function getHutangFromUtang($noInvoice, $jenis)
  {
    $get = $this->db->query("SELECT nofaktur, saldo_hutang FROM zhl_acc_tbl_trn_hutang WHERE nofaktur = '" . $noInvoice . "' AND jenis_trans ='" . $jenis . "' ");
    $row = $get->row();
    return $row->saldo_hutang;
  }
  function getBayarFromUtang($noInvoice, $jenis)
  {
    $get = $this->db->query("SELECT nofaktur, bayar FROM zhl_acc_tbl_trn_hutang WHERE nofaktur = '" . $noInvoice . "' AND jenis_trans ='" . $jenis . "'");
    $row = $get->row();
    return $row->bayar;
  }
  function getHutangFromUtangBulanan($noInvoice, $jenis, $blnPeriod, $thnPeriod)
  {
    $get = $this->db->query("SELECT nofaktur, saldo_hutang FROM zhl_acc_tbl_trn_hutang_bulanan WHERE nofaktur = '" . $noInvoice . "' AND jenis_trans ='" . $jenis . "' AND "
  ...
  {
    $get = $this->db->query("SELECT nofaktur, bayar FROM zhl_acc_tbl_trn_hutang_bulanan WHERE nofaktur = '" . $noInvoice . "' AND jenis_trans ='" . $jenis . "' AND "
      . "periode_bulan = '" . $blnPeriod . "' AND periode_tahun = '" . $thnPeriod . "'");
    $row = $get->row();
    return $row->bayar;
  }
  ...
  function getAPheaderByHeaderID($hdrID)
  {
    ...
    $get = $this->db->get('zhl_fin_tbltrn_payment_hdr');
    return $get->row();
  }
  function getAPdetailByHeaderID($hdrID)
  {
    $this->db->where('header_id', $hdrID);
    $get = $this->db->get('zhl_fin_tbltrn_payment_dtl');
    return $get->result();
  }
  // ==== Insert (Do Action)
  ...
  function updateRealisasiInAPdetail($idVoucher, $noInvoice)
  {
    ...
    $this->db->update('zhl_acc_tbl_trn_appaymentdtl', array('realisasi_by' => $this->session->userdata('userid_1'), 'realisasi_date' => date('Y-m-d H:i:s')));
  }
  // == Insert Into Jurnal
  ...
  {
    $get = $this->db->query("SELECT NoInvoice FROM zhl_acc_tbl_trn_arpaymentdtl WHERE NomorAR = '" . $arNumber . "'");
    return $get->result_array();
  }
  function getTotalFromARAcc($noInvoice, $idVoucher)
  {
    $get = $this->db->query("SELECT Total FROM zhl_acc_tbl_trn_arpaymentdtl WHERE NoInvoice = '" . $noInvoice . "' AND NomorAR = '" . $idVoucher . "'");
    $row = $get->row();
    return $row->Total;
  }
  function getBayarFromPiutang($noInvoice, $jenis)
  {
    $get = $this->db->query("SELECT nofaktur, bayar FROM zhl_acc_tbl_trn_piutang WHERE nofaktur = '" . $noInvoice . "' AND jenis_trans = '" . $jenis . "'");
    $row = $get->row();
    return $row->bayar;
  }
  function getBayarFromPiutangBulanan($noInvoice, $jenis, $blnPeriod, $thnPeriod)
  {
    $get = $this->db->query("SELECT nofaktur, bayar FROM zhl_acc_tbl_trn_piutang_bulanan WHERE nofaktur = '" . $noInvoice . "' AND "
  ...
  {
    $this->db->query("UPDATE zhl_acc_tbl_trn_piutang_bulanan SET bayar = bayar + '" . $payment . "' WHERE nofaktur = '" . $noInvoice . "' AND "
  ...
  function updateRealisasiInARdetail($idVoucher, $noInvoice)
  {
    $this->db->where('NomorAP', $idVoucher);
    $this->db->where('NoInvoice', $noInvoice);
    $this->db->update('zhl_acc_tbl_trn_arpaymentdtl', array('realisasi_by' => $this->session->userdata('userid_1'), 'realisasi_date' => date('Y-m-d H:i:s')));
  }
