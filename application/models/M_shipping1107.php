...
//    ----------------------------------------------------------------------------------------------------------------------------------
    ...
    function simpan_cont_sp($datahdr){
        $this->db->trans_begin();
            ...
            $query2=$this->simpan_cont_sp_dtl($contid,$id,$shipid,$destination,$reff,$vessel,$convessel,$depot,$pod,$opcode,$etd,$eta,$container,$seal,$weight,$outward,$jml,$carrier,$urut,$stuffing,$actual_seal);
            $flag2=$query2->flag;
    ...
    function simpan_cont_sp_dtl($contid,$id,$shipid,$destination,$reff,$vessel,$convessel,$depot,$pod,$opcode,$etd,$eta,$container, $seal,$weight,$outward,$jml,$carrier,$urut, $stuffing, $actual_seal){
        for($i = 0;$i < $jml;$i++){
            $this->db->query("SET @flag = 0");
...
//-----------------JANGAN GANGGU KODING DIATAS------------------------//
    ...
    function container_stock_noted($contstock){
        //$this->db->query("UPDATE zhl_ship_tbl_trn_cont_stock_dtl set status = '1' where container_number = '".$contstock."'");
        
        $this->db->query("UPDATE zhl_ship_tbl_trn_cont_stock_dtl set status_note = '1' where container_number ='".$contstock."'");
    }
