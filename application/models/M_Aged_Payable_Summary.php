...
class M_Aged_Payable_Summary extends CI_Model {
    ...
    function get_group_supp($dari, $sup, $currency) {
        $sql = $this->db->query("SELECT a.kode_sup,b.suppliercompany FROM zhl_acc_tbl_trn_hutang as a JOIN zhl_pur_tbl_mst_supplier as b on a.kode_sup = b.supplierid LEFT JOIN
    ...
    function call_data($sup, $currency, $dari) {
        $sql = $this->db->query("call zhl_sp_acc_rpt_aged_payable_summary('$sup', '$currency', '$dari')");
