          ...
          <div class="portlet light">
            <div class="portlet-title">
              <div class="caption theme-font">
                ...
                <span class="caption-helper"><?php echo date('F Y', strtotime($_periode = '2016-03-01')); ?></span>
              </div>
              <div class="actions">
                <a class="btn btn-circle btn-icon-only btn-default fullscreen" href="javascript:;" data-original-title="" title="">
                </a>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $("#txtJurNum").keyup(function() {
        $.ajax({
          type: "POST",
          url: "<?php echo base_url(); ?>Transaction_CashBank/getJournalTransactionAR",
          data: {
          ...
          success: function(n) {
            n.length > 0 ? ($("#ddJurNum").empty(), $("#txtJurNum").attr("data-toggle", "dropdown"), $("#ddJurNum").dropdown("toggle")) : 0 == n.length && $("#txtJurNum").attr("data-toggle", ""), $.each(n, function(e, d) {
              n.length >= 0 && $("#ddJurNum").append('<li role="presentation" ><a role="menuitem dropdownnameli" class="dropdownlivalue">' + d.NomorAR + "</a></li>");
            });
          }
        });
      }),
      $("ul.txtJurNum").on("click", "li a", function() {
        ...
        $("#txtJurNum").val(idJur);
        $.ajax({
          url: "<?php echo site_url('Transaction_CashBank/ajaxGetJournalTransactionAR'); ?>",
          type: "POST",
          data: "noAP=" + idJur,
          datatype: "json",
          cache: false,
        ...
        $.ajax({
          url: "<?php echo site_url('Transaction_CashBank/ajaxSetTableTransactionAR'); ?>",
          type: "POST",
          data: "noAP=" + idJur,
          datatype: "json",
          cache: false,
