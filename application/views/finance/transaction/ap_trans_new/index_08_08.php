                  ...
                  <div class="col-sm-4">
                    ...
                    <script>
                      $('#inputNoReff').on('blur', function() {
                        ...
                        $.ajax({
                          type: "POST",
                          url: "<?php echo base_url(); ?>APtransNew/cekNumReffAP",
                          data: {
                            value: val
                          },
                          dataType: "json",
              ...
              <table class="table-ismo" id="tbl-cashGeneral">
                ...
                <tfoot id="detailRowForAddCost">
                  ...
                  <tr id="addCostRow1">
                    ...
                    <td nowrap>
                    <input type="text" name="txtDeptCode[]" id="txtInputDeptCodeRow-addCostRow-1" class="txt" value="001" readonly />
                    </td>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $("#btn-generate-noreff").click(function() {
      $.ajax({
        url: "<?php echo site_url('APtransNew/generateReffNumAP'); ?>",
        dataType: 'json',
    ...
    $("#txtInputSuplierID, #txtInputSuplierName").click(function() {
      ...
      if (!txtTglTrn) {
        ...
        $.ajax({
          url: "<?php echo site_url('APtransNew/selectSupplierForAP'); ?>",
          data: {
    ...
    $("#btnSelectInvoice").click(function() {
      if (inSuppID === '') {
        ...
        if ($('#modal-invoice').hasClass('has-ismo-modal')) {
          ...
          var a = "<?php echo site_url('APtransNew/selectInvoiceForAP'); ?>";
...
<script>
  ...
  function Pilih_Invoice(x) {
    ...
    var table = document.getElementById('tbl-selectInvoice');
...
for (var rowIndex = 1; rowIndex < table.rows.length; rowIndex++) { 
  ...
  function calculateInvoiceDetail() {
    ...
    var exRateNew = Number(totalAP.toFixed(2)) - Number(grandExcUsd.toFixed(2));

    if(grandExcUsd < 0){
      // Ini cari exchange lama tetapi tidak muncul di kolom sgd nya - old
      if (totalAP < 0) {
      ...
      if (totalAP < 0) {
        ...
        $('#txtInputCreditCBdetailRow-1').val("0.00");
        $('#txtInputDebitRow-2').val("0.00");
        $('#txtInputCreditRow-2').val(addCommas(Number(Math.abs(exRateNew)).toFixed(2)));
      } else {
        $('#txtInputDebitCBdetailRow-1').val("0.00");
        $('#txtInputCreditCBdetailRow-1').val(addCommas(Number(grandPay).toFixed(2)));
        $('#txtInputDebitRow-2').val(addCommas(Number(exRateNew).toFixed(2)));
        $('#txtInputCreditRow-2').val("0.00");
      }
    }

    // var valExR = Number(exRate).toFixed(2);
    var valExR = Number(grandExcUsd).toFixed(2);
    if (exRate >= 0) {
      $('#txtInputDebitRow-3').val(addCommas(Number(grandExcUsd).toFixed(2)));
