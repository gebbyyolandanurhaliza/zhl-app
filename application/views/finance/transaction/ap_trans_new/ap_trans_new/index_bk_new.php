...
<div class="page-content">
  <div class="container-fluid">
    <div class="row">
      ...
      <form role="form" method="post" id="form-APtrans" action="<?php echo site_url('APtransNew') . $_actionFrom; ?>" class="form-horizontal">
        <div class="col-md-12">
                ...
                <div class="col-md-12">
                  ...
                  <div class="col-sm-4">
                    <div class="form-group">
                      ...
                      <div class="col-sm-8">
                        <input type="text" id="txtInputRateBank" name="txtRateBank" class="form-control input-sm txtnumRate back-null-inv" required />
                      </div>
                    </div>
                  </div>
                  ...
                  <table class="table-ismo" id="tbl-detail-invoice">
                    <thead>
                      <tr>
                        ...
                        <th colspan="3">[<span class="txtHeadCurrency"></span>] Payment</th>
              ...
              <table class="table-ismo" id="tbl-cashGeneral">
                ...
                <tbody>
                  <tr id="rowSetCOA">
                    ...
                    <td nowrap><input type="text" name="txtDebit[]" id="txtInputDebitCBdetailRow-1" onKeyup="checkGST(); convertToUSD();" onblur="checkGST(); convertToUSD();" class="col-debit col-deb txt txtnum" readonly value="0.00" /></td>
                  ...
                  <tr id="secendRowCashGeneral">
                    ...
                    <td nowrap><input id="txtInputCreditRow-2" type="text" name="txtCredit[]" onKeyup=" checkGST(); convertToUSD();" onblur="checkGST(); convertToUSD();" class="col-credit col-cre txt txtnum" readonly value="0.00" /></td>
  ...
  $(document).ready(function() {
    ...
    $('#chkSaveDraf').on('click', function() {
      if ($(this).prop('checked')) {
        ...
        $("#form-APtrans").attr('action', '<?php echo site_url(); ?>APtransNew/insertAPpaymentToDraf');
      } else {
        //alert('Not Save Draf');
        $('#txtInputCheckBank').prop("readonly", false);
        $('#txtInputCheckBank').prop("required", true);
        $("#form-APtrans").attr('action', '<?php echo site_url(); ?>APtransNew/insertAPpayment');
      }
    });
    ...
    $("#inputCOA").click(function() {
      if ($('#inputTotalVoucher').val() === '') {
        ...
        $.ajax({
          url: "<?php echo site_url('APtransNew/selectCOA'); ?>",
          type: "POST",
          datatype: "json",
          cache: false,
          success: function(respon) {
    ...
    $("#btnSelectInvoice").click(function() {
      if (inSuppID === '') {
        ...
        if ($('#modal-invoice').hasClass('has-ismo-modal')) {
          ...
          $.ajax({
            url: "<?php echo site_url('APtransNew/selectInvoiceForAP'); ?>",
            type: "POST",
            data: {
...
<script>
  ...
  function Pilih_Invoice(x) {
    ...
    var noInv = getText(document.getElementById('tbl-selectInvoice').rows[rr].cells[0]);
    var currInv = getText(document.getElementById('tbl-selectInvoice').rows[rr].cells[1]).replace(/,/g, '');
    var rateInv = getText(document.getElementById('tbl-selectInvoice').rows[rr].cells[2]).replace(/,/g, '');
    var totBef = getText(document.getElementById('tbl-selectInvoice').rows[rr].cells[4]).replace(/,/g, '');
    var cls = getText(document.getElementById('tbl-selectInvoice').rows[rr].cells[6]);
    ...
    $('#tbl-detail-invoice tbody').append('<tr class="' + cls + ' added-row-ismo">\n\
            ...
            <td><input value="' + Number(ratePay).toFixed(6) + '" name="txtNewRateToPayDtl[]" class="txt txtnum" readonly/></td>\n\
  ...
  function calculateInvoiceDetail() {
    ...
    //##== for Journal
    var exRate = Number(grandBank.toFixed(2)) - Number(grandAP.toFixed(2));
    $('#txtInputCreditCBdetailRow-1').val(addCommas(Number(grandBank / valRateUSD).toFixed(2)));
    $('#txtInputDebitRow-2').val(addCommas(Number(grandAP / valRateUSD).toFixed(2)));

    var valExR = Number(exRate / valRateUSD).toFixed(2);
    if (exRate >= 0) {
      $('#txtInputDebitRow-3').val(addCommas(Number(exRate / valRateUSD).toFixed(2)));
  ...
  function Pilih_MCOA(x) {
    ...
    if (valCUR != 'ALL') {
      $.post("<?php echo site_url('APtransNew/getCurrBank'); ?>", {
        txtCurrency: valCUR,
