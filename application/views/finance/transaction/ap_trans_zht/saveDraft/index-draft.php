...
<div class="page-content">
  <div class="container-fluid">
    <div class="row">
      ...
      <form role="form" method="post" id="form-APtrans" action="<?php echo site_url('APtrans_zht/payAPfromDraft'); ?>" class="form-horizontal">
        <div class="col-md-12">
                ...
                <div class="col-md-12">
                  ...
                  <div class="col-sm-4">
                    <div class="form-group">
                      ...
                      <div class="col-sm-8">
                        <input id="txtInputTransDate" name="txtTransDate" type="text" class="form-control input-sm" value="<?php echo date('d-m-Y', strtotime($_selectHeaderAP->trans_date)); ?>" data-date-format="dd-mm-yyyy" readonly />
                      </div>
                    </div>
                    <div class="form-group ismo-hidden">
                ...
                <div class="col-md-12">
                  <div class="col-sm-4">
                    <div class="form-group">
                      ...
                      <div class="col-sm-8">
                        <input type="text" name="txtTotalVoucher" value="<?php echo $_selectHeaderAP->amount; ?>" id="inputTotalVoucher" class="form-control input-sm txt-ismo-back-null txtnum" readonly="" />
                      </div>
                    </div>
                  </div>
                  <div class="col-sm-4">
                    <div class="form-group">
                      ...
                      <div class="col-sm-8">
                        <input type="text" id="txtInputRateSGD" value="<?php echo number_format($_selectHeaderAP->currency_rate * $_selectHeaderAP->amount, 6, '.', ''); ?>" name="txtRateSGD" class="form-control input-sm txt-ismo-back-null txtnum" readonly="" />
                      </div>
                    </div>
                  </div>
                </div>
                    ...
                    <tbody>
                      <?php foreach ($_selectInvoiceAP as $inv) : ?>
                        <tr class="added-row-ismo">
                          <td class="td-btn-delete-inv text-center"></td>
                          <td><input value="<?php echo $inv->NoInvoice; ?>" name="txtNoInvoiceDtl[]" class="txt" readonly /></td>
                          <td><input value="<?php echo number_format($inv->curr_bayar, 6); ?>" name="txtRateToCurr[]" class="txt col-rate-curr txtnum" readonly /></td>
                          <td>
                            <input value="<?php echo number_format(str_replace(',', '', number_format($inv->rate_akhir * $inv->hutang, 2)) / str_replace(',', '', number_format($inv->curr_bayar, 6)), 2); ?>" name="txtEquiToCurr[]" class="txt col-equi-curr txtnum" readonly />
                          </td>
          ...
          <div class="portlet light">
            <div class="portlet-title">
              <div class="caption theme-font">
                ...
                <input type="hidden" name="txtAPtransPeriod" value="<?php echo date('d-m-Y', strtotime($_periode)); ?>">
              </div>
              <div class="actions">
                <a class="btn btn-circle btn-icon-only btn-default fullscreen" href="javascript:;" data-original-title="" title="">
                </a>
              ...
              <div class="row">
                    ...
                    <tbody id="ajax-tblAP">
                      ...
                      foreach ($_selectDetailAP as $dtl) : ?>
                        <tr>
                          ...
                          <td nowrap><input value="<?php echo number_format($dtl->amount, 2); ?>" name="txtTotalDetal[]" id="inputAmountRow<?php echo $n1++; ?>" class="txt txtnum txt-ismo-back-null" readonly /></td>
                          <td nowrap><input value="<?php echo number_format($dtl->cur_equi, 2); ?>" name="txtToCurr[]" id="inputToCurr<?php echo $n2++; ?>" class="txt txtnum txt-ismo-back-null" readonly /></td>
                          <td nowrap><input value="<?php echo number_format($dtl->usd_equi, 2); ?>" name="txtEquiDetail[]" id="inputUSDRow<?php echo $n3++; ?>" class="txt txtnum txt-ismo-back-null" readonly /></td>
                          <td nowrap><input value="USD" name="txtCurrDetail[]" id="inCurFirstRow" class="txt txt-ismo-back-null" readonly /></td>
                          <td nowrap><input value="<?php echo $dtl->no_coa; ?>" name="txtCOADetail[]" id="txtCOADetailRow<?php echo $n4++; ?>" class="txt txt-ismo-back-null" readonly /></td>
                ...
                <div class="col-md-12" style="padding-top: 10px;">
                  ...
                  <div class="col-sm-offset-8 col-sm-2 text-right">
                    <div class="form-group">
                      <button class="btn btn-sm btn-success" id="btnSumbutPayAP" name="btnSubmit" type="submit">Submit</button>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    $("#btnCancel").click(function() {
      window.location = "<?php echo site_url('APtrans'); ?>";
    });
    $("#form-APtrans").submit(function(e) {
      var currentForm = this;
      e.preventDefault();
      bootbox.confirm("Are you really want to save this transaction?", function(result) {
        if (result) {
          currentForm.submit();
        }
      });
    ...
    // ================== Start Delete Row Invoice ======================================================
    $('#tbl-detail-invoice tbody tr')
      .find('.td-btn-delete-inv')
      .append('<button class="btn btn-xs btn-link buruk btn-del-inv" type="button">\n\
                <i class="fa fa-trash-o"></i></button></td>');
    $('.btn-del-inv').on('click', function(e) {
      e.preventDefault();
    ...
    $('#txtInputCheckBank').attr('readonly', false);
    ...
    $.post("<?php echo site_url('APtrans_zht/selectSuppliertForAPbySuppCode'); ?>", {
    ...
    $.post("<?php echo site_url('APtrans_zht/selectCurrencyAP'); ?>", {
    ...
    $('#btnPrint').click(function() {
      ...
      //alert(noAP);
      window.open('<?php echo site_url(); ?>APtrans_zht/paymentAdvice/' + noAP + '?sup=' + suppNm + '&adr=' + suppAd + '&adr2=' + suppDr, '_blank');
    });
  });
</script>
...
<script>
  function CountGrandTotal() {
    ...
    var avgRate = parseFloat('<?php echo $_selectHeaderAP->currency_rate; ?>');
    ...
    var sumCur = 0;
    $(".col-equi-curr").each(function() {
      var valCurr = this.value;
      var newCurr = parseFloat(valCurr.replace(/,/g, ''));
      if (!isNaN(newCurr) && this.value.length !== 0) {
        sumCur += parseFloat(newCurr);
      }
    });
    var avgRateCurr = sumDeb / sumCur;
    $('#rateToCurr').val(addCommas(avgRateCurr.toFixed(6)));
    $('#equiToCurr').val(addCommas(sumCur.toFixed(2)));
  }

  function calculateForJournal() {
