...
//		$this->Ln(5);
		...
		$this->Image('assets/pss-header.png', 13, 12, 120, 18, 'PNG');
...
//		$this->SetTextColor(0, 51, 153);
	...
	function Content($rec_hdr, $rec_dtl)
	{
		...
		$this->Cell(275,4, 'Facsimile Transmission', 0, 1, 'C');
		
		$this->Ln(5);
		
		$this->SetFont('arial', '', 9);
		$arr_w = array(10, 20, 2, 140, 2, 20, 2, 89, 1);
		$arr_x = array(
			0	=> $arr_w[0],
			1	=> $arr_w[0] + $arr_w[1],
		...
		$this->MultiCell($arr_w[4], $h, '');
		$this->SetXY($arr_x[4], $y);
		$this->MultiCell($arr_w[5], $h, 'Our Ref No');
		$this->SetXY($arr_x[5], $y);
		$this->MultiCell($arr_w[6], $h, ':');
		$this->SetXY($arr_x[6], $y);
		$this->MultiCell($arr_w[7], $h, $rec_hdr->ref_no);
		
		$y = $this->GetY();
		$this->SetXY($arr_x[0], $y);
		$this->MultiCell($arr_w[1], $h, 'C.C');
		$this->SetXY($arr_x[1], $y);
		$this->MultiCell($arr_w[2], $h, ':');
		$this->SetXY($arr_x[2], $y);
		$this->MultiCell($arr_w[3], $h, $rec_hdr->cc1);
		$this->SetXY($arr_x[3], $y);
		$this->MultiCell($arr_w[4], $h, '');
		$this->SetXY($arr_x[4], $y);
		$this->MultiCell($arr_w[5], $h, 'Tel');
		$this->SetXY($arr_x[5], $y);
		$this->MultiCell($arr_w[6], $h, ':');
		$this->SetXY($arr_x[6], $y);
		$this->MultiCell($arr_w[7], $h, $rec_hdr->tel);
		
		$y = $this->GetY();
		$this->SetXY($arr_x[0], $y);
		$this->MultiCell($arr_w[1], $h, 'From');
		$this->SetXY($arr_x[1], $y);
		$this->MultiCell($arr_w[2], $h, ':');
		$this->SetXY($arr_x[2], $y);
		$this->MultiCell($arr_w[3], $h, $rec_hdr->from);
		$this->SetXY($arr_x[3], $y);
		$this->MultiCell($arr_w[4], $h, '');
		$this->SetXY($arr_x[4], $y);
		$this->MultiCell($arr_w[5], $h, 'Fax');
		$this->SetXY($arr_x[5], $y);
		$this->MultiCell($arr_w[6], $h, ':');
		$this->SetXY($arr_x[6], $y);
		$this->MultiCell($arr_w[7], $h, $rec_hdr->fax);
		...
		$this->MultiCell($arr_w[2], $h, ':');
		$this->SetXY($arr_x[2], $y);
		$this->MultiCell($arr_w[3], $h, $rec_hdr->cc2);
		$this->SetXY($arr_x[3], $y);
		$this->MultiCell($arr_w[4], $h, '');
		$this->SetXY($arr_x[4], $y);
		$this->MultiCell($arr_w[5], $h, '');
		$this->SetXY($arr_x[5], $y);
		$this->MultiCell($arr_w[6], $h, '');
		$this->SetXY($arr_x[6], $y);
		$this->MultiCell($arr_w[7], $h, '');
		...
		$this->Cell(100, $h, 'RE : SHIPPING ADVICE', 0, 1);
		$this->SetFont('arial', '', $font_size);
		$this->Cell(100, $h, 'CONFIRMED SHIPMENT', 0, 1);
		
		$this->Ln(2);
		
		$dw = array(20, 25, 30, 15, 15, 25, 59, 7, 7, 7, 35, 30);
		$da = array('C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C', 'C');
		
		$head_title	= array(
			'P/O No.', 'Buyer Ref', 'Vessel / Voyage', 'ETD', 'ETA', 'Port', 'Product', "20'", "40'",'CT', 'Container No.', 'Seal No.',
		);
		
		$this->SetFont('arial', 'B', $font_size);
		for($i=0; $i < count($head_title); $i++):
            $this->Cell($dw[$i],5,$head_title[$i], 'BLT', 0, $da[$i]);
        endfor;
		$this->Cell(1,5,'','L',1);	// garis kanan
		$this->SetFont('arial', '', $font_size);
		...
		foreach ($rec_dtl as $r) {
			...
			$this->Row(
				array(
					...
					$r->buyer_si,
					$r->vessel,
					str_replace('-','/',$r->etdsin),
					str_replace('-','/',$r->etasin),
