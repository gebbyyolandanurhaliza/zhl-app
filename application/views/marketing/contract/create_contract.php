...
<div class="page-content">
	<div class="container-fluid">
		<div class="row ">
			...
			<form action="<?php echo $action; ?>" method="post" class="form-horizontal" id="form_sales_contract" onsubmit="return validate(this);">
				<div class="col-md-12">
						...
						<div class="portlet-body form">
							<div class="form-body row">
								<div class="col-md-12">
									...
									<?php
									echo form_hidden('act', $act);
									echo "<input id='act' value='$act' name='act' type='hidden'>";
									echo "<input id='alreadypo' name='alreadypo' value='$po_already' type='hidden'>";
									echo form_hidden('quotation_hdr_id', $quotation_hdr_id);
									echo "<input id='contract_hdr_id' value='$contract_hdr_id' name='contract_hdr_id' type='hidden'>";
									echo "<input id='allow_auto_product' value='$allow_auto_product' type='hidden' name='allow_auto_product'>";

									?>

									<div class="form-group">
										<div class="col-md-6" style="padding: 0px">
											<label class="col-md-4 control-label" for="varchar">Sales Contract No</label>
											<div class="col-md-4">
												<input type="text" class="form-control uppercase" name="sales_contract_no" id="sales_contract_no" value="<?php echo $sales_contract_no?>" placeholder="Auto Generate Once Saved"/>
												<?php
												if ($act == 'add'){ ?>
												...
												<?php } else { ?>
													<!--<input type="text" class="form-control" name="sales_contract_no" id="sales_contract_no" value="<?php // echo $sales_contract_no?>" placeholder="Auto Generate Once Saved"/>-->
												<?php } ?>
											</div>

											<label class="col-md-2 control-label" for="varchar" style="padding-right:0;">Contract Date</label>
											<div class="col-md-1">
												<input required type="text" class="form-control date date-picker" style="width:100px;" data-date="<?php echo $current_date ?>" data-date-format="dd/mm/yyyy" name="contract_date" id="contract_date" value="<?php echo $contract_date; ?>" title="date format : dd/mm/yyyy" />
											</div>
										</div>
										...
										<?php
											if($act == 'edit') {
										?>
											<label class="col-md-2 control-label" style="padding-right:5px; color:#FF0000;" for="rev">Revision<input type="checkbox" id='rev' class="form-control" name="rev_check" <?php echo ($rev_check != 1 ? '' : 'checked') ?>/></label>
											<div class="col-md-2">
												<?php
													$extra_destination = 'id="rev_value" class="form-control" style="margin-left:-35px;"';
													$option_destination['0'] = '00';
													$i_max=10;
													if($rev_value > $i_max){
														$i_max=$rev_value;
													}
													for($i = 1;$i <= $i_max;$i++)
													{
														$option_destination[$i]=substr('0'.$i,-2);
													}
													echo form_dropdown('rev_value', $option_destination, $rev_value, $extra_destination);
												?>
											</div>
										<?php } ?>
											...
											<label class="col-md-3 control-label pull-right" style="text-align: right;" for="varchar">Sales Quotation No</label>
										</div>
									</div>
									...
									<div class="form-group required">
										...
										<div class="col-md-4">
											...
											<input type="hidden" id="current_customer_id" name="current_customer_id" value="<?php echo $customer_id ?>">
										</div>
									...
									<script type="text/javascript">
										$('#customer_list').change(function(){
											...
											var con_hdr_id	= {con_hdr_id:$('#contract_hdr_id').val()};
											var aksi		= $('#act').val();
											var allow_auto_product = $('#allow_auto_product').val();

											var cust_list	= $("#customer_list").val();
											var cust_id		= $("#current_customer_id").val();


											$.ajax({
												type: "POST",
												url : "<?php echo site_url('sales_contract/get_payterm_by_customer')?>",
//												url : "<?php // echo site_url('marketing_misc/get_payterm_by_customer')?>",
											...
											$.ajax({
												type: "POST",
												url : "<?php echo site_url('sales_contract/get_bank_by_customer')?>",
												data: customer_id,
												success: function(msg){
													$('#bank_container').html(msg);
												}
											});

											$.ajax({
												type: "POST",
												url : "<?php echo site_url('sales_contract/get_selected_document_by_customer')?>",
												data: customer_id,
												success: function(msg){
											...
											if (aksi == 'add'){
												if (allow_auto_product == 1){
													$.ajax({
														type: "POST",
														url: "<?php echo site_url('sales_contract/get_product_purchase') ?>",
														data: customer_id,
														success: function (msg) {
															$('#tblcontract_container').html(msg);
														}
													});
												}
											} else {
												if (cust_id == cust_list){
													$.ajax({
														type: "POST",
														url: "<?php echo site_url('sales_contract/get_product_purchase_on_edit') ?>",
														data: con_hdr_id,
														success: function (msg) {
															$('#tblcontract_container').html(msg);
														}
										...
										<div id="div_rate">
											<div class="col-sm-3">
												<div id="div_rate_usd" class="input-group">
													...
													<input required type="text" rate-set='0' class="form-control text-right input-rate" name="rate_usd" id="rate_usd" placeholder="0.000000" value="<?php echo $rate_usd; ?>" title="6 digits decimal" />
												</div>
											</div>
											<div class="col-sm-3">
									...
									<div class="agent-scroll">
										<table width="100%" id="tbl_agent" class="table table-detail">
											<thead>
												<tr>
													<!--<th></th>-->
													<th width="35%" class="text-left">Agent Name</th>
													<th width="20%" class="text-left">Agent Reference</th>
													<th width="13%" class="text-center">USD Com (%)</th>
													<th width="13%" class="text-center">USD Com/Unit</th>
													<th width="10%" class="text-center">S. Contract</th>
													...
													<tr>
														...
														<td class="bg-editable">
															<?php
																...
																echo form_dropdown("agent_id[$agc]", $option_agent, $ag->agent_id, $extra_agent);
															?>
														</td>
														<td class="bg-editable">
															<?php
															$agent_reference =(isset($ag->agent_reference))?$ag->agent_reference:'';
															?>
															<input type="text" value="<?php echo $agent_reference ?>" name="agent_reference[<?php echo $agc ?>]" class="form-control">
														</td>
														<td class="bg-editable">
															<input type="text" value="<?php echo $ag->com_percent ?>" onkeyup="agent_percent()" class="form-control autonum_com_percent text-right autofocus agent_com_percent" name="agent_com_percent[<?php echo $agc ?>]" id="agent_com_percent" data-a-sign=" %" data-p-sign="s" data-v-max="100" placeholder="0.00 %" />
														</td>
														<td class="bg-editable">
															<input type="text" value="<?php echo $ag->com_unit ?>" onkeyup="agent_unit()" class="form-control autonum_com_unit text-right autofocus agent_com_unit" name="agent_com_unit[<?php echo $agc ?>]" id="agent_com_unit" data-p-sign="s" placeholder="0.00"/>
														</td>
														<td class="bg-editable text-center">
															<?php
															$checked_c = ($ag->show_contract == 1 ? true : false);
															echo form_checkbox("show_contract[$agc]", "1", $checked_c, "id='show_contract-$agc'");
															?>
														</td>
														<td class="bg-editable text-center">
															<?php
															$checked = ($ag->invoice == 1 ? true : false);
															echo form_checkbox("agent_invoice[$agc]", "1", $checked, "id='agent_invoice-$agc'");
															?>
														</td>
														...
														<tr>
															...
															<td class="bg-editable">
																<?php
																	...
																	echo form_dropdown("agent_id[$agc]", $option_agent, '', $extra_agent);
																?>
															</td>
															<td class="bg-editable">
																<input type="text" value="" name="agent_reference[<?php echo $agc ?>]" class="form-control">
															</td>
															<td class="bg-editable">
																<input type="text" class="form-control autonum_com_percent text-right"  onkeyup="agent_percent()" name="agent_com_percent[<?php echo $agc ?>]" id="agent_com_percent" data-a-sign=" %" data-p-sign="s" data-v-max="100" placeholder="0.00 %" value=""/>
															</td>
															<td class="bg-editable">
																<input type="text" class="form-control autonum_com_unit text-right"  onkeyup="agent_unit()" name="agent_com_unit[<?php echo $agc ?>]" id="agent_com_unit" data-p-sign="s" placeholder="0.00" value=""/>
															</td>
															<td class="bg-editable text-center">
																<?php echo form_checkbox("show_contract[$agc]", '1', false, "id='show_contract-$agc'") ?>
															</td>
															<td class="bg-editable text-center">
																<?php echo form_checkbox("agent_invoice[$agc]", '1', false, "id='agent_invoice-$agc'") ?>
															</td>
												...
												<tbody>
													...
													if ($detail){
														...
														foreach ($detail as $d){
															...
															$qty_quotation = $d->total_qty_quotation;
															$qty_contract = $d->total_qty_contract;

//															$po_qty = $this->M_mar_sales_contract->get_qty_po($d->contract_hdr_id, $d->product_id);
															$po_qty = 0;
...
//																echo "<input type='button' class='btn default btn-xs red-stripe' onclick='removeRow(this)' value='Remove'>";
																...
																echo "<input type='button' class='btn default btn-xs red-stripe remove_detail' value='Remove' poqty='0' id='0'>";
...
//																echo '<input name="brand_name[]" value="'.$d->detail_brand_name.'" id="brn-'.$last_count.'" '.$w_brand.' onClick="viewModalSelectBrand(this.id)" class="form-control input-xs input-table" placeholder="Select Brand" readonly="readonly">';
															...
															if ($d->packing_view){
																...
																$pack_size = floatval($d->uom_volume).' '.  strtolower($d->uom_volume_name).' x '.floatval($d->per_packing).' '.$d->packing_size.' per '.$d->cma_uom_quantity_id;
															}
															echo "<td class=''>";
																echo "<input name='pack_size[]' readonly $w_pack_size value='$pack_size' class='form-control input-xs input-table' title='$pack_size'>";
															echo "</td>";
															//Kolom UOM
															echo "<td>";
															...
															//Kolom Qty
															echo "<td class='bg-editable'>";
																echo "<input name='qty[]' required $w_qty value='$d->quantity' po_qty_value='$po_qty' type='text' class='form-control input-xs text-right input-table autonum_qty autofocus' data-v-min='0' onkeyup='calculate()'>";
//																echo "<input name='qty[]' required $w_qty value='$d->quantity' po_qty_value='$po_qty' type='text' class='form-control input-xs text-right input-table autonum_qty autofocus' data-v-min='0' data-v-max='$balance_qty' onkeyup='calculate()'>";
															echo "</td>";
															//Kolom Unit Price
															echo "<td class='bg-editable'>";
...
//																echo "<input type='button' class='btn default btn-xs red-stripe' onclick='removeRow(this)' value='Remove'>";
																echo '<div class="input-group input-table-group">';
																echo "<input type='button' class='btn default btn-xs red-stripe remove_detail' value='Remove' poqty='$po_qty' id='".encode_str($cd->contract_dtl_id,'contract')."'>";
	...
	//															echo "<input type='hidden' name='quotation_dtl_id[]' value='$d->quotation_dtl_id'>";
															...
															//Kolom Product Description
															echo "<td class='bg-editable'>";
																if ($cd->detail_product_desc){
																	$product_name = $cd->detail_product_desc;
																} else {
																	$product_name = $cd->product_name;
																}
																echo "<input name='product_name[]' $w_prod_desc value='$product_name' class='form-control input-xs input-table' placeholder='Product Name' title='$cd->product_name'>";
															echo "</td>";
															//Kolom Product Code
															echo "<td>";
	...
	//															echo form_hidden('estimated[]', $cd->estimated_qty);
										...
										<script type="text/javascript">
											$('#tblsales_contract .remove_detail').on('click', function(){
												...
												var poqty = $(this).attr('poqty');

												if (poqty > 0){
													bootbox.alert('You can not remove this product, because PO already created for this product.');
													return false;
												}
											...
											$('#tblsales_contract .remove_brand').on('click', function(){
...
<!--											Dihilangkan sementara
										...
										<div class="col-md-9" id="payterm_container">
											<?php
												$extra_payterm = 'id="payment_term_id" class="form-control" required="required" ';
												$option_payterm[''] = '';
...
//												echo form_dropdown('payment_term', $option_payterm, $payment_term, $extra_payterm);
									...
									<div class="form-group required">
										...
										<div class="col-md-9" id="bank_container">
											<?php
									...
									<div class="form-group">
										...
										<div class="col-md-9">
											<div id="document_container" class="doc-scroll">
							...
							<div class="form-actions">
								<div class="row">
									<div class="col-md-12">
										...
										<?php echo $btn_excel ?>
										<?php echo $btn_delete ?>
										<a href="<?php echo site_url('sales-contract')?>" type="button" class="btn red pull-right" id="btn_save"><i class="fa fa-close"></i> Cancel</a>
				...
				<div class="modal-header">
					...
					<div class="row">
						...
						<div class="col-md-4" style="width: 390px;">
							<button type="button" id="search_product_purchased" class="btn btn-block red-stripe" style="border-width: 1px;">
								<i class="icon-list"></i> Show Current Customer's Product Purchased
							</button>
						</div>

						<?php if ($quotation_hdr_id > 0){ ?>
<!--						<div class="col-md-3">
							<button type="button" id="search_product_quotation" class="btn btn-block red-stripe" style="border-width: 1px;">
								<i class="icon-list"></i> Show Current Sales Quotation's Product
							</button>
						</div>-->
						<?php } ?>
					</div>
				...
				<div class="modal-body padding-5">
					<div id="table_container">
						...
						<div class="v-scroll" style="height: 400px;">
	...
	<div id="modal_find" class="modal fade bs-modal-lg" role="dialog" tabindex="-1" data-backdrop="static" data-keyboard="false" aria-hidden="true">
		<div class="modal-dialog modal-full">
			<div class="modal-content">
				...
				<div class="modal-body padding-5">
					<div id="table_find">
						<div class="v-scroll loading_find">
...
//	$('#form_sales_contract').on('submit',function(){
//		var contract_no = $("#sales_contract_no").val();
//
//		$.ajax({
//			type: "POST",
//			url : "<?php // echo site_url('sales-contract/contract-number-exists')?>",
//			data: 'sales_contract_no' = contract_no,
//			success: function(msg){
//				if (msg > 0){
//					$.bootstrapGrowl('<strong><i class="fa fa-warning"></i>'+contract_no+' already exists.</strong>', {
//							ele: 'body', // which element to append to
...
//
	...
	$('#search_product_purchased').on('click', function(){
		var customer_id = {customer_id:$("#customer_list").val()};

		$.ajax({
			type: "POST",
			url	: "<?php echo site_url('sales_contract/get_product_purchased_on_add_product')?>",
			data: customer_id,
			success: function(msg){
				$('#table_container').html(msg);
			}
	...
	$('#btn_delete').click(function(){
		...
		var po_already = $(this).attr("po_already");

		if (po_already > 0){
//			bootbox.alert("Sales Contract can't be deleted because PO already created use this contract.");
			$.bootstrapGrowl("<h5><i class='fa fa-warning'></i>  Sales Contract can't be deleted because PO already created use this contract.</h5>", {
				ele: 'body', // which element to append to
				type: 'danger', // (null, 'info', 'danger', 'success', 'warning')
				offset: {
					from: 'top',
...
<div class="modal fade" id="modal-select-brand" tabindex="-1" role="basic" aria-hidden="true">
    <div class="modal-dialog" style="width: 50%;">
        <div class="modal-content">
            ...
            <div class="modal-footer">
				<!--<button type="button" class="btn red-sunglo" id="btn_clear-brand" onclick="ClearBrand();">Clear Brand</button>-->
...
//				} else {
					var baris = $('#tblsales_contract tr').length;
					var nn = currentID++;
					var nnn = currentID2++;
					$('#tblsales_contract > tbody:last-child').append(
						'<tr>\n\
							<td class="text-center w-50 bg-editable valign-middle">\n\
								...
								<input type="button" class="btn default btn-xs red-stripe remove_detail_add" value="Remove" poqty="0" id="0">\n\
								...
								<input type="hidden" name="contract_dtl_id[]" value="0" id="contract_dtl_id">\n\
							</td> \n\
							<td class="bg-editable"><input name="product_name[]" <?php echo $w_prod_desc?> class="form-control input-xs input-table" placeholder="Product Description" value="'+getText(document.getElementById('tbl_product').rows[i].cells[2])+'" title="'+getText(document.getElementById('tbl_product').rows[i].cells[2])+'"></td>\n\
							...
							<td class="bg-editable">\n\
									...
									<div class="input-group" style="margin-bottom: 0px;">\n\
										...
										<div class="input-group-btn">\n\
											<input type="button" class="btn btn-xs fontawesome-font fa-grey fa-transparent remove_brand" title="Clear brand name" value="×" id="'+nn+'">\n\\n\
										</div>\n\
									</div>\n\
                            </td>\n\
							<td class=""><input name="pack_size[]" readonly <?php echo $w_pack_size?> value="'+getText(document.getElementById('tbl_product').rows[i].cells[8])+'" class="form-control input-xs input-table" title="'+getText(document.getElementById('tbl_product').rows[i].cells[8])+'">\n\
							...
							<td class="sembunyi"><input name="qty_quotation[]" <?php echo $w_qty?> value="0" class="form-control input-xs text-right input-table" readonly="readonly"></td>\n\
							<td><input name="qty_contract[]" <?php echo $w_bal_qty?> value="0" class="form-control input-xs text-right input-table" readonly="readonly"></td>\n\
							...
							<td class="bg-editable">\n\
									<input name="fcl[]" <?php echo $w_fcl?> required value="" type="text" class="form-control input-xs text-right input-table autonum_fcl autofocus">\n\
					...
					$('#tblsales_contract .remove_detail_add').on('click', function(){
						var tr = $(this).closest('tr');
...
//				}
	...
	function cekpo(id){
		...
		var poval = $('#'+id).attr('po_qty_value');
		var msg = 'PO already created, please enter a quantity greater than '+poval;
...
//			$(this).find("input[name='qty_balance[]']").val(number_format(bal_qty, 0));
		...
		if (container_size == 0){
			$.bootstrapGrowl("<i class='fa fa-exclamation-circle'></i>  Please specify FCL's value manually.", {
				ele: 'body', // which element to append to
	...
	function agent_percent(){
		...
		// }
		var persen = document.getElementsByClassName('agent_com_percent');
		var unit = document.getElementsByClassName('agent_com_unit');
		// alert(unit[0].value);
		// alert('tes');
		// alert(persen.length);
		for(var i=0; i < persen.length; i++){
			// var pers[i] = remove_percent(persen[i].value);
			if(remove_percent(persen[i].value) > 0){
				unit[i].value = '';
			}
			// alert(remove_percent(persen[i].value));
		}
	}
...
//		var container_size = arr_container_list[1];
	...
	function validate()
	{
		var product_count =  document.getElementById("tblsales_contract").getElementsByTagName("tbody")[0].getElementsByTagName("tr").length;
		...
		// Check SM in Charge
		var smarketingid = $('#sales_marketing_id').val();

		if (!smarketingid){
			$.bootstrapGrowl('<i class="fa fa-exclamation-circle"></i>  Please select SM in charge!', {
				ele: 'body', // which element to append to
				type: 'danger', // (null, 'info', 'danger', 'success', 'warning')
				offset: {
					from: 'top',
		...
		// Check Sales Person
		var spersonid = $('#sales_person_id').val();

		if (!spersonid){
			$.bootstrapGrowl('<i class="fa fa-exclamation-circle"></i>  Please select sales person!', {
				ele: 'body', // which element to append to
				type: 'danger', // (null, 'info', 'danger', 'success', 'warning')
				offset: {
					from: 'top',
		...
		// Check SM in Charge
		var sprodmgt = $('#product_manager').val();

		if (!sprodmgt){
			$.bootstrapGrowl('<i class="fa fa-exclamation-circle"></i>  Please select product Manager!', {
				ele: 'body', // which element to append to
				type: 'danger', // (null, 'info', 'danger', 'success', 'warning')
				offset: {
					from: 'top',
