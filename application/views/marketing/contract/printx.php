...
//		$this->Line(10, $this->GetY(), 200, $this->GetY());
        ...
        $this->Ln(2);
        $this->setFont('Arial', '', $fontsize);
        $this->SetWidths(array(5,44,25,25,20,20,12,13,24));
...
//                        number_format($baris->uom_volume,0).' '.$baris->uom_volume_name.' x '.number_format($baris->per_packing,0).' '.$baris->packing_size.' per '.$baris->cma_uom_quantity_id,
        ...
        $this->SetFont('Arial', '', $fontsize);
        $this->MultiCell(155, 4, $record_header->currency_say_in_words.' '.  convert_number_to_words($grand_total_float).' Only', 0, 1, 'L', 1);
//		$this->MultiCell(155, 4, $record_header->currency_say_in_words.' '.convert_number_to_words($grand_total_float).' Only', 0, 1, 'L', 1);
