							...
							<div class="col-md-6">
								<div class="form-group">
									...
									?>
										...
										<div class="col-md-2">
											<?php 
												$extra_destination = 'id="rev_value" class="form-control select2me" style="margin-left:-35px;"';
											...
											?>
												<tr>
													<td class="text-center bg-editable">
                                                        <?php if (decode_str($po_hdr_id) == $r->po_hdr_id) { ?>
														<div class="input-group input-table-group">
                                                            <input disabled="disabled" type="button" class="btn default btn-xs green-stripe" sid="<?php echo $r->po_hdr_id?>"  style="width: 70px; margin-left: 5px; margin-top: 2px;" onclick="removePO(this)" value="Main PO">
														</div>
                                                        <?php } else { ?>
                                                        <div class="input-group input-table-group">
															<input <?php echo $disabled_btn ?> type="button" class="btn default btn-xs red-stripe removemixpo" poid="<?php echo $r->po_hdr_id?>" shipid="<?php echo $r->ship_id?>" style="width: 70px; margin-left: 5px; margin-top: 2px;"  value="Remove">
														</div>
                                                        <?php } ?>
													</td>
													<td>														
														...
														<input type="text" name="detail_po_number[]" id="detail_po_number" value="<?php echo $r->po_number ?>" class="form-control input-xs input-table" readonly="readonly" />
													</td>
											...
											?>
												...
												<tr>
													...
													<td>
														...
														<input type="text" name="detail_po_number[]" id="detail_po_number" value="<?php echo $po_number ?>" class="form-control input-xs input-table" readonly="readonly" />
													</td>
													<td class="center" style="width: 150px">
														<div class="btn-group btn-group-xs btn-group-solid" style="margin-left: 2px; margin-top: 2px;">
			...
			if (chk_arr[r].checked == true) {
				...
				$('#tbl_po > tbody:last-child').append(
					...
					+'<td>'
						...
						+'<input type="text" name="detail_po_number[]" id="detail_po_number" value="'+ getText(document.getElementById('tbl_filter_po').rows[i].cells[2]) +'" class="form-control input-xs input-table" readonly="readonly" />'
					+'</td>'
					+'<td class="center" style="width: 150px">'
						+'<div class="btn-group btn-group-xs btn-group-solid" style="margin-left: 2px; margin-top: 2px;">'
...
<script type="text/javascript">
    $('#tbl_po .removemixpo').on('click', function(){
        var tr          = $(this).closest('tr');
        var ship_id     = $(this).attr('shipid');
        var po_hdr_id	= $(this).attr('poid');
    

        bootbox.confirm('Are you sure want to remove this PO?<br>This will remove the Shipping Instruction PO',function(result){
            if (result){
                if (ship_id !== '0'){
                    $.ajax({
                        type: "POST",
                        url	: "<?php echo site_url('marketing-transaction/shipping-instruction/remove_mix')?>",
                        data: {poid : po_hdr_id, shipid : ship_id},
                        success : function(){
                            $.bootstrapGrowl('<i class="fa fa-info-circle"></i> Remove PO Success.', {
                                    type: 'success', // (null, 'info', 'danger', 'success', 'warning')
                                    offset: {
                                        from: 'top',
                                        amount: 250
