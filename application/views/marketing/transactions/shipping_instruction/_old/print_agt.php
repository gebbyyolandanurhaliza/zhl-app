...
//		$this->Cell(30, 4, ': '.$this->PageNo().' of {nb}', 0, 0, 'L', false);
	...
	function Content($rec_hdr, $rec_dtl, $rec_doc, $rec_doc_s, $agent,$po1, $po2, $via)
	{
		$fontsize = 9;
...
//			$this->MultiCell(15, 4, 'Factory');
			...
			$this->Cell(75, 4, 'Issue By : '.$rec_hdr->issue_by,0,1,'R');
		...
		//==> CUSTOMER & ORDER INFO
			...
			$this->Cell(35, 4, 'No of FCL');
			$this->Cell(80, 4, ': '.$rec_hdr->container_name);		
			$this->Cell(35, 4, 'Client Ref No');
			$this->Cell(40, 4, ': '.$rec_hdr->client_ref_no, 0, 1);

			$y = $this->GetY();
			$this->SetXY(10,$y);
			$this->MultiCell(35, 4, 'Purchase Order');
			$this->SetXY(45, $y);
			$this->MultiCell(80, 4, ': '.$po1.' '.$po2);
			$this->SetXY(45+80, $y);
			$this->MultiCell(35, 4, 'Client Contract No');
			$this->SetXY(45+80+35, $y);
...
//			$this->Cell(40, 4, ': '.$rec_hdr->client_contract_no, 0, 1);
			...
			$this->Cell(35, 4, 'Client');
			$this->Cell(80, 4, ': '.$rec_hdr->customer_company_name);		
			$this->Cell(35, 4, 'Customer Code');
			$this->Cell(40, 4, ': '.$rec_hdr->customer_code, 0, 1);

			$this->Cell(35, 4, 'Agent');
			$show_agent = ($agent) ? $agent->agent_name : '';
			$this->Cell(80, 4, ': '.$show_agent);		
			$this->Cell(35, 4, 'Our Sales Contract No');
			$this->Cell(40, 4, ': '.$rec_hdr->contract_no, 0, 1);

			$this->Cell(35, 4, 'Invoice To Buyer');
			$this->Cell(80, 4, ': '.$rec_hdr->invoice2buyer);		
			$this->Cell(35, 4, '');
			$this->Cell(40, 4, '', 0, 1);
		...
		//==> PRODUCT INFO
			...
			foreach ($rec_dtl as $pro){
				...
				$this->Cell(165, 4,': '.$product_desc, 0, 1, 'L', false);

				$this->Cell(5, 4);
				$this->Cell(30, 4, 'Packing');
				$this->Cell(80, 4,': '.$pro->detail_pack_size, 0, 0, 'L', false);
	//			$this->Cell(165, 4,': '.$pro->uom_volume_id.' '.$pro->uom_volume_name.$per_quantity, 0, 1, 'L', false);
				$this->Cell(20, 4, 'Brand');
				$this->Cell(55, 4, ': '.$pro->brand_name, 0, 1, 'L', false);
	...
	//			$this->Cell(165, 4,': '.$pro->brand_name, 0, 1, 'L', false);
				...
				$this->Cell(20, 4, 'Palletized');
				$this->Cell(55, 4, ': '.$palletized, 0, 1, 'L', false);
				...
				$this->Cell(20, 4, 'Pallet No.');
				$this->Cell(55, 4, ': '.$pro->pallet_qty, 0, 1, 'L', false);
...
//			$this->Cell(80, 4, ': '.$rec_hdr->trading_term_name.' '.$rec_hdr->shipment_from);		
			...
			$this->Cell(80, 4, ': '.$rec_hdr->ocean_freight, 0, 1);		
	//		$this->Cell(35, 4, 'Trucking Date');
	//		$this->Cell(40, 4, ': '.tgl_dmy($rec_hdr->trucking_date), 0, 1);

			$this->Cell(35, 4, 'Shipping Liner');
			$this->Cell(165, 4, ': '.$rec_hdr->shipping_name, 0, 1);

			$this->Cell(35, 4, 'Service Number');
			$this->Cell(165, 4, ': '.$rec_hdr->service_number, 0, 1);

			$this->Cell(35, 4, 'Final Destination');
			$this->Cell(165, 4, ': '.$rec_hdr->port_name.' '.$rec_hdr->destination, 0, 1);
...
foreach ($mix_po as $mp){
	
	$get_list_po	= $this->M_mar_shipping_instruction->get_mix_po($mp->ship_id, $mp->po_hdr_id);
	if ($get_list_po){
		$i = 1;
	...
	$pdf->AliasNbPages();
	$pdf->set_factory($mp->factory_abbr);	
	$pdf->StartPageGroup();
	$pdf->AddPage();	
	
	$rec_dtl = $this->M_mar_shipping_instruction->get_detail_product_by_po($mp->ship_id, $mp->po_hdr_id);
	
	$pdf->Content($rec_hdr,$rec_dtl,$rec_doc,$rec_doc_s,$agent,$mp->po_number,$list_po, $mp->via);
}
$pdf->Output();
