...
//		$this->Cell(30, 4, ': '.$this->PageNo().' of {nb}', 0, 0, 'L', false);
	...
	function Content($rec_hdr, $rec_dtl, $rec_doc, $rec_doc_s, $agent,$po1, $po2, $via, $con_issued)
	{
		$fontsize	= 9;
		$border		= 0;
		$align		= 'L';
...
//			$this->Cell(80, 4, ': '.$rec_hdr->trading_term_name.' '.$rec_hdr->shipment_from);		
			...
			$this->setFont('Arial', '', $fontsize);
			$this->Cell(35, 4, 'Service Number');
			$this->Cell(80, 4, ': '.$rec_hdr->service_number);
			
			$this->setFont('Arial', 'B', $fontsize);
			$this->Cell(35, 4, 'Quantity Grand Total');
			$this->Cell(40, 4, ': '.number_format($grand_tot_qty, 0), 0, 1);
...
for($i=0; $i < $total_si; $i++){
	...
	$rec_hdr	= $this->M_mar_shipping_instruction->get_by_id($header_id);
	$rec_doc	= $this->M_mar_shipping_instruction->get_view_document($header_id);
	$rec_doc_s	= $this->M_mar_shipping_instruction->get_view_document($header_id, 1);	// for special document
	$agent		= $this->M_mar_shipping_instruction->get_agent_for_print($header_id)->row();
	$mix_po		= $this->M_mar_shipping_instruction->get_detail_po($header_id);
	...
	foreach ($mix_po as $mp){
		...
		$pdf->Content($rec_hdr,$rec_dtl,$rec_doc,$rec_doc_s,$agent,$mp->po_number,$list_po, $mp->via, $con_issued);
	}
}
