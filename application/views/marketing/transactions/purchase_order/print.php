...
class PDF1 extends TFPDF {
    ...
    function set_status_id($status){
		$this->status_id = $status;
	}
    
    function canceled_reason($cancel_remark){
        $this->canceled_reason = $cancel_remark;
    }
    ...
    function Header() {
        ...
        if($this->rev_check == 1) {
	        ...
	        $this->Cell(24, 4, ': '.$this->rev_number, 0, 0, 'L', false);
			$this->Cell(1, 4, '', 'L', 1, 'L');
        } else {
			$this->Cell(48, 3, '', 0, 0);
			$this->Cell(1, 3, '', 'L', 1);
    ...
    //        $this->Cell(150,5,'CANCELED REASON : '.$this->canceled_reason,0,0,'L',1);
            $this->MultiCell(150,5,'CANCELED REASON : '.$this->canceled_reason, 0, 'L', false);
        }
    }
...
//		$this->cell(70, 5, ': '.$rec_hdr->port_name.', '.$rec_hdr->destination, 0, 1, 'L', 1);
		...
		$arr_cell_height = array(
			$this->NbLines(63, $rec_hdr->ship_mark),
			$this->NbLines(68, $rec_hdr->ocean_freight),
		);
		...
		$this->SetFont('Arial', '', 9);
		$this->SetXY(10, $y);
		$this->Multicell(30, 5, 'Shipping Mark', 0, 'L');
		$this->SetXY(10+30, $y);
		$this->MultiCell(2, 5, ': ');
		$this->SetXY(10+30+2, $y);
        $this->Multicell(63, 5, $rec_hdr->ship_mark, 0, 'L');
		$this->SetXY(10+30+2+63, $y);
		$this->Multicell(26, 5, 'Ocean Freight', 0, 'L');
		$this->SetXY(10+30+2+63+26, $y);
		$this->MultiCell(2, 5, ': ', 0, 'L');
		$this->SetXY(10+30+2+63+26+2, $y);
		$this->cell(68, 5, $rec_hdr->ocean_freight, 0, 1, 'L', 1);
...
//		$this->SetFont('Arial', '', 9);
		$this->SetFont('DroidSansFallback', '', 9);
//		$this->setFont('NotoSans-Regular', '', 9);		// <== gak bisa  baca character china, ex. #C114/06/17 PSS
		...
		$this->Cell(30, 7, 'Unit Value ('.$rec_hdr->currency_id.')', 'R', 1, 'C');
		$this->Line(10, $this->GetY(), 200, $this->GetY());
...
//			if ($r->price == 0){
//				$this->MultiCell(30, $h, number_format($r->foc_price,2,'.',',').' per '.$r->cma_uom_quantity_id, $border, 'R');
//			} else {
//				$this->MultiCell(30, $h, number_format($r->fob_price,2,'.',',').' per '.$r->cma_uom_quantity_id, $border, 'R');
//			}
...
//			$arr_i++;
		...
		$this->MultiCell(30, 4, $rec_hdr->container_name, 0, 'C');
...
$pdf->AliasNbPages();
$pdf->set_status_id($rec_hdr->status_id);
$pdf->canceled_reason($rec_hdr->cancel_po_remark);
$pdf->set_revisi($rec_hdr->revisi,$rec_hdr->revisi_num);
