...
//		$this->Cell(24, 4, ': '.$this->PageNo().' of {nb}', 0, 0, 'L', false);
	...
	function Content($rec_hdr,$rec_dtl,$rec_doc,$rec_agent) {
		...
		$this->cell(69, 5, ': '.$rec_hdr->factory_name, 0, 1, 'L', 1);
...
//		$this->cell(69, 5, ': '.$rec_hdr->ocean_freight, 0, 1, 'L', 1);
...
//
// 	$pdf->Content($hdr,$detail,$doc,$agent);
// }

for($i=0; $i < $total_po; $i++){

	$hdr				= json_decode($js_hdr[$i]);
	$contract_hdr_id 	= json_decode($js_con[$i]);
	...
	$detail		= json_decode($js_det[$i]);
	$doc		= json_decode($js_doc[$i]);
