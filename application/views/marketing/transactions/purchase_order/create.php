								...
								<div class="col-md-12">
									...
									<div class="col-md-12">											
										...
										<div class="form-group">
											...
											if($act == 'edit'){
											?>
												<label class="col-md-1 control-label" style="padding-right:5px; color:#FF0000;" for="rev">Revision<input type="checkbox" id='rev' class="form-control" name="rev_check" <?php echo ($rev_check != 1 ? '' : 'checked') ?>/></label>
												<div class="col-md-1">
													<?php 
														$extra_destination = 'id="rev_value" class="form-control" style="margin-left:-5px;"';
...
//													var hdr_id = <?php // echo decode_str($po_hdr_id) ?>;
										...
										<script type="text/javascript">											
											...
											$('#local_currency').change(function(){
												var currency_id = $("#local_currency").val();
												var tanggal		= $("#po_date").val();
												
												if (tanggal == ''){
													$.bootstrapGrowl('<i class="fa fa-danger"></i> Please fill the Issue Date!', {
												...
												$.ajax({
													type: "POST",
													url : "<?php echo site_url('marketing_misc/get_rate_by_date')?>/"+tanggal,
													data: {
...
//														$sub_total = $d->fob_price * $d->quantity;
														...
														switch ($factory_id){
															...
															case 4:
																...
																break;
															case 2:
																$detail_po_format = $d->po_code_prefix_pske;
																break;
															default:
...
//															echo "<input name='carton_remark[]' $s_cart_remark type='text' class='form-control input-xs input-table text-right carton_remark_class'>";
												...
												if ($detail){
													...
													foreach($detail as $d){
														...
														$qty_contract	= $this->M_mar_purchase_order->get_qty_contract($contract_hdr_id, $d->contract_dtl_id, $d->product_id);
//														$qty_po			= $this->M_mar_purchase_order->get_qty_po($contract_hdr_id, $d->contract_dtl_id, $d->product_id, $d->po_hdr_id);
														$qty_po			= $this->M_mar_purchase_order->get_qty_po($contract_hdr_id, $d->contract_dtl_id, $d->product_id);
...
//														echo "<input name='marking_short_side[]' value='$d->marking_short_side' style='width:200px;' type='text' class='form-control input-xs input-table'>";
												...
												if ($detail_copy){
													...
													foreach($detail_copy as $d){
														...
														$qty_contract	= $this->M_mar_purchase_order->get_qty_contract($contract_hdr_id, $d->contract_dtl_id, $d->product_id);
														$qty_po			= $this->M_mar_purchase_order->get_qty_po($contract_hdr_id, $d->contract_dtl_id, $d->product_id, 0);
//														$qty_po			= $this->M_mar_purchase_order->get_qty_po($contract_hdr_id, $d->contract_dtl_id, $d->product_id);
...
//							ele: 'body', // which element to append to
    ...
    $('#btn_cancel_po').on('click',function(){
        var used_si = $(this).attr("used_si");
        var po_hdr_id = $('#po_hdr_id').val();
        
        if (used_si > 0){
			$.bootstrapGrowl("<h5><i class='fa fa-warning'></i>  This PO can't be canceled because Shipping Instruction already created using this PO.</h5>", {
//				ele: 'body', // which element to append to
				type: 'danger', // (null, 'info', 'danger', 'success', 'warning')
				offset: {
					from: 'top',
        ...
        $.ajax({
           type : "POST",
           data : "po_hdr_id="+po_hdr_id,
           url  : "<?php echo site_url('marketing_transaction/purchase_order/cancel_po') ?>",
           success : function(msg){
...
//		}
		...
		$('#tbl_po tr').each(function() {
			...
			if (qty == 0 || qty == ''){
				$.bootstrapGrowl('<i class="fa fa-exclamation-circle"></i>  Quantity can not be empty or 0.', {
					ele: 'body', // which element to append to
					type: 'danger', // (null, 'info', 'danger', 'success', 'warning')				
					offset: {
						from: 'top',
...
// cuk gak iso mlaku
//			var poformat_sekarang = $(this).find("input[name='detail_po_format[]']").val();
//			
//			if (poformat_sekarang != undefined){
//				console.log('posblm : ' + poformat_sebelum + 'poskrg : ' + poformat_sekarang);
//				if (poformat_sebelum != poformat_sekarang){
//					$.bootstrapGrowl('<i class="fa fa-exclamation-circle"></i> There are different product category in the product list, Please check again!', {
//						ele: 'body', // which element to append to
//						type: 'danger', // (null, 'info', 'danger', 'success', 'warning')				
//						offset: {
//							from: 'top',
...
//					keluar = 1;
//				}
//				poformat_sebelum = poformat_sekarang;
//			}
