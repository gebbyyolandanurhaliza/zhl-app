...
//															echo form_hidden('po_format[]', $po_format);
															...
															if ($qty_contract > 0 && $d->price == 0){
																echo "<td class='bg-editable'>";															
																echo "<input name='foc_price[]' $s_focprice type='text' class='form-control input-xs text-right input-table autonum_fob' data-v-min='0' onkeyup='calculate()'>";
															} else {
																echo "<td>";
																echo "<input name='foc_price[]' readonly $s_focprice type='text' class='form-control input-xs text-right input-table autonum_fob' data-v-min='0' onkeyup='calculate()'>";
															}
															//Kolom Total
															echo "<td>";
...
//														$up =  $qty_contract > 0 ? 'true' : 'false';
														echo "<td class='bg-editable' u_price='$d->price' $qty_contract >";
														if ($qty_contract > 0 && $d->price == 0){
														...
														if ($qty_contract > 0 && $d->price == 0){
															echo "<td class='bg-editable'>";														
															echo "<input name='foc_price[]' value='$d->foc_price' $s_focprice type='text' class='form-control input-xs text-right input-table autonum_fob autofocus' data-v-min='0' onkeyup='calculate()'>";
														} else {
															echo "<td>";
															echo "<input name='foc_price[]' readonly value='$d->foc_price' $s_focprice type='text' class='form-control input-xs text-right input-table autonum_fob' data-v-min='0' onkeyup='calculate()'>";
														}
														echo "</td>";
														//Kolom Total
														echo "<td>";
...
//	}
	...
	function select_product(){
		...
		var s_focprice		= "style='width:110px;'";
...
//				var unit_price_readonly = "";
				...
				var fob_max = "";
				var foc_editable = "bg-editable";
				var foc_readonly = "";
				
				if (unit_price > 0){
//					unit_price_readonly =" readonly='readonly' ";
//					unit_price_editable = "";
					fob_max = "data-v-max='"+unit_price+"' ";
					foc_editable = "";
					foc_readonly = " readonly='readonly' ";
				}
...
//				var cbo_carton_remark = <?php // echo form_dropdown('cbo_carton_remark[]', $option_carton_remark, '', $extra_carton_remark.' crid='+nn+' prev_value=""'); ?>
				...
				$('#tbl_po > tbody:last-child').append(
					'<tr>'
						...
						//Kolom FOC Price
						+'<td class="'+foc_editable+'">'														
						+'<input name="foc_price[]" '+s_focprice+foc_readonly+' type="text" class="form-control input-xs text-right input-table autonum_fob autofocus" data-v-min="0" onkeyup="calculate()">'
						+'</td>'
...
//			var estimated	= $(this).find("input[name='estimated_qty[]']").val();
			...
			var foc_price	= remove_thousand_separator($(this).find("input[name='foc_price[]']").val());
			var per_pack	= remove_thousand_separator($(this).find("input[name='per_packing[]']").val());
...
//			var total_row	= qty * fob_price;
			...
			if (fob_price == 0){
				total_row	= qty * foc_price;
			} else {
				total_row	= qty * fob_price;
			}
