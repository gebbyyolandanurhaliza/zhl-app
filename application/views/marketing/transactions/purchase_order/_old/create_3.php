									...
									<div class="col-md-12">											
										...
										<div class="form-group">
											...
											<div class="col-md-2">
												...
												?>
												<input type="text" class="form-control" name="po_number" id="po_number" value="<?php echo $po_number ?>" <?php echo $required ?>/>
											</div>
...
//												$extra_carton_remark = 'id= "cbo_carton_remark" class="form-control input-xs input-table cbo_carton_remark_class" placeholder="Select History" '.$s_cart_remark;
//												$option_carton_remark[''] = '';
//												foreach($cbo_carton_remark as $r):
//													if ($r->carton_remark){
//														$option_carton_remark[$r->carton_remark] = $r->carton_remark;
//													}
//												endforeach;
////												echo form_dropdown('carton_remark[]', $option_carton_remark, '', $extra_carton_remark);
...
//														$sub_total = $d->fob_price * $d->quantity;
														...
														if ($outstanding_qty > 0){
															...
															echo "<input type='button' $disabled_btn style='width: 80px; margin-left: 5px; margin-top: 2px;' class='btn default btn-xs red-stripe' onclick='removeRow(this)' value='Remove'>";
...
//														echo "<input type='button' style='width: 80px; margin-left: 5px; margin-top: 2px;' class='btn default btn-xs red-stripe' onclick='removeRow(this)' value='Remove'>";
														echo "<input type='button' $disabled_btn style='width: 80px; margin-left: 5px; margin-top: 2px;' class='btn default btn-xs red-stripe remove_detail' value='Remove' id='".encode_str($d->po_dtl_id,'po')."'>";
//														echo "<a data-target='#modal_product' data-toggle='modal' type='button' class='btn default btn-xs blue-stripe'>Detail Product</a>";
...
//														echo "<input name='brand[]' value='$d->brand_name' $s_brand class='form-control input-xs input-table' readonly='readonly'>";
														...
														echo "<input name='qty[]' required value='$d->quantity' $s_qty $qty_max type='text' class='form-control input-xs text-right input-table autonum_qty autofocus' data-v-min='0' onkeyup='calculate()'>";
														echo "</td>";
														//Kolom FOB Price
														echo "<td class='bg-editable'>";
														if ($qty_contract > 0 && $d->price == 0){
	...
	function select_product(){
		...
		for(r=0;r < chk_length ;r++){
			if (chk_arr[r].checked == true){
				...
				var unit_price_editable = "bg-editable";
				if (unit_price > 0){
				...
				$('#tbl_po > tbody:last-child').append(
					'<tr>'
						+'<td class="text-center bg-editable" style="vertical-align: top;">'
							...
							+'<input type="hidden" name="contract_dtl_id[]" value="'+getText(document.getElementById('tbl_product').rows[i].cells[12])+'">'
						+'</td>'
						...
						+'<td class="bg-editable"><input name="qty[]" '+s_qty+' value="'+getText(document.getElementById('tbl_product').rows[i].cells[15])+'" type="text" class="form-control input-xs text-right input-table autofocus autonum_qty" data-v-min="0" onkeyup="calculate()">'
						+'<td class="bg-editable"><input name="fob_price[]" '+s_fobprice+' type="text" class="form-control input-xs text-right input-table autonum_fob autofocus" data-v-min="0" onkeyup="calculate()">'
