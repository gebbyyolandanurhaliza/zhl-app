...
<script>
  $(function() {
    $('.no_coa').autocomplete({
      serviceUrl: "<?php echo site_url('Receivable_recognition/get_coa'); ?>",
...
<script>
  ...
  function buat_faktur() {
    ...
    if (tgl != '') {
      ...
      if (refno.value === "INV") {
        $.ajax({
          url: "<?php echo base_url(); ?>Receivable_recognition/buat_nofak?tahun=" + y,
          success: function(e) {
  ...
  function get_coa() {
    ...
    if(btn_update === 'Save'){
      var url2 = "<?php echo base_url(); ?>Receivable_recognition/getTerm?supplier=" + supplierID;
      $.ajax({
        url: "<?php echo base_url(); ?>Receivable_recognition/getTerm?supplier=" + supplierID,
        type: "GET",
        dataType: "json", 
    ...
    listCoa.forEach(function(element) {
        ...
        if(Nocoa == nocao.value){
          ...
          if(sub_account == "DEPT"){
            ...
            $('#txtDeptCodeRow5').html(`<input name="dept_code[5]" type="text" class="txt" value="000" required />`);
          }
        }
    });
    ...
    $.ajax({
      url: '<?php echo site_url('Receivable_recognition/get_supp_piu'); ?>',
      method: 'GET',
      dataType: 'json',
      data: {
        supp: supplierID,
  ...
  function ambil(x) {
    ...
    var dept_code = getText(document.getElementById('tbl_coa').rows[$r].cells[5]);

    var rate = document.getElementById('rate_currency').value;
    for (var i = 0; i < num; i++) {
      $('table[id="tabel"]').append('<tr><td width="3%"><button class="tombol" onclick="hapus_dp(this)" >Remove</button></td>\n\
                ...
                <td><input type="text" onkeydown="return validasi_enter(event)" name="txtdept[]" class="txt" value="' + dept_code + '" readonly/></td>\n\
                <td><textarea type="text" name="txtBlCode[]" rows="1" cols="30" class="txt txtBlCode"> </textarea></td>\n\
                ...
                <td><input type="text" name="txtAmountR[]" class="txt number amount autonumber"  data-a-sep="," data-a-dec="."  value="0"  onkeypress="return isNumber(event)"  value="0" /></td>\n\
                ...
                <td width="10%"><select name="txtGST[]" onchange="cek_gst()" class="txt txtGST">\n\
                        <option value="">Select</option>\n\
  ...
  function tambah_baris() {
    ...
    for (var i = 0; i < num; i++) {
      $('table[id="tabel"]').append('<tr><button class="tombol" onclick="hapus_dp(this)" >Remove</button><td></td>\n\
                ...
                <td><input type="text" name="txtUnit[]" class="txt" /><input type="hidden" name="txtSummary[]" class="txt txtSummary" value="0" /></td>\n\
                <td><input type="text" name="txtPrice[]" class="txt number prices autonumber"  onKeyup="hitung_total()"  placeholder="0" required/></td>\n\
                <td><input type="text" name="txtAmountR[]" class="txt number amount autonumber"  onkeypress="return isNumber(event)"   placeholder="0" required /></td>\n\
  ...
  function ambil_tabel() {
    ...
    $.ajax({
      url: "<?php echo base_url(); ?>Receivable_recognition/data_dp?id=" + supp + "&currency=" + curency,
      success: function(response) {
        $(".err").html(response);
      },
      dataType: "html"
  ...
  function cek_nofak() {
    ...
    $.ajax({
      url: "<?php echo base_url(); ?>Receivable_recognition/cek_tabel_ar?id=" + refno,
      success: function(response) {
        $(".CurID").html(response);
      },
      dataType: "html"
...
if (!empty($get_data_header)) {
  foreach ($get_data_header as $s) {
    ...
    $year            = date('Y', strtotime(str_replace('/', '-', $date_invoice))); 
    $term = $s->term;
...
<div class="page-content">
  <div class="container">
    ...
    <div class="row">
      <form action="<?php echo base_url(); ?>index.php/Receivable_recognition/save_receivable_rec" name="form1" id="form1" method="post">
        <div class="col-md-12">
          ...
          <div class="portlet light">
            <div class="portlet-title">
              ...
              <div class="form-group">
                <?php if ($this->input->get('id') <> '') { ?>
                  <a class="btn btn-primary kanan" href="<?php echo base_url(); ?>Receivable_recognition/add_new"><i class="fa fa-plus"></i> Create New</a>
                <?php
                } else {
                ...
                <div class="row">
                  <div class="col-md-6">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-9">
                        <?php
                        if($year == '' || $year > 2024){
                          $style_kategori = "class='select2me form-control' required  onchange='get_coa()' id='supplier' $disable";
                          echo form_dropdown('supplier', $SupplierIDNew, $supplier_id, $style_kategori);
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-9">
                        <div id="cur_id">
                          <?php
                          $style_currency = "class='select2me form-control' required id='currency' onchange='get_cur();Rate_notfound();'";
                  ...
                  <div class="col-md-6">
                    ...
                    <div class="form-group" hidden>
                      <label class="control-label col-md-3">B/L No.</label>
                      <div class="col-md-3">
                        <input type="text" id="blnnum" name="blnnum" class="form-control" value="<?= $blno; ?>" />
                      </div>
                    </div>
                    <div class="form-group" hidden>
                      <label class="control-label col-md-3" >CNTR No.</label>
                      <div class="col-md-3">
                ...
                <tbody>
                  ...
                  ?>
                      <tr id="coa<?php echo $no++; ?>">
                        ...
                        <td style="width: 6%;"><input type="text" class="txt" name="Detail_po[]" value="<?php echo $v->no_po; ?>" /></td>
                        <td>
                        ...
                        <?php if($v->dept_code == '0' || $v->dept_code == ''){?>
                          <td style="width: 10%;"><input type="text" class="txt" name="txtdept[]" value="000" /></td>
                        <?php } else { ?>
                          <td style="width: 10%;">
                        ...
                        <?php }?>
                        <td style="width: 15%;"><textarea type="text" class="txt" rows="1" cols="30" name="txtBlCode[]"><?php echo trim($v->blCode); ?></textarea></td>
                        ...
                        <td><input type="text" class="txt number amount" name="txtAmountR[]" onkeypress="return isNumber(event);" value="<?php echo number_format($v->Qty * $v->Harga, 2, '.', ','); ?>"/></td>
                        <!--<td><input type="text" class="txt number txtRateDtl" name="txtRate[]" onkeypress="return isNumber(event)" value="<?php //echo number_format($v->rate, 6, '.', '');                                       
                                                                                                                                              ?>"/></td>-->
                        <td style="width: 7%;"><input type="text" class="txt number txtSGD" name="txtSGD[]" onkeypress="return isNumber(event)" value="<?php echo number_format($v->Qty * $v->Harga * $v->rate, 2, '.', ','); ?>" /></td>
                        <td>
              ...
              <table class="table table-bordered" id="table_jurnal">
                ...
                <tbody>
                  ...
                  <tr id="ds1">
                    ...
                    <td>
                        <input type="text" name="dept_code[0]" value="000" class="dept_code txt">
                    </td>
                    <td>
                  ...
                  <tr id="ds2">
                    ...
                    <?php if ($dept1 == null): ?>
                      <td>
                        <select name="dept_code[1]" class="txt">
                          <option value="000"> Without Dept</option>
                          <?php foreach ($dept_code as $dept) : ?>
                            <option value="<?php echo $dept->dept_code; ?>"> <?php echo $dept->dept_name; ?></option>
                          <?php endforeach; ?>
                        ...
                        <select name="dept_code[1]" class="txt">
                          ...
                          <?php foreach ($dept_code as $deptItem): ?>
                            <option value="<?php echo $deptItem->dept_code; ?>" 
                              <?php echo ($dept1 == $deptItem->dept_code) ? 'selected' : ''; ?>>
                    ...
                    <td><input type="text" name="credit_jr[1]" value="<?php echo number_format($credit2, 2, ".", ","); ?>0.00" class="txt number jur_credit" onkeypress="return isNumber(event)"></td>
                  </tr>
                  ...
                  <tr id="ds3">
                    ...
                    <td>
                        <input type="text" name="dept_code[2]" value="000" class="dept_code txt">
                    </td>
                    <td>
                  ...
                  <tr id="ds4">
                    ...
                    <?php if ($dept4 == null): ?>
                      <td>
                        <select name="dept_code[3]" class="txt">
                          <option value="000"> Without Dept</option>
                          <?php foreach ($dept_code as $dept) : ?>
                            <option value="<?php echo $dept->dept_code; ?>"> <?php echo $dept->dept_name; ?></option>
                          <?php endforeach; ?>
                    ...
                    <?php else: ?>
                      <td>
                        <select name="dept_code[3]" class="txt">
                          ...
                          <?php foreach ($dept_code as $deptItem): ?>
                            <option value="<?php echo $deptItem->dept_code; ?>" 
                              <?php echo ($dept4 == $deptItem->dept_code) ? 'selected' : ''; ?>>
                              <?php echo $deptItem->dept_name; ?>
                            </option>
                          <?php endforeach; ?>
                        </select>
                  ...
                  <tr id="ds5">
                    ...
                    <td>
                        <input type="text" name="dept_code[4]" value="000" class="dept_code txt">
                    </td>
                    <td>
                  ...
                  <tr>
                    ...
                    <?php else: ?>
                      <?php if ($dept6 == '000'): ?>
                        <td>
                          <input type="text" name="dept_code[5]" value="000" class="dept_code txt">
                        </td>
                      <?php else: ?>
                        <td>
            ...
            <a class="btn btn-warning" href="<?php echo base_url(); ?>index.php/Receivable_recognition"><i class="fa fa-warning"></i> Cancel</a>
            <?php if ($this->input->get('id') <> '') { ?>
              <a class="btn btn-primary  kanan" id="cetak" href="<?php echo base_url(); ?>index.php/Receivable_recognition/print_report?id=<?php echo htmlspecialchars($this->input->get('id'), ENT_QUOTES); ?>&cur=<?php echo $currency_id; ?>&signature=Mr. Tahir Aziz" target="_blank"><i class="fa fa-print"></i> Print</a>
              <a class="btn btn-danger kanan" href="<?php echo base_url(); ?>index.php/Receivable_recognition/delete_transaction?id=<?php echo htmlspecialchars($this->input->get('id'), ENT_QUOTES); ?>" onclick="return confirm('Are you sure to delete this transaction?')"><i class="fa fa-trash"></i> Delete</a>
              ...
              <div class="col-md-2 kanan">
                <select name="signiture" onchange="ganti_signature()" id="signature" class="form-control kanan">
                  ...
                  <option value="Mr. Adrian">Mr. Adrian </option>
...
<script type="text/javascript">
  $(document).ready(function() {
    ...
    var invoice = $('#tanggal_invoice').val();
    if(invoice){
        var partsInvoice = invoice.split('/');
        var partsClosing = tgl.split('/');

        var yearInvoice = parseInt(partsInvoice[2], 10);
        var yearClosing = parseInt(partsClosing[2], 10);
        if(yearInvoice < yearClosing){
  ...
  $("#btnFindRecord").click(function() {
    $.post("<?php echo site_url(); ?>Receivable_recognition/selectInvoiceReceivable", function(data) {
                        ...
                        <table id='tbl-masterbank' class='table table-bordered'>
                            ...
                            <tbody id='tblmasterbank'>
                                <?php foreach ($bank as $r) : ?>
                                 ...
                                 <td nowrap hidden><textarea><?= 'Beneficiary Name : for Account of Zhenghe Logistic Pte Ltd' . '&#10;' . ltrim($r->bank_name) . '&#10;' . 'SWIFT: ' . $r->bank_swift . '&#10;' . 'Please select the shared (OUR) payment method for SWIFT transfers.' . '&#10;' . $r->bank_currency_id . ' Account No: ' . $r->bank_account_number . '&#10;' . $r->bank_currency_id_2 . ' Account No: ' . $r->bank_account_number_2 . '&#10;' . (($r->swift_intermediary != '') ? '&#10;' . 'Intermediary Bank ' . $r->intermediary  . '&#10;' . 'SWIFT: ' . $r->swift_intermediary : '') ?>
                                </textarea>
                                 </td>
                                    </tr>
                                <?php endforeach; ?>
  ...
  $(document).on('blur', '.txtBlCode', function () {
