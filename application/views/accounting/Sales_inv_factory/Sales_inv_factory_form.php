...
<script>
  $(function() {
    $('.no_coa').autocomplete({
      serviceUrl: "<?php echo site_url('Sales_inv_factory/get_coa'); ?>"
    });
  });
</script>
<script>
    ...
    for (var i = 0; i < gst_type.length; i++) {
      if (sgd_txt[i].value === 0) {
        ...
        if (gst_type[i].value === 'GST') {
          var total = qty[i].value * harga[i].value * rate_sgd.value * 7 / 100;
  ...
  function get_coa() {
    ...
    //alert("Sales_inv_factory/tampil_po?supplier=" + vendor + "&currency=" + currency + "");
    $.ajax({
      url: "<?php echo base_url(); ?>Sales_inv_factory/tampil_po?supplier=" + vendor + "&currency=" + currency + "",
      success: function(response) {
  ...
  function ambil_tabel() {
    ...
    $.ajax({
      url: "<?php echo base_url(); ?>Sales_inv_factory/data_dp?id=" + supp + "&currency=" + curency,
      success: function(response) {
        $(".err").html(response);
      },
      dataType: "html"
  ...
  function cek_nofak() {
    ...
    $.ajax({
      url: "<?php echo base_url(); ?>Sales_inv_factory/cek_tabel_ar?id=" + refno,
      success: function(response) {
        $(".CurID").html(response);
      },
      dataType: "html"
...
<div class="page-content">
  <div class="container">
    ...
    <div class="row">
      <form action="<?php echo base_url(); ?>Sales_inv_factory/save_receivable_rec" name="form1" id="form1" method="post">
        <div class="col-md-12">
          ...
          <div class="portlet light">
            <div class="portlet-title">
              <div class="caption">
                ...
                <span class="caption-subject theme-font">Sales Invoice Journal Factory</span>
              </div>
              <div class="form-group">
                <?php if ($this->input->get('id') <> '') { ?>
                  <a class="btn btn-primary kanan" href="<?php echo base_url(); ?>Sales_inv_factory/add_new"><i class="fa fa-plus"></i> Create New</a>
                <?php
                } else {
            ...
            <div class="portlet-body">
              <div class="form-body">
                  ...
                  <div class="col-md-6">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-9">
                        <div id="cur_id">
                          <?php
                          $style_currency = "class='select2me form-control' id='currency' onchange='get_cur_Sales()'";
                ...
                <hr />
                <a class="btn green" data-toggle="modal" id="po" href="#po_v" title="Serch PO Factory"><i class="fa fa-search"></i> Searh PO Factory</a>
                  ...
                  ?>
                      <tr id="coa<?php echo $no++; ?>">
                        <td style="text-align: center;">
                          <a href="<?php echo base_url(); ?>Sales_inv_factory/delete?id=<?php echo $v->DetailID; ?>&nofaktur=<?php echo $v->HeaderID; ?>"><i class="fa fa-trash"></i></a>
                        </td>
                        <td><input type="hidden" name="txtSono[]" class="txt txtdocno" onkeypress="return isNumber(event)" value="<?php echo $v->sono; ?>" />
                        ...
                        <td><input type="text" class="txt number quantity" name="txtQty[]" value="<?php echo number_format($v->Qty, 3, '.', ','); ?>" onkeypress="return isNumber(event);" onKeyup="hitung_total()" /></td>
                        <td>
                        ...
                        <td><input type="text" class="txt number prices" name="txtunitprice[]" value="<?php echo number_format($v->price, 3, '.', ','); ?>" onKeyup="hitung_total()" /></td>
                        <td><input type="hidden" name="txtRate[]" class="txt number autonumber" onkeypress="return isNumber(event)" value="<?php echo $v->rate; ?>" />
                          <input type="hidden" name="txtNppbItemId[]" class="txt" value="<?php echo $v->npbbitem; ?>" />
                          <input type="text" class="txt number amount" name="txtamount[]" onkeypress="return isNumber(event);" value="<?php echo number_format($v->Qty * $v->price, 3, '.', ','); ?>" />
                        </td>
                        <!--<td><input type="text" class="txt number txtRateDtl" name="txtRate[]" onkeypress="return isNumber(event)" value="<?php //echo number_format($v->rate, 6, '.', '');                                           
                                                                                                                                              ?>"/></td>-->
                        <td><input type="text" class="txt number txtSGD" name="txtusd[]" onkeypress="return isNumber(event)" value="<?php echo number_format($v->Qty * $v->price * $v->rate, 3, '.', ','); ?>" /></td>
                        <td><input type="text" class="txt txtGST" name="txtgsttype[]" value="<?php echo $v->gst_type; ?>" /></td>
                        <td><input type="text" class="txt number txtGSTValue" name="txtgstvalue[]" value="<?php echo number_format($v->gst_value, 2, '.', ','); ?>" />
                          <input type="hidden" class="txt number txtnppbno" name="txtnpbbno[]" value="<?php echo $v->npbbitem; ?>">
                        </td>
                      </tr>
                  <?php
                    }
                ...
                <tbody>
                  ...
                  if (!empty($get_data_jurnal1)) {
                    foreach ($get_data_jurnal1 as $z) {
                      ...
                      $Debit1 = 0;
                      $credit1 = number_format($z->Total * $z->Rate, 2, '.', '');
                  ...
                  if (!empty($get_data_jurnal2)) {
                    foreach ($get_data_jurnal2 as $x) {
                      ...
                      if ($x->chk == 'D') {
                        ...
                        $chk2 = 'value="C"';
                        $Debit2 = number_format($x->Total * $x->Rate, 2, '.', '');
                        $credit2 = "0";
                      } else {
                        $chk2 = 'value="C" selected';
                        $chk2x = 'value="D"';
                        $credit2 = number_format($x->Total * $x->Rate, 2, '.', '');
                        $Debit2 = "0";
                      }
                      $desc2 = $x->Uraian;
                  ...
                  <tr id="ds2">
                    ...
                    <td class="total">
                      <input type="text" name="total_jr[1]" value="<?php //echo number_format($Total2, 2, ".", ",");     
                                                                    ?>0.00" class="txt number jur_total" onkeyup="hitung_total()" onkeypress="return isNumber(event)">
                    </td>
                    ...
                    <td><input type="text" name="credit_jr[1]" value="<?php //echo number_format($credit2, 2, ".", ",");     
                                                                      ?>0.00" class="txt number jur_credit" onkeypress="return isNumber(event)"></td>
                  </tr>
                  ...
                  <tr id="ds3">
                    ...
                    <td>
                      ...
                      <input type="hidden" name="SubAccountId[2]" value="GOT" class="txt">
                    </td>
            ...
            <a class="btn btn-warning" href="<?php echo base_url(); ?>Sales_inv_factory"><i class="fa fa-warning"></i> Cancel</a>
            <?php if ($this->input->get('id') <> '') { ?>
              <a class="btn btn-primary  kanan" href="<?php echo base_url(); ?>Sales_inv_factory/print_report?id=<?php echo htmlspecialchars($this->input->get('id'), ENT_QUOTES); ?>" target="_blank"><i class="fa fa-print"></i> Print</a>
              <!--<a class="btn btn-danger kanan" href="<?php echo base_url(); ?>Sales_inv_factory/delete_transaction?id=<?php echo htmlspecialchars($this->input->get('id'), ENT_QUOTES); ?>" onclick="return confirm('Are you sure to delete this transaction?')" ><i class="fa fa-trash"></i> Delete</a>-->
            <?php } ?>
          </div>
        </div>
      </form>
