		...
		<?php 
			...
			if(!empty($_detail)){
				foreach ($_detail as $r) {
					...
					$el = array('EL'=>'Export Laden', 'EE' => 'Export Empty', 'LE' => 'Local Empty', 'LL'=>'Local Laden', 'IT'=>'Import Transhipment', 'RE'=>'Recall Container');
			...
			if(!empty($_detail2)){
				foreach ($_detail2 as $r) {
					...
					$el = array('EL'=>'Export Laden', 'EE' => 'Export Empty', 'LE' => 'Local Empty', 'LL'=>'Local Laden', 'IT'=>'Import Transhipment', 'LE' => 'Local Empty', 'EI' => 'Empty Import', 'RE'=>'Recall Container');
					echo "<td><input type='hidden' name='jenis_stuffing[]' class='txt' value='$r->stuffing' readonly>".$el[$r->stuffing]."</td>";
					echo "</tr>";
					$i++;
				}
