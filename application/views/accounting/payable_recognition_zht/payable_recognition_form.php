...
<div class="page-content">
  <div class="container">
    ...
    <div class="row">
      <form action="<?php echo base_url(); ?>Payable_recognition_zht/save_payable_rec" name="form1" id="form1" method="post" enctype="multipart/form-data">
        <div class="col-md-12">
          ...
          <div class="portlet light">
            <div class="portlet-title">
              ...
              <div class="caption">
                ...
                <span class="caption-subject theme-font">Payable Recognition ZHT</span>
              </div>
              <div class="form-group">
                <?php if ($this->input->get('id') <> '') { ?>
                  <a class="btn btn-primary kanan" href="<?php echo base_url(); ?>Payable_recognition_zht/add_new"><i class="fa fa-plus"></i> Create New</a>
                <?php } ?>
              </div>
            </div>
            <?php if ($bayar > 0) { ?>
            ...
            <div class="portlet-body">
                ...
                <div class="row">
                  ...
                  <div class="col-md-6">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-3">
                        <input type="text" name="tgl_invoice" id="tanggal_invoice" class="form-control date target" onchange="get_cur_purchase();Rate_notfound();" value="<?php echo $date_invoice; ?>" data-date-format="dd/mm/yyyy" <?php echo $readonly; ?> required />
                      </div>
                    </div>
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-3">
                        <select name="type_note" id="type_note" class="form-control">
                          <option <?php if($type_note =='Invoice') { echo "selected";} ?> value="Invoice">Invoice</option>
                          <option <?php if($type_note =='Debit') { echo "selected";} ?> value="Debit">Debit Note</option>
                          <option <?php if($type_note =='Credit') { echo "selected";} ?> value="Credit">Credit Note</option>
                        </select>
                      </div>
                    </div>
                    <div class="form-group">
              ...
              <div class="table-scrollable-borderless">
                <table class="table table-bordered scrollable" id="tabel">
                  <thead>
                    <tr>
                      <th width="5px">
                  ...
                  <tbody>
                    ...
                    ?>
                        <tr>
                          ...
                          <td>
                            <input type="text" name="txtdept[]" value="<?php echo $v->dept_code; ?>" class="dept_code txt txtdept">
                          </td>
                          <td><textarea name="txtItem[]" rows="1" cols="30" id="namacoa<?php echo $m++; ?>" class="txt txtNmCOA"><?php echo $v->Items; ?></textarea></td>
                          <td><input type="text" name="txtContainerNo[]" class="txt txtContainerNo" value="<?php echo $v->containerNo; ?>" /></td>
                          <td>
                            <?php
                            $style_kategori = 'class="select2mecus txt"  id="customer"';
                            echo form_dropdown('customerID[]', $CostumerID, $v->customerID, $style_kategori);
                            ?>
                          </td>
                          <td><input type="text" class="txt number quantity" name="txtQty[]" value="<?php echo number_format($v->Qty, 2, '.', ','); ?>" onKeyup="hitung_amount()" onkeypress="return isNumber(event)" /></td>
                          <td>
                            <input type="text" name="txtUnit[]" class="txt" value="<?php echo $v->Unit; ?>" />
                            <input type="hidden" name="txtSummary[]" class="txt txtSummary" />
                          </td>
                          <td><input type="text" class="txt number prices" name="txtPrice[]" value="<?php echo number_format($v->Harga, 2, '.', ','); ?>" onKeyup="hitung_amount()" /></td>
              ...
              <table class="table table-bordered" id="table_jurnal">
                ...
                <tbody>
                  ...
                  <tr id="ds1">
                    ...
                    <td>
                      <select name="dk[]" onchange="hitung_total()" id="dk0" class="txt dk">
                  ...
                  <tr id="ds6">
                    ...
                    <?php endif; ?>
                    <td>
                      <select name="dk[5]" onchange="hitung_total()" id="dk5" class="txt dk">
              ...
              <a class="btn btn-warning" href="<?php echo base_url(); ?>Payable_recognition_zht"><i class="fa fa-warning"></i> Cancel</a>
              <?php if ($this->input->get('id') <> '') { ?>

                <?php if ($bayar == 0) { ?>
                  <a class="btn btn-danger kanan" href="<?php echo base_url(); ?>Payable_recognition_zht/delete_transaction?id=<?php echo htmlspecialchars($this->input->get('id'), ENT_QUOTES); ?>" onclick="return confirm('Are you sure to delete this transaction?')"><i class="fa fa-trash"></i> Delete</a>
              <?php }
              }
              ?>
...
<?php
$style_kategori = 'class="select2mecus txt"  id="customer"';
$custlist = form_dropdown('customerID[]', $CostumerID, [], $style_kategori);
?>
...
<script>
  ...
  $(function() {
    $('.no_coa').autocomplete({
      serviceUrl: "<?php echo site_url('Payable_recognition_zht/get_coa'); ?>"
    });
  });
  ...
  function hitung_total() {
    ...
    var type = document.getElementById('type_note').value;
    ...
    if (type === "Credit") {
      if (dk[0].value === "C") {
        ...
        var selisih = (sum_crt - sum_dbt).toFixed(2);
      }
    }else{
      if (dk[0].value === "D") {
        var selisih = (sum_dbt - sum_crt).toFixed(2);
      ...
      if (sgd_txt[i].value === 0) {
        ...
        if (gst_type[i].value === 'GST') {
          if (tahun > '2023') {
            ...
            var Amount = qty[i].value * harga[i].value;
            txtsummary[i].value = Amount;
            gst_value[i].value = (Amount * 8 / 100).toFixed(2);
  ...
  function ambilnew(x) {
    ...
    for (var i = 0; i < num; i++) {
      var getVendor = `<tr><td style="text-align: center;"><button class="tombol" onclick="hapus_dp(this)" >Remove</button></td>
                  <td><input type="hidden" name="Detail_item_id[]" value="0" /><input type="hidden" name="Detail_jurnal_id[]" value="0" /><input type="text" name="Detail_po[]" class="txt" value="" /></td>
                  ...
                  <td><input name="txtContainerNo[]" type="text" value="" class="txt txtContainerNo"></td>
                  <td><?php echo $custlist;?></td>
                  <td><input type="text" class="txt number quantity autonumber auto" name="txtQty[]" placeholder="0" onClick="make_blank()" onkeypress="return isNumber(event);" onKeyup="hitung_amount()" required/></td>
                  <td><input type="text" name="txtUnit[]" class="txt" /><input type="hidden" name="txtSummary[]" class="txt txtSummary" value="0" /></td>
                  ...
                  <td><input type="text" name="txtAmount[]" class="txt number amount autonumber" onKeyup="hitung_total()" data-a-sep="," data-a-dec="."  value="0"  onkeypress="return isNumber(event)"  value="0" /></td>
                   <input type="hidden" name="txtRate[]" class="txt number autonumber" onkeypress="return isNumber(event)"  value="' + rate + '" />
                   <td><input type="text" name="txtSGD[]" class="txt number txtSGD autonumber" onkeypress="return isNumber(event)"  value="0"  /></td>
                  <td><select name="txtGST[]" onchange="cek_gst()" class="txt txtGST">
                        <option value="">Select</option>
                        <option value="GST">GST</option>
        ...
        $('table[id="tabel"]').append(getVendor);

      //   $('.select2mecus').select2({
      //   placeholder: "",
      //   allowClear: true
      // });
    }
  ...
  function tambah_baris() {
    $.ajax({
      url: "<?php echo base_url(); ?>index.php/Payable_recognition_zht/list_currency",
  ...
  function hapus_atch_delete(btn) {
    ...
    $.ajax({
      type: "post",
      url: "<?= site_url('Payable_recognition_zht/delete_file/') ?>",
      data: {
        id: id,
      },
      dataType: "json"
  ...
  function ambil_tabel() {
    ...
    $.ajax({
      url: "<?php echo base_url(); ?>Payable_recognition_zht/data_dp?id=" + supp + "&currency=" + curency,
      success: function(response) {
        $(".err").html(response);
      },
      dataType: "html"
  ...
  function cek_nofak() {
    ...
    $.ajax({
      url: "<?php echo base_url(); ?>Payable_recognition_zht/cek_tabel_ap?id=" + refno,
      success: function(response) {
        $(".CurID").html(response);
      },
      dataType: "html"
  ...
  $("#btnFindRecord").click(function() {
    $.post("<?php echo site_url(); ?>Payable_recognition_zht/selectInvoicePayable", function(data) {
      $('#contentFindAP').html(data);
    });
    $('#modal-findAP').modal('show');
  });

  $('#type_note').change(function(e) {
    e.preventDefault();
    var value = $(this).val();
    // alert(value);
    if (value == 'Credit') {
