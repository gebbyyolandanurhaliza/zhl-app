<?php
    ...
    foreach($get_data_detail as $ll){
        $gst_type= $ll->gst_type;
        $totalgst += $ll->gst_value;
    }
    if($gst_type == 'GST'){
            $pdf->SetFont('Times','',10);
             $pdf->SetFont('Times','',12);
            $pdf->SetX(20);
            $pdf->Cell(36,4,'Description');
            $pdf->SetX(140);
            $pdf->Cell(36,4,'Total USD',0,0,'R');
            $pdf->Ln();
            $pdf->Line(10, $pdf->GetY(), 250 - 50, $pdf->GetY());
            $pdf->Ln();
            $pdf->SetX(20);
            $pdf->Cell(36,4,'Freight Charges');
            $pdf->SetX(140);
        ...
        $pdf->SetFont('Times','',12);
            ...
            $pdf->Cell(36,4,'Freight Charges');
            $pdf->SetX(140);
            $pdf->Cell(36,4,number_format($hutang,3),0,0,'L');
    }
    $pdf->AddPage();
    $pdf->ln(10);
    $pdf->SetFont('Times','B',10);
    $pdf->Cell(20, 5, 'Item', 'BT', 0, 'C');
    $pdf->Cell(80, 5, 'Description', 'BT', 0, 'C');
    $pdf->Cell(40, 5, 'Unit', 'BT', 0, 'R');
    $pdf->Cell(25, 5, 'Unit Price', 'BT', 0, 'R');
    $pdf->Cell(27, 5, 'Total (USD)', 'BT', 0, 'R');
    $pdf->ln(5);
    $pdf->setFont('Arial', '', 9);
    foreach($get_data_detail as $l){
        ...
        $pdf->Ln();
        $max = $pdf->NbLines(60,$l->description. "   ");
        $y8 = $pdf->GetY();
