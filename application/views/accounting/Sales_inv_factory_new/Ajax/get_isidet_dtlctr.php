		...
		<?php 
			...
			if(!empty($_detail)){
				foreach ($_detail as $r) {
					...
					$el = array('EL'=>'Export Laden', 'ELCN' => 'EXPORT LADEN (CN)', 'EE' => 'Export Empty', 'EECN' => 'Export Empty CN', 'LE' => 'Local Empty','LC' => 'Local Laden (LC)', 'LL'=>'Local Laden', 'IT'=>'Import Transhipment','ITCN'=>'Import Transhipment CN', 'RE'=>'Recall Container', 'EE_TP' => 'Export Empty (TP)');
			...
			if(!empty($_detail2)){
				foreach ($_detail2 as $r) {
					...
					$el = array('EL'=>'Export Laden','ELCN' => 'EXPORT LADEN (CN)', 'EE' => 'Export Empty', 'LE' => 'Local Empty', 'LL'=>'Local Laden', 'IT'=>'Import Transhipment', 'LE' => 'Local Empty', 'EI' => 'Empty Import', 'RE'=>'Recall Container', 'EE_TP' => 'Export Empty (TP)');
					echo "<td><input type='hidden' name='jenis_stuffing[]' class='txt' value='$r->stuffing' readonly>".$el[$r->stuffing]."</td>";
					echo "</tr>";
					$i++;
				}
			...
			if(!empty($_detailtet)){
				foreach ($_detailtet as $r) {
					...
					echo "<td><input type='text' name='container_number2[]' class='txt' value='$r->container' readonly></td>";
					echo "<td><input type='text' name='seal_number2[]' class='txt' value='$r->seal' readonly></td>";
					$el = array('EL'=>'Export Laden', 'EECN' => 'Export Empty (CN)', 'LE' => 'Local Empty', 'LL'=>'Local Laden', 'ITCN'=>'Import Transhipment (CN)', 'LE' => 'Local Empty', 'EI' => 'Empty Import', 'RE'=>'Recall Container', 'ELCN' => 'EXPORT LADEN (CN)');
					echo "<td><input type='hidden' name='jenis_stuffing[]' class='txt' value='$r->stuffing' readonly>".$el[$r->stuffing]."</td>";
					echo "</tr>";
					$i++;
				}
