...
class PDF extends FPDF 
{
    ...
    function Content($_tampil_item,$Get_vendor){
        $this->setFillColor(255, 255, 255);
        if (!empty($_tampil_item)) {
            foreach ($Get_vendor as $key_vendor) {
                ...
                $this->cell(275, 6, $key_vendor->namacustomer,0, 1, 'L', 1);
                $this->cell(275, 6, $key_vendor->address,0, 1, 'L', 1);
                $this->setFont('Arial', 'B', 6);
                ...
                foreach ($_tampil_item as $key_item) {
                    ...
                    if ($key_item->kode_sup == $key_vendor->kode_sup) {
                        ...
                        $this->cell(15, 6, $key_item->currency, 'BTRL', 0, 'L', 0);
                        ...
                        $this->cell(15, 6, number_format($key_item->piutang, 2), 'BTRL', 0, 'R', 1);
...
$pdf->AliasNbPages();
$pdf->AddPage();
$pdf->Content($_tampil_item,$Get_vendor);
$pdf->Output();
