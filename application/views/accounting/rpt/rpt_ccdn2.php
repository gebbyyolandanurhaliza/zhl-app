...
class PDF extends TFPDF {
    ...
    function Header() {
        ...
        $this->setFillColor(255, 255, 255);
        $this->Cell(85);
        $this->cell(25, 6, ' Debet / Credit Note List - '.date('M Y', strtotime($this->dari)), 0, 0, 'C', 1);
        $this->Ln(9);
    ...
    function Content($List_ccdn) {
        foreach ($List_ccdn as $s) {
            ...
            $this->cell(15, 6, $s->currency_rate,  '', 0, 'L', 1);
            if($s->currency == 'USD'){
                $this->cell(15, 6, number_format($s->total-$s->gst_value, 2), '', 0, 'R', 1);
            }else{
                $this->cell(15, 6, '-', '', 0, 'C', 1);
            }
            if($s->gst_value > '0'){
                $this->cell(15, 6, number_format($s->gst_value, 2), '', 0, 'R', 1);
            }else{
                $this->cell(15, 6, '-', '', 0, 'C', 1);
            }
            if($s->currency == 'USD'){
                $this->cell(15, 6, number_format($s->total, 2), '', 0, 'R', 1);
            }else{
                $this->cell(15, 6, '-', '', 0, 'C', 1);
            }
            if($s->currency == 'SGD'){
            ...
            if($s->currency == 'SGD'){
                $this->cell(15, 6, number_format($s->total, 2), '', 1, 'R', 1);
            }else{
        ...
        foreach ($List_ccdn as $m) {
            // $amount_usd  += $m->total;
            if ($m->currency == 'USD'){
                $amount_usd += $m->total-$m->gst_value;
            }
            if ($m->gst_value > '0.0000'){
                $gst_value_usd += $m->gst_value;
            }
            if ($m->currency == 'USD'){
                $total_usd += $m->total;
            }
            ...
            if ($m->currency == 'SGD'){
                $amount_sgd += $m->total-$m->gst_value;
            }
            if ($m->gst_value > '0.0000'){
                $gst_value_sgd += $m->gst_value;
            }
            if ($m->currency == 'SGD'){
                $total_sgd += $m->total;
            }
