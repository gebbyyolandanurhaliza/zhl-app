...
class PDF extends FPDF
{
    ...
    function Header()
    {
        ...
        $dari = $_GET['dari'];
        $sampai = $_GET['sampai'];
        $coa = isset($_GET['jenis_coa']) && !empty($_GET['jenis_coa']) ? $_GET['jenis_coa'] : $_GET['new_coa'];


        $titel = 'GENERAL LEDGER FOR THE PERIOD AT ' . $dari . ' TO ' . $sampai;
        $this->SetX(10);
        $this->Cell(200, 25, '', 0, 0);
        $this->Image('assets/zhl-kop.PNG', 3, 8, 235, 33);
        $this->Ln();
        $this->SetX(10);
    ...
    function Content($detail_trans)
    {
        ...
        foreach ($detail_trans as $s) {
            ...
            $this->SetWidths(array(20, 20, 20, 24, 38, 15, 15, 21, 15, 15, 24));
            ...
            if ($s->tmp_uraian == 'BEGINING BALANCE') {
                $this->Row(array(
                    $s->tmp_no_coa . " - " . $s->tmp_namaakun, str_replace(".", "/", $s->tmp_nojurnal), $tgl_jurnal, '', $s->tmp_uraian, $tmp_debet, $tmp_kredit,
                    str_replace("$", "", number_format($s->tmp_balance, 2, ',', '.')), $tmp_debet_sgd, $tmp_kredit_sgd,
                    "SGD " . str_replace("$", "", number_format($s->tmp_balance_sgd, 2, ',', '.'))
                ));
            } elseif ($s->tmp_uraian == 'TOTAL') {
                $this->Row(array(
                    '', str_replace(".", "/", $s->tmp_nojurnal), '', '', $s->tmp_uraian, $tmp_debet, $tmp_kredit,
                    str_replace("$", "", number_format($s->tmp_balance, 2, ',', '.')), $tmp_debet_sgd, $tmp_kredit_sgd,
                    "SGD " . str_replace("$", "", number_format($s->tmp_balance_sgd, 2, ',', '.'))
                ));
            } elseif ($s->tmp_uraian != 'TOTAL' || $s->tmp_uraian != 'BEGINING BALANCE') {
                $this->Row(array(
                    '', str_replace(".", "/", $s->tmp_nojurnal), $tgl_jurnal, $s->tmp_check_bank, $s->tmp_uraian, $tmp_debet, $tmp_kredit,
                    str_replace("$", "", number_format($s->tmp_balance, 2, ',', '.')), $tmp_debet_sgd, $tmp_kredit_sgd,
                    "SGD " . str_replace("$", "", number_format($s->tmp_balance_sgd, 2, ',', '.'))
                ));
            }
        ...
        /*$this->SetWidths(array(15,25, 53, 21, 21, 21, 21, 21, 24, 24));
            $this->Row(array( "", "", "Total ", number_format($tDebet, 2), number_format($tKredit, 2), str_replace("$", "", number_format($tBalance)), number_format($tDebetSGD, 2), number_format($tKreditSGD, 2), "SGD ".str_replace("$", "", number_format($tBalanceSGD)) ));
    */
    }
    private $widths;
    private $aligns;
