<script>
  ...
  function get_vendor() {
    ...
    $.ajax({
      url: "http://localhost/pss/index.php/PO_journal/tampilpo/" + po + "",
...
if (!empty($get_data_header)) {
  foreach ($get_data_header as $l) {
    ...
    $kode_sup = $l->kode_sup;
    $currency_id = $l->rate . "|" . $l->currency_id;
    $Currency_symbol = $l->currency_id;
    $rate = $l->rate;
...
<div class="page-content">
  <div class="container">
    ...
    <div class="row">
      <form action="<?php echo base_url(); ?>index.php/po_journal/save_jurnal_invoice" method="post" id="from" name="from">
        <div class="col-md-12">
            ...
            <div class="portlet-body">
                  ...
                  <div class="col-md-6">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-3">
                        ...
                        if ($dt == 0) {
                        ?>
                          <input type="text" id="tgl_tempo" name="tgl_tempo" class="form-control date date-picker" value="<?php echo $date_of_journal; ?>" onchange="getTerm()" data-date-format="mm/dd/yyyy" <?php echo $readonly; ?> required />
                        <?php
                        } else {
                        ?>
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-3">
                        ...
                        if ($dt == 0) {
                        ?>
                          <input type="text" id="tgl_invoice" name="tgl_invoice" class="form-control date date-picker" value="<?php echo $date_invoice; ?>" onchange="getTerm()" data-date-format="mm/dd/yyyy" <?php echo $readonly; ?> required />
                        <?php
                        } else {
                        ?>
                  ...
                  ?>
                    <tbody>
                      ...
                      ?>
                        <tr>
                          <td></td>
                          <td><input type="text" name="txtidem[]" readonly class="txt" value="<?php echo $r->ItemID; ?>" required /><input type="hidden" name="npbbno[]" class="txt" value="<?php echo $r->HeaderID; ?>" required /></td>
                  ...
                  <tbody>
                    ...
                    if (!empty($get_data_footer)) {
                      ...
                      foreach ($get_data_footer as $f) {
                        if ($f->NoUrut == '1') {
                          $DetailID1 = $f->DetailID;
                          $NoCOA1 = $f->NoCOA;
                          $Uraian1 = $f->Uraian;
                          $Total1 = number_format($f->Total);
                          $Rate1 = number_format($f->Rate);
                          $Debet1 = number_format($f->Debet);
                          $Kredit1 = number_format($f->Kredit);
                          $sum_debet += $f->Debet;
                          $sum_credit += $f->Kredit;
                        }
                        if ($f->NoUrut == '2') {
                          $DetailID2 = $f->DetailID;
                          $NoCOA2 = $f->NoCOA;
                          $Uraian2 = $f->Uraian;
                          $Total2 = number_format($f->Total);
                          $Rate2 = number_format($f->Rate);
                          $Debet2 = number_format($f->Debet);
                          $Kredit2 = number_format($f->Kredit);
                          $sum_debet += $f->Debet;
                          $sum_credit += $f->Kredit;
                        }
                        if ($f->NoUrut == '3') {
                          $DetailID3 = $f->DetailID;
                          $NoCOA3 = $f->NoCOA;
                          $Uraian3 = $f->Uraian;
                          $Total3 = number_format($f->Total);
                          $Rate3 = number_format($f->Rate);
                          $Debet3 = number_format($f->Debet);
                          $Kredit3 = number_format($f->Kredit);
                          $sum_debet += $f->Debet;
                          $sum_credit += $f->Kredit;
                        }
                        if ($f->NoUrut == '4') {
                          $DetailID4 = $f->DetailID;
                          $NoCOA4 = $f->NoCOA;
                          $Uraian4 = $f->Uraian;
                          $Total4 = number_format($f->Total);
                          $Rate4 = number_format($f->Rate);
                          $Debet4 = number_format($f->Debet);
                          $Kredit4 = number_format($f->Kredit);
                          $sum_debet += $f->Debet;
                          $sum_credit += $f->Kredit;
                        }
                        if ($f->NoUrut == '5') {
                          $DetailID5 = $f->DetailID;
                          $NoCOA5 = $f->NoCOA;
                          $Uraian5 = $f->Uraian;
                          $Total5 = number_format($f->Total);
                          $Rate5 = number_format($f->Rate);
                          $Debet5 = number_format($f->Debet);
                          $Kredit5 = number_format($f->Kredit);
                          $sum_debet += $f->Debet;
                          $sum_credit += $f->Kredit;
                        }
                        if ($f->NoUrut == '6') {
                          $DetailID6 = $f->DetailID;
                          $NoCOA6 = $f->NoCOA;
                          $Uraian6 = $f->Uraian;
                          $Total6 = number_format($f->Total);
                          $Rate6 = number_format($f->Rate);
                          $Debet6 = number_format($f->Debet);
                          $Kredit6 = number_format($f->Kredit);
                          $sum_debet += $f->Debet;
                          $sum_credit += $f->Kredit;
                        }
                      }
              ...
              <table class="table table-bordered" id="table_total">
                <tr>
                  <td>TOTAL DEBET</td>
                  <td> : </td>
                  <td><input readonly type="text" id="total_debet" name="total_debet" value="<?php echo $sum_debet; ?>" class="form-control" onkeypress="return isNumber(event)" required /></td>
                  <td>&nbsp;
                  <td>TOTAL CREDIT</td>
                  <td> : </td>
                  <td><input readonly type="text" id="total_credit" name="total_credit" value="<?php echo $sum_credit; ?>" class="form-control" onkeypress="return isNumber(event)" required /></td>
                </tr>
              </table>
              <hr />
            ...
            <tbody width="100%">
              ...
              ?>
                    <tr id="sour<?php echo $no++; ?>">
                      ...
                      <td><input type="text" onKeyup="debit()" name="txtqty[]" onkeypress="return isNumber(event)" class="txt number txtqty" value="<?php echo $s->qty; ?>" /></td>
                      ...
                      <td><input type="text" name="txtgrand[]" readonly class="txt number txtgrand" value="<?php echo $s->total * $s->rate; ?>" /><input type="hidden" name="txtnpbb[]" readonly class="txt" value="<?php echo $s->npbbno; ?>" /></td>
                    </tr>
              <?php
                  }
                }
