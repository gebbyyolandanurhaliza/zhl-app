      ...
      <form action="<?php echo base_url(); ?>Purchase_inv_vendor/save_receivable_rec" name="form1" id="form1" method="post">
        <div class="col-md-12">
          ...
          <div class="portlet light">
                ...
                <div class="row">
                  <div class="col-md-6">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-4">
                        <div id="cur_id">
                          <?php
                          // onchange='get_cur_purchase();Rate_notfound();'
                  ...
                  <div class="col-md-6">
                    <div id="daftar_kurs">
                      ...
                      <div class="form-group">
                        <label class="control-label col-md-3 col-md-offset-6">SGD Rate Nego</label>
                        <div class="col-md-3">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-3">
                        <!-- onchange="get_cur_purchase();Rate_notfound();"  -->
                        <input type="text" name="tgl_invoice" id="tanggal_invoice" class="form-control date target" value="<?php echo $date_invoice; ?>" data-date-format="dd/mm/yyyy" <?php echo $readonly; ?> required />
                      </div>
                    </div>
                    <div class="form-group">
                    ...
                    <div class="form-group">
                      ...
                      <div class="col-md-3">
                        <input type="text" id="tgl_tempo" name="tgl_jurnal" class="form-control input-sm " value="<?php echo $date_of_journal; ?>" data-date-format="dd/mm/yyyy" readonly required />
                      </div>
                    </div>
                  ...
                  ?>
                      <tr id="coa<?php echo $no++; ?>">
                        ...
                        <td><input type="hidden" class="txt number" name="txtQty1000[]" value="<?php echo $v->per1000; ?>" onClick="make_blank()" onkeypress="return isNumber(event);" onKeyup="hitung_total()" required />
                          <input type="text" class="txt number quantity" name="txtQty[]" value="<?php echo number_format($v->Qty, 2, '.', ','); ?>" onkeypress="return isNumber(event);" onKeyup="hitung_amount()" />
                        </td>
                        <td>
                        ...
                        <td><input type="hidden" name="txtRate[]" class="txt number autonumber txtrate" onkeypress="return isNumber(event)" value="<?php echo $v->rate; ?>" />
                          ...
                          <input type="text" class="txt number amount" name="txtamount[]" onkeypress="return isNumber(event);" onKeyup="hitung_total()" value="<?php echo number_format($v->Qty * $v->price, 2, '.', ','); ?>" />
                        </td>
                        ...
                        <td hidden="">
                          <input type="text" name="DtlId[]" class="txt" value="<?php echo $v->DtlIdGr; ?>" />
                        </td>
                      </tr>
                  <?php
                    }
            ...
            <a class="btn btn-info" id="cetak" onclick="bukaurl()"><i class="fa fa-print"></i> Print</a>
            <?php if ($this->input->get('id') <> '') { ?>
              <!-- 
...
<!-- Find Recorded Purchase Inventory Vendor Modal -->
...
<script type="text/javascript">
  ...
  $("#btnFindRecord").click(function() {
    $.post("<?php echo site_url(); ?>Purchase_inv_vendor/selectInvoiceVendor", function(data) {
...
<script type="text/javascript">
  function bukaurl() {
    ...
    var cur = "<?php echo $currency_id; ?>";

    var url = "<?php echo base_url(); ?>Purchase_inv_vendor/print_report?id=" + refno + "&cur=" + cur;
    // alert(url);
...
<script>
  ...
  function hitung_total() {
    ...
    for (var i = 0; i < price.length; i++) {
      ...
      var disc_amount = amount[i].value * txtDisc[i].value / 100;
      txtDiscDol[i].value = (amount[i].value - disc_amount).toFixed(2);
    }
  ...
  $('#tanggal_invoice').datepicker().on('changeDate', function() {
    var currency_id = document.getElementById('currency').value;
    var dateInv = document.getElementById('tanggal_invoice').value;
    var supplier = document.getElementById('supplier').value;
    get_cur_purchase(currency_id, dateInv, supplier);
  });
  ...
  function get_cur_purchase(currency_id, dateInv, supplier) {

    document.getElementById('tgl_tempo').value = dateInv;
    var tgl = dateInv.split("/");
    var tahun = tgl[2];
    var bulan = tgl[1];

    var belah = supplier.split("|");
    var vendor = belah[0];

    $.ajax({
      url: "<?php echo base_url(); ?>Purchase_inv_vendor/ambil_currency?cur=" + currency_id + "&date=" + dateInv + "",
    ...
    $.ajax({
      url: "<?php echo base_url(); ?>Purchase_inv_vendor/tampil_po?supplier=" + vendor + "&currency=" + currency_id + "",
      success: function(response) {
    ...
    Rate_notfound(currency_id, dateInv);
  }

  function Rate_notfound(currency_id, dateInv) {
    $.ajax({
      url: "<?php echo base_url(); ?>Purchase_inv_factory/accounting_rate?cur=" + currency_id + "&date=" + dateInv + "",
      success: function(response) {
        $("#rate2").html(response);
      },
      dataType: "html"
    ...
    for (var i = 0; i < gst_type.length; i++) {
      if (sgd_txt[i].value === 0) {
        ...
        if (gst_type[i].value === 'GST') {
          ...
          // gst_value[i].value = (total * 8 / 100).toFixed(2);
        } else {
          gst_value[i].value = '0';
  ...
  function ambil(x) {
    ...
    for (var i = 0; i < num; i++) {
      $('table[id="tabel"]').append('<tr><td><button class="tombol" onclick="hapus_dp(this)" >Remove</button></td>\n\
                    ...
                    <td><input type="text" class="txt number quantity" onKeyup="hitung_amount()" name="txtQty[]" value="0"></td>\n\
                    <td><input type="text" class="txt" name="txtunit[]" value="">\n\
                        <input type="hidden" name="txtSummary[]" class="txt txtSummary" value="0" /></td>\n\
                    <td><input type="text" class="txt number prices" name="txtunitprice[]" onKeyup="hitung_amount()" value="0"></td>\n\
                    <td><input type="text" class="txt number amount" name="txtamount[]" onKeyup="hitung_total()" value="0"></td>\n\
                    <td><input type="text" class="txt number dis_per" name="dis_per[]" onKeyup="hitung_disc()"  value="0"></td>\n\
                    <td><input type="text" class="txt number dis_dol" name="dis_dol[]" value="0"></td>\n\
      ...
      //        <td><input type="text" class="txt number quantity" onKeyup="hitung_total()" name="txtQty[]" value="' + $(this).find("td").eq(2).html() + '"></td>\n\
      //        <td><input type="text" class="txt" name="txtunit[]" value="' + $(this).find("td").eq(9).html() + '">\n\
      //            <input type="hidden" name="txtSummary[]" class="txt txtSummary" value="0" /></td>\n\
      //        <td><input type="text" class="txt number prices" name="txtunitprice[]" onKeyup="hitung_total()" value="' + $(this).find("td").eq(3).html() + '"></td>\n\
      //        <td><input type="text" class="txt number amount" name="txtamount[]" value="' + $(this).find("td").eq(4).html() + '"></td>\n\
      //        <td><input type="text" class="txt number dis_per" name="dis_per[]" onKeyup="hitung_disc()"  value="0"></td>\n\
