...
class PDF extends FPDF
{
    function Header()
    {
        ...
        $this->SetX(70);
        $this->Cell(0, 10, 'PROFORMA INVOICE', 0, 0);
        $this->Ln();
        $this->Ln();
        ...
        $this->SetX(21);
        $this->Cell(30, 10, 'Item Name', '', 0, 'C');
        $this->Cell(30, 10, 'UOM', '', 0, 'R');
        $this->Cell(25, 10, 'Quantity', '', 0, 'R');
        $this->Cell(24, 10, 'Unit Price', '', 0, 'R');
        $this->Cell(24, 10, 'Nett Weight', '', 0, 'R');
        $this->Cell(24, 10, 'Gross Weight', '', 0, 'R');
        $this->Cell(18, 10, 'Total', '', 0, 'R');
...
foreach ($_getSO as $r) {
    $totalqty += $r->qty;
    $totNettWeight += $r->NettWeight;
    $totGrossWeight += $r->GrossWeight;
    ...
    $pdf->SetXY(89, $y);
    $pdf->MultiCell(20, 5, number_format($r->qty, 2), 0, 'C');
    $pdf->SetXY(112, $y);
    $pdf->MultiCell(20, 5, number_format($r->unitprice, 2), 0, 'C');
    $pdf->SetXY(125, $y);
    $pdf->MultiCell(24, 5, number_format($r->NettWeight, 2), 0, 'R');
    $pdf->SetXY(147, $y);
    ...
    $y2 = $pdf->GetY() - 5;
    $pdf->SetXY(21, $y2);
    $pdf->MultiCell(50, 5, $r->itemname, 0, 'L');
    $pdf->Ln();
    ...
    $pdf->cell(10, 10, ': ', '', 0, 'L');
    $pdf->SetXY(163, $y7 + 12);
    $pdf->cell(34, 10, ($taxprice > 0 ? number_format($taxprice, 2) : '-'), '', 0, 'R');
    $pdf->Ln();
    $pdf->SetXY(40, $y7 + 16);
    $pdf->cell(10, 10, ': ', '', 0, 'L');
    $pdf->SetXY(163, $y7 + 16);
