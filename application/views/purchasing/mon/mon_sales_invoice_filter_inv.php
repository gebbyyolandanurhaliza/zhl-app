...
foreach ($inv as $r) {
    ...
    if ($invno != $r->invno) {
        echo '<td nowrap><a class="btn-sm btn-default" target="_blank" href="' . site_url('purchasing_inv/sales_invoice_print?inv=' . $r->invno) . '"><i class="fa fa-eye"></i></a></td>';
    ...
    if ($r->per1000 == 1) {
        echo '<td nowrap class="text-right">' . number_format(($r->qty / 1000) * $r->unitprice, 2) . '</td>';
    } else {
        echo '<td nowrap class="text-right">' . number_format($r->qty * $r->unitprice, 2) . '</td>';
    }
    echo '<td nowrap class="text-right">' . number_format($r->invoiceprice, 4) . '</td>';
