      ...
      <div class="col-md-12">
        ...
        <div class="portlet light">
          ...
          <div class="portlet-body">
            <form action="<?php echo site_url('purchasing_lr/save_lr/update'); ?>" method="post" class="form-horizontal" role="form">
              <div class="row">
                <div class="col-md-9">
                  <div class="form-body">
                    <div class="row">
                        ...
                        <thead>
                          <tr>
                            <th><?php if (trim($type_trans) != 'DP') { ?>
                                <button class="btn btn-sm btn-primary" type="button" onclick="fnDialogPO()" id="btn-click"><i class="fa fa-arrow-down"></i></button>
                              <?php } else { ?>
                                <button class="btn btn-sm btn-primary" type="button" onclick="addrow()" id="btn-dp">DP</button>
                              <?php
                                }
                              ?>
                            </th>
                          ...
                          <?php foreach ($lr as $x) { ?>
                            ...
                            ?>
                              ...
                              <td><?php if (trim($x->type_trans) != 'DP') { ?>
                                ...
                                <?php } else { ?><a class="btn btn-danger" href="<?php echo site_url('purchasing_lr/loading_report_delete_item_dp?itemid=' . $x->itemid . '&lrno=' . $x->lrno); ?>" onclick="javasciprt: return confirm('Are you sure delete this item?')"><i class="fa fa-trash"></i></a><?php } ?>
                              </td>
                  ...
                  <div class="form-actions">
                    ...
                    <div class="col-md-6">
                      ...
                      <a type="button" class="col-md-2 col-md-push-9 btn btn-info" href="<?php echo site_url('purchasing_lr/print_report_lr?lr=' . $lrno); ?>" target="_blank">Print</a>
                    </div>
                  </div>
                </div>
                <div class="col-md-3">
...
<script>
      ...
      if (chk_arr[k].checked == true) {
        $('table[id="tblList"]').append('<tr onclick="deleterow(this)">\n\
                        <td><button class="btn btn-sm btn-danger" type="button" ><i class="fa fa-trash" ></i></button></td>\n\
                            ...
                            <td hidden nowrap onclick="event.stopPropagation();return false;"><input type="text" class="form-control input-sm" style="width: 140px;" name="txtItemNPBB[]" value="' + getText(document.getElementById('tbl-po').rows[i].cells[7]) + '" readonly></td>\n\\n\
  ...
  function filterporeplace() {
    ...
    $.ajax({
      url: "<?php echo base_url(); ?>purchasing_lr/loading_report_po?cust=" + $cust + "&po=" + $findpo + "",
      success: function(response) {
        $("#tblpo").html(response);
      },
      dataType: "html"
  ...
  function filterinv() {
    ...
    $.ajax({
      url: "<?php echo base_url(); ?>purchasing_lr/loading_report_inv?inv=" + $findinv + "",
      success: function(response) {
...
<script type="text/javascript">
  ...
  function modal_delete(data) {
    $.ajax({
      url: "<?php echo base_url(); ?>purchasing_lr/remove_loading_report?delete=" + data + "",
      success: function(response) {
