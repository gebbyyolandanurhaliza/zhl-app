...
//		$this->Line(10, $this->GetY(), 200, $this->GetY());
		...
		$this->Cell(45, 7, 'Product / Packing', 'TLR', 0, 'L', true);
		$this->Cell(20, 7, 'Brand', 'TLR', 0, 'C', true);
//		$this->Cell(15, 7, 'Cus Ref', 'TLR', 0, 'C', true);
		$this->Cell(30, 7, 'Pack Size', 'TLR', 0, 'C', true);
...
//		$this->MultiCell($w, $h, $txt, $border, $align, $fill)
		...
		foreach ($record_detail as $r) {
			...
			if ($r->packing_view){
				$packingview = $r->packing_view;
			} else {
				$packingview =  floatval($r->uom_volume).' '.$r->uom_volume_name.' per '.$r->cma_uom_quantity_id;
			}

			$arr_cell_height = array(
				$this->NbLines(8, $no),
				$this->NbLines(45, $r->product_name),
				$this->NbLines(20, $r->detail_brand_name),
				$this->NbLines(30, $packingview),
				$this->NbLines(20, $record_header->currency_id.' 0'),
				$this->NbLines(27, $record_header->currency_id.' 0'),
			);
			...
			$this->SetXY(18, $yval);
			$this->MultiCell(45, ($hmax / $arr_cell_height[1]) * 5, $r->product_name, 'LB', 'L', false);

			$this->SetXY(10+8+45, $yval);
			$this->MultiCell(20, ($hmax / $arr_cell_height[2]) * 5, $r->detail_brand_name, 'LB', 'C', false);

//			$this->SetXY(78, $yval);
//			$this->MultiCell(15, ($hmax / $arr_cell_height[0]) * 5, $r->customer_reference, 'LB', 'C', false);

			$this->SetXY(10+8+45+20, $yval);
			$this->MultiCell(30, ($hmax / $arr_cell_height[3]) * 5, $packingview, 'LB', 'C', false);

			$this->SetXY(10+8+45+20+30, $yval);
			$this->MultiCell(20, ($hmax / $arr_cell_height[0]) * 5, $r->quantity, 'LB', 'C', false);

			$this->SetXY(10+8+45+20+30+20, $yval);
			$this->MultiCell(20, ($hmax / $arr_cell_height[0]) * 5, $r->uom_quantity_name, 'LB', 'C', false);

			$this->SetXY(10+8+45+20+30+20+20, $yval);
			$this->MultiCell(20, ($hmax / $arr_cell_height[4]) * 5, $record_header->currency_id.' 0', 'LB', 'R', false);

			$this->SetXY(10+8+45+20+30+20+20+20, $yval);
			$this->MultiCell(27, ($hmax / $arr_cell_height[5]) * 5, $record_header->currency_id.' 0', 'LBR', 'R', false);
...
//			$this->Ln();
		...
		$this->Cell(40, 5, $record_header->currency_id.' 0', 0, 1, 'R');
...
//		$this->Cell($w, $h, $txt, $border, $ln, $align, $fill)
		...
		$payterm = ($record_header->payment_term == null) ? $record_header->payment_terms : $record_header->payment_term;
		...
		if ($record_header->shipping_period1 == $record_header->shipping_period2){
			$shp_period = $record_header->shipping_period1;
		} else {
			$shp_period = $record_header->shipping_period1.' to '.$record_header->shipping_period2;
		}
		$this->setFont('Arial', '', 9);
		$this->Cell(210, 5, $shp_period, 0, 1 );

		$this->Ln(2);

		$this->setFont('Arial', 'BU', 9);
		$this->Cell(210, 5, 'Shipment Schedule  :', 0, 1 );
		$this->setFont('Arial', '', 9);
		$y_sc = $this->GetY();
		$this->SetXY(10, $y_sc);
		$this->MultiCell(210, 5, $record_header->shipment_schedule );
