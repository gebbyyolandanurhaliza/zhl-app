											...
											<tbody>
												...
												if ($search_result){
													foreach($search_result as $r){
                                                        ...
                                                        $quot_link = site_url('factory/generate-pdf/?id='.encode_str($r->quotation_hdr_id)).'&fid=' . $r->det_factory_id . '&no=' . encode_str($r->quotation_number) . '&dt=' . encode_str(date('d-M-Y', strtotime($r->document_date)));
...
//                                                            echo ($r->quotation_hdr_id > 0) ? "<a class='btn btn-xs pull-right' target='_blank' href='$quot_pdf' style='padding-right:0px;' title='View PDF'><i class='fa fa-file-pdf-o font-grey-gallery'></i></a>" : "";
														...
														if($r->factory_abbr == 'RSUP')
														{
															$viewed = strtoupper($r->downloadedby).' - '.$r->downloadeddate;	
														}
														else
														{
															$viewed = strtoupper($r->downloadedbypsg).' - '.$r->downloadeddatepsg;		
														}
														// $viewed = strtoupper($r->downloadedby);
														echo "<td>$viewed</td>";
														echo "</tr>";
													}
												}
