...
class PDF extends FPDF
{
	...
	function header(){
	    ...
	    $this->setFillColor(255, 255, 255);
	    $this->Cell(90);
	    $this->cell(20, 6, 'Customer Card For Period '.date('d-m-Y', strtotime($this->from)).' to '.date('d-m-Y', strtotime($this->to)), 0, 0, 'C', 1);
	    $this->Ln();
		...
		$this->cell(15,6, 'Amount(SGD)', 1, 0,'C',1);
		$this->cell(15,6, 'GST(SGD)', 1, 0,'C',1);
	...
	function Content($_getInv){
		foreach($_getInv AS $m)
		{
			...
			$this->cell(20, 6, $m->ponumber, 1, 0, 'L', 1);
			$this->cell(40, 6, $m->custcompany, 1, 0, 'L', 1);
			$this->cell(12, 6, $m->currency, 1, 0, 'L', 1);
			if($m->currency == 'USD'){
				$this->cell(15, 6, $m->maintotal, 1, 0, 'C', 1);
			}else{
				$this->cell(15, 6, ' ', 1, 0, 'C', 1);
			}

			if($m->currency == 'USD'){
				$this->cell(15, 6, $m->freight, 1, 0, 'C', 1);
			}else{
				$this->cell(15, 6, ' ', 1, 0, 'C', 1);
			}

			if($m->currency == 'USD'){
				$this->cell(10, 6, $m->gst, 1, 0, 'C', 1);
			}else{
				$this->cell(10, 6, ' ', 1, 0, 'C', 1);
			}

			if($m->currency == 'USD'){
				$this->cell(14, 6, $m->totaldue, 1, 0, 'C', 1);
			}else{
			...
			if($m->currency == 'SGD'){
				$this->cell(15, 6, $m->gst, 1, 0, 'C', 1);
			}else{
				$this->cell(15, 6, '-', 1, 0, 'C', 1);
			}

			if($m->currency == 'SGD'){
				$this->cell(10, 6, $m->totaldue, 1, 1, 'C', 1);
			}else{
				$this->cell(10, 6, '-', 1, 1, 'C', 1);
			}
