...
$objPHPExcel->setActiveSheetIndex(0)
    ->setCellValue('B2', 'BARGE - ' . $hdr->barge)
    ->setCellValue('B3', 'VOYAGE - ' . $hdr->voyage)
    ->setCellValue('F2', 'ETD - ' . $hdr->etd)
    ->setCellValue('F3', 'SHIPMENT DATE - ' . $hdr->shipmentdate)
...
foreach ($local  as $v) :
    ...
    $objPHPExcel->getActiveSheet()->getStyle('D' . $counter)->applyFromArray($style_col_angka);
    ...
    $objPHPExcel->setActiveSheetIndex(0)
        ...
        ->setCellValue('D' . $counter, '$ '. number_format($v->barge_cost, 2, ',', '.').'')
...
$objPHPExcel->getActiveSheet()->setTitle('LOCAL LADEN');
header('Last-Modified:' . gmdate("D, d M Y H:i:s") . 'GMT');
header('Chace-Control: no-store, no-cache, must-revalation');
header('Chace-Control: post-check=0, pre-check=0', FALSE);
header('Pragma: no-cache');
header('Content-Type: application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
header('Content-Disposition: attachment;filename="LOCAL-LADEN ' . $hdr->shipmentdate . '.xlsx"');
