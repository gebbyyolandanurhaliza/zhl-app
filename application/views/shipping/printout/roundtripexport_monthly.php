...
$objPHPExcel->setActiveSheetIndex(0)
    ->setCellValue('B2', ''. ' - Barge Billing (Round Trip)')
    ->setCellValue('B3', 'TOTAL SUMMARY -   (' . $p_dari . ' - ' . $p_sampai . ')')
...
endforeach;
$objPHPExcel->getActiveSheet()->getStyle('C' . $counter)->applyFromArray($style_col_right);
$objPHPExcel->getActiveSheet()->getStyle('D' . $counter)->applyFromArray($style_col_right);
...
header('Content-Disposition: attachment;filename="ROUNDTRIP-EXPORT (' . $p_dari . ') - (' . $p_sampai . ') .xlsx"');
