...
<script>
  ...
  function changerow(x, y) {
    ...
    $url = "<?php echo base_url(); ?>Shipping/get_shipmentdate?etd=" + $etd + "&id=" + x + "&flag=" + y;
    console.log($url);
    $.ajax({
      url: $url,
  ...
  function save_move() {
    ...
    if (confirm("Are you sure move this Shipment?") == true) {
      $url2 = "<?php echo base_url(); ?>shipping/container_inward_move?shipdate=" + $shipdate + "&id=" + $id + "&flag=" + $flag + "&et=" + $etd;
