                      ...
                      <?php foreach ($cont as $r) { ?>
                        <tr>
                          <td>
                            <?php
                            if ($r->proses != 1) { ?>
                              <button class="btn btn-sm btn-danger" type="button" id="btn-delete" onclick="deleterow(<?= $r->detail_id; ?>,<?= $r->shipid; ?>)"><i class="fa fa-trash"></i></button>
...
<script>
  ...
  function deleterow(x, y) {
    ...
    if (confirm("Are you sure remove this row?") == true) {
      ...
      $.ajax({
        url: "<?php echo base_url(); ?>shipping/container_shippping_delete_outward?stock=" + x + "&shipid=" + y,
