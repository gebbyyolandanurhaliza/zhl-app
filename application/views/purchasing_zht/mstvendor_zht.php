      ...
      <div class="col-md-12">
        ...
        <div class="portlet light">
          ...
          <div class="portlet-body form">
            <form action="<?php echo site_url('purchasing_zht/vendor_save/add'); ?>" method="post" class="form-horizontal" role="form">
              <div class="form-body">
                <div class="row">
                  <div class="col-md-12">
                    <div class="col-md-6">
                      <div class="form-group" style="margin-bottom:1px;">
                        ...
                        <div class="col-md-3">
                          <input type="text" class="form-control input-sm" name="vendorid" id="inputVendorID" placeholder="Auto generate" readonly required>
                        </div>
                      </div>
                      <div class="form-group " style="margin-bottom:1px;">
                        <label class="col-md-4 label-sm">Vendor Company</label>
                        <div class="col-md-7">
                          <input class="form-control input-sm" id="vendorNameCompany" name="vendorcompany" required>
                        </div>
                      </div>
                      <div class="form-group" style="margin-bottom:1px;">
                ...
                <tbody id="tbl-mon">
                  ...
                  foreach ($supp as $r) { ?>
                    <tr style="cursor: pointer;">
                      <td nowrap>
                        <a class="btn-sm btn-warning" href="<?php echo site_url('purchasing_zht/vendor_edit?vendor=' . $r->supplierid); ?>"><i class="fa fa-pencil"></i></a>
                        <a class="btn-sm btn-danger" href="<?php echo site_url('purchasing_zht/vendor_delete?vendor=' . $r->supplierid); ?>" onclick="javasciprt: return confirm('Are you sure delete Company <?php echo $r->suppliercompany; ?> ?')"><i class="fa fa-trash"></i></a>
                      </td>
...
<script>
  function search() {
    ...
    $.ajax({
      url: "<?php echo base_url(); ?>purchasing_zht/vendor_search?vendor=" + $search + "",
...
<script type="text/javascript">
  $(document).ready(function () {
    $('#vendorNameCompany').on('blur change', function () {
      var vendorName = $(this).val().trim();

      if (vendorName !== '') {
        $('#inputVendorID').attr('readonly', true);
        $('#inputVendorID').attr('placeholder', 'Auto Generate...');

        $.get("<?= base_url(); ?>Purchasing_zht/newGenerateSuplierPurchasing", {
          vendorNameCompany: vendorName
        }, function (data) {
          $('#inputVendorID').val(data);
        });
      } else {
        $('#inputVendorID').val('').attr('placeholder', 'Kosong');
      }
    });
  });
</script>