	...
	<div class="container-fluid">
		...
		<div class="hor-menu ">
			<ul class="nav navbar-nav">
				...
				$head_menu = $this->db->get_where('gen_vw_utl_menu_access', $param1)->result();
				
				foreach ($head_menu as $h) {
					// sql dibawah ini untuk mendeteksi apakah pakai menu classic atau mega menu
					$sql = 'select count(menuhdr_id) as rec_count from gen_vw_utl_menu where menuhdr_id='.$h->menu_id.' and menudtlsub_id is null';
					...
					if ($row) {
						if ($row->rec_count > 0){
							...
							$detail_menu = $this->db->get_where('gen_vw_utl_menu_access', $param2)->result();
							...
							$row = $this->db->query('select column_group from gen_tbl_utl_menu_dtl where menuhdr_id='.$h->menu_id.' order by column_group desc limit 1')->row();
							
							if ($row){
								$menu_width = $row->column_group * 355;
							...
							for ($column = 1; $column < 4; $column ++){
								...
								foreach ($detail_menu as $d){
									...
									$this->db->order_by('menu_id');
									$detailsub_menu = $this->db->get_where('gen_vw_utl_menu_access', $param3)->result();
